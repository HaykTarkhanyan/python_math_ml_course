...
\begin{tikzpicture}[
  ...
  \node[sbox] at (0, -1.2) {\textbf{Tools:} Permutation tests (no assumptions). Multiple testing: Bonferroni or BH.};
\end{tikzpicture}
\end{center}
...
\fcolorbox{orange1}{orange1!5}{\parbox{10cm}{\centering\small
  ...
  The $z$-test, $t$-test, and $\chi^2$-test are all special cases of the LRT.
}}
\end{center}
\end{frame}
...
\fcolorbox{orange1}{orange1!5}{\parbox{10cm}{\centering\footnotesize
  \textbf{Default to Welch's.} Correct whether variances are equal or not.
  The pooled version only gains a tiny bit of power when $\sigma_1 = \sigma_2$.\\[2pt]
  The Welch df formula is ugly --- in practice, software computes it automatically.
}}
\end{center}
\end{frame}
...
\vspace{0.05cm}
{\footnotesize\textcolor{gray}{Effect size: Cram\'er's $V = \sqrt{X^2/(n \cdot \min(r{-}1,c{-}1))} = \sqrt{24/200} = 0.35$ (medium). \;\textbf{Small expected counts?} Use Fisher's exact test (\texttt{scipy.stats.fisher\_exact}).}}
\end{frame}

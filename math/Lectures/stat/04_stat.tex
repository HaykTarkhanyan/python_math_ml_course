...
\subtitle{Score Function $\cdot$ Fisher Information $\cdot$ CR Bound $\cdot$ Efficiency $\cdot$ Admissibility}
\date{}
...
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!5, rounded corners=10pt, text width=12cm, align=center, inner sep=12pt] {
    For $X_i \sim N(\mu, \sigma^2)$, the sample mean $\bar{X}$ estimates $\mu$ with $\text{Var}(\bar{X}) = \sigma^2/n$.\\[8pt]
...
\begin{center}
\fcolorbox{popblue}{popblue!5}{\parbox{10cm}{\centering\small
  \textbf{Key property:} $\;\mathbb{E}[s(\theta)] = 0$ at the true $\theta$.\\[2pt]
  The score points in the right direction on average, but \textbf{cancels out}.\\
  What matters is how much it \textbf{varies} --- that's Fisher information.
}}
\end{center}

\vspace{0.1cm}
\begin{center}
\fcolorbox{orange1}{orange1!5}{\parbox{11cm}{\small
  \textbf{Why does the score average to zero?}\; Not because of unbiasedness --- it's a property of the model itself.
  Any density integrates to $1$: \;$\int f(x \mid \theta)\,dx = 1$.\\[2pt]
  Differentiate both sides w.r.t.\ $\theta$: \;$\int \frac{\partial f}{\partial\theta}\,dx = 0$.\\[2pt]
  Since $\frac{\partial f}{\partial\theta} = f \cdot \frac{\partial \log f}{\partial\theta} = f \cdot s$, \;this gives \;$\int s \cdot f\,dx = \mathbb{E}[s(\theta)] = 0$.\quad\textcolor{paramgreen}{$\checkmark$}
}}
\end{center}
\end{frame}

% --- Score function: interpretation + plot ---
\begin{frame}
\frametitle{Reading the Score Function}
...
\begin{itemize}\setlength{\itemsep}{4pt}
  \item When $x = 1$: $\;s(p) = \frac{1}{p}$\\
    {\footnotesize Score is positive: ``$p$ should be \textbf{higher}''}
  \item When $x = 0$: $\;s(p) = \frac{-1}{1-p}$\\
    {\footnotesize Score is negative: ``$p$ should be \textbf{lower}''}
  \item Near $p = 0$ or $p = 1$: score is \textbf{huge}\\
    {\footnotesize The data is very ``surprising'' $\to$ strong signal}
  \item Near $p = 0.5$: score is \textbf{moderate}\\
    {\footnotesize Neither outcome is very surprising}
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\begin{center}
\begin{tikzpicture}
  \begin{axis}[
    ...
    legend style={at={(0.03,0.03)}, anchor=south west, font=\scriptsize,
      fill=white, fill opacity=0.85, draw=gray!40},
  ]
    \addplot[sampred, domain=0.05:0.95] {1/x};
    \addlegendentry{$x=1$: ``raise $p$''}
    \addplot[popblue, domain=0.05:0.95] {-1/(1-x)};
    \addlegendentry{$x=0$: ``lower $p$''}
    \draw[dashed, gray!50, thin] (axis cs:0, 0) -- (axis cs:1, 0);
  \end{axis}
\end{tikzpicture}
\end{center}
\end{column}
...
\vspace{0.1cm}
\centering\small The score is like a \textbf{compass needle}: it always points toward the true $p$, but swings more when the data is surprising.
\end{frame}
...
The score averages to zero, but it \textbf{varies}. More variation means different values of $\theta$ produce \textbf{noticeably different} data --- making $\theta$ easier to pinpoint:
...
\vspace{0.1cm}
$$\boxed{I(\theta) = \underbrace{\mathbb{E}\!\left[s(\theta)^2\right]}_{\text{variance of the score}} = \underbrace{-\mathbb{E}\!\left[\frac{\partial^2}{\partial\theta^2}\log f(X \mid \theta)\right]}_{\text{expected curvature of } \ell}}$$

\pause
\vspace{0.1cm}
\textbf{Intuition:} Why should score variance $=$ curvature?

\begin{itemize}\setlength{\itemsep}{2pt}
  \item The score $s = \ell'$ is the \textbf{slope} of the log-likelihood
  \item At the true $\theta$, the slope averages to $0$: \;$\mathbb{E}[\ell'] = 0$
  \item A \textbf{sharply curved} $\ell$ (large $|\ell''|$) $\to$ slope swings far from $0$ $\to$ high $\text{Var}(s)$
  \item A \textbf{flat} $\ell$ (small $|\ell''|$) $\to$ slope barely moves $\to$ low $\text{Var}(s)$
\end{itemize}

\pause
\vspace{0.05cm}
\textbf{Formally:} Differentiate $\mathbb{E}[s] = 0$ w.r.t.\ $\theta$. Using the product rule under $\int$ (note: $\tfrac{\partial f}{\partial\theta} = s \cdot f$):
$$0 = \int \underbrace{\frac{\partial s}{\partial\theta}}_{\ell''}\! f\, dx + \int s \cdot \underbrace{\frac{\partial f}{\partial\theta}}_{s \cdot f}\, dx = \mathbb{E}[\ell''] + \mathbb{E}[s^2] \quad\Longrightarrow\quad \mathbb{E}[s^2] = -\mathbb{E}[\ell''] \;\;\textcolor{paramgreen}{\checkmark}$$
\end{frame}

% --- Verify two forms with Bernoulli ---
\begin{frame}
\frametitle{Verifying the Two Forms: Bernoulli}
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textcolor{popblue}{\textbf{Form 1: Variance of score}}
...
\end{column}
\begin{column}{0.48\textwidth}
\textcolor{sampred}{\textbf{Form 2: Expected curvature}}
...
\fcolorbox{paramgreen}{paramgreen!5}{\parbox{10cm}{\centering\small
  Both forms give $I(p) = \frac{1}{p(1-p)}$. \;\textcolor{paramgreen}{\checkmark}\\[2pt]
  In practice, Form~2 ($-\mathbb{E}[\ell'']$) is usually easier to compute.
}}
\end{center}
\end{frame}

% --- Fisher info: common distributions + additivity ---
\begin{frame}
\frametitle{Fisher Information: Beyond Bernoulli}
...
\begin{tabular}{@{}lccc@{}}
  \textbf{Distribution} & $\ell''(\theta)$ & $I(\theta)$ & \textbf{Intuition} \\
  \hline
  $\text{Bern}(p)$ & $-\frac{x}{p^2} - \frac{1{-}x}{(1{-}p)^2}$ & $\frac{1}{p(1{-}p)}$ & Fair coin = hardest to pin down \\[4pt]
  $N(\mu, \sigma_0^2)$ & $-\frac{1}{\sigma_0^2}$ & $\frac{1}{\sigma_0^2}$ & Low noise $\to$ more info \\[4pt]
  $\text{Pois}(\lambda)$ & $-x/\lambda^2$ & $\frac{1}{\lambda}$ & Rare events $\to$ more info \\[4pt]
...
\pause
\vspace{0.05cm}
\textbf{For $n$ i.i.d.\ observations:}\; the score is a sum $s_n = \sum_{i=1}^n s_i$ of i.i.d.\ terms, so:
$$I_n(\theta) = \text{Var}(s_n) = n\,\text{Var}(s_1) = n \cdot I(\theta)$$

\vspace{-0.15cm}
\begin{center}
\fcolorbox{popblue}{popblue!5}{\parbox{10cm}{\centering\small
  \textbf{Fisher information is additive:}\; $I_n(\theta) = n \cdot I(\theta)$.\\
...
% --- Transition to Cramér-Rao ---
\begin{frame}
\begin{center}
\vspace{1cm}
{\LARGE\bfseries\textcolor{popblue}{The Cram\'er--Rao Bound}}

\vspace{0.6cm}
{\large We've quantified how much \textbf{information} each observation carries.}

\vspace{0.4cm}
{\large Now: is there a \textbf{fundamental limit} on how precise\\[4pt]
...
\begin{frame}
\frametitle{Efficient Estimators}

\begin{center}
\begin{tikzpicture}
  \node[draw=paramgreen!70, fill=paramgreen!5, rounded corners=10pt, text width=12cm, align=center, inner sep=12pt] {
    {\large\textbf{Definition:}} An unbiased estimator $\hat\theta$ is \textbf{\textcolor{paramgreen}{efficient}} if\\[6pt]
    $\text{Var}(\hat\theta) = \dfrac{1}{n \cdot I(\theta)}$\\[8pt]
    \small i.e., it \textbf{achieves the Cram\'er--Rao lower bound exactly}.
  };
\end{tikzpicture}
\end{center}
...
\vspace{0.2cm}
\small
\textbf{What does efficiency mean in practice?}
\begin{itemize}\setlength{\itemsep}{4pt}
  \item You \textbf{cannot do better} among unbiased estimators --- it extracts all available information
  \item No data is ``wasted'' --- every observation contributes maximally
  \item If your estimator is efficient, \textbf{stop looking} for a better unbiased one
  \item If it's \textbf{not} efficient, there might be room for improvement
\end{itemize}

\vspace{0.15cm}
\begin{center}
\fcolorbox{orange1}{orange1!5}{\parbox{10cm}{\centering\small
  \textbf{Not all models have efficient estimators!}\\[2pt]
  But when one exists, it's usually the MLE (for large $n$, the MLE is \textbf{asymptotically efficient}).
}}
\end{center}
\end{frame}
...
% --- Transition to admissibility ---
\begin{frame}
\begin{center}
\vspace{1cm}
{\LARGE\bfseries\textcolor{popblue}{Beyond Unbiasedness}}

\vspace{0.6cm}
{\large The CR bound only applies to \textbf{unbiased} estimators.}

\vspace{0.4cm}
{\large What if we \textbf{allow bias} to reduce MSE?\\[4pt]
We need new criteria to compare estimators\ldots}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Two Approaches to Estimation}
\begin{center}
\begin{tikzpicture}[
  rbox/.style={draw, rounded corners=5pt, minimum width=5.5cm, minimum height=3cm, align=center, text width=5.2cm, inner sep=5pt, font=\small}
]
  \node[rbox, fill=popblue!8] at (-3.5, 0) {
    \textbf{\normalsize Plug-in (Unbiased)}\\[6pt]
    Use sample statistic directly\\
    ($\bar{X}$, $S^2$, $\hat{p}$)\\[6pt]
    Simple and intuitive\\
    CR bound measures precision\\[2pt]
    Efficient $=$ best possible
  };
  \node[rbox, fill=paramgreen!8] at (3.5, 0) {
    \textbf{\normalsize Minimax}\\[6pt]
    Minimize worst-case risk\\
    across all values of $\theta$\\[6pt]
    May introduce bias\\
    Conservative guarantee\\[2pt]
...
\fcolorbox{violet1}{violet1!5}{\parbox{11cm}{\centering\small
  \textbf{Coming soon:} Bayesian estimation adds a \textbf{prior} on $\theta$, which acts as\\
  automatic \textbf{regularization} --- pulling estimates toward a central value.\\[2pt]
...
\begin{tikzpicture}[
  ...
  \node[fbox] at (0, 2.15) {\textbf{Point estimation:} How to \textit{construct} estimators --- MoM, MLE (Lecture~5)};
  \node[fbox] at (0, 1.3) {\textbf{Bayesian estimation:} Priors, posteriors, MAP, regularization (Lecture~6)};
  \node[fbox] at (0, 0.45) {\textbf{Sampling distributions:} How estimators behave across samples, CLT (Lecture~7)};
  \node[fbox] at (0, -0.4) {\textbf{CIs \& Bootstrap:} How uncertain is our estimate? Resampling methods (Lecture~8)};
  \node[fbox] at (0, -1.25) {\textbf{Hypothesis testing:} Is the effect real or just noise? (Lectures~9--10)};
  \node[fbox] at (0, -2.1) {\textbf{ANOVA \& regression:} Comparing groups and modeling relationships (Lectures~11--13)};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
  ...
  \node[sbox] at (0, -2.25) {\textbf{Efficiency:} Achieves the CR bound. Best possible among unbiased.};
  \node[sbox] at (0, -3.15) {\textbf{Admissibility / Minimax:} Compare estimators even when biased.};
\end{tikzpicture}
\end{center}
\end{frame}

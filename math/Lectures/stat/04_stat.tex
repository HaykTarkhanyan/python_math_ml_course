...
\begin{tikzpicture}[
  ...
  \node[fbox] at (0, 0.45) {\textbf{Computational methods:} EM algorithm, MCMC for complex models (Lectures~7--8)};
  \node[fbox] at (0, -0.4) {\textbf{Confidence intervals:} How uncertain is our estimate? (Lecture~9)};
  \node[fbox] at (0, -1.25) {\textbf{Bootstrap:} Resampling to estimate uncertainty without formulas (Lecture~10)};
  \node[fbox] at (0, -2.1) {\textbf{Hypothesis testing:} Is the effect real or just noise? (Lectures~11--12)};
\end{tikzpicture}
\end{center}

...
\]
From the standard normal table: $P(Z \geq 2.0) = 1 - \Phi(2.0) = 1 - 0.9772 = 0.0228$.
\[
...
The p-value is $P(\text{data this extreme} \mid H_0 \text{ true})$, \textbf{not} $P(H_0 \text{ true} \mid \text{data})$. These are very different quantities -- confusing them is sometimes called the ``prosecutor's fallacy'' or ``transposing the conditional.''

To see why they differ, consider an analogy: $P(\text{spots} \mid \text{dalmatian}) = 1$, but $P(\text{dalmatian} \mid \text{spots}) \ll 1$ (leopards, ladybugs, and many other things have spots). Conditioning in one direction doesn't tell you the probability in the other direction without Bayes' theorem and a prior.
...
\begin{itemize}
  ...
  \item If $H_0$ is \textbf{false}: the replication rate depends on the \textbf{power} of the study (which depends on the true effect size and sample size). A single observed $p = 0.03$ does not tell you the power.
\end{itemize}

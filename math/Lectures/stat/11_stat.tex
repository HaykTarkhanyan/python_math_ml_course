...
\subtitle{Everything We Learned in 10 Lectures (+ a Preview of What's Next)}
\date{}
...
\begin{tikzpicture}[
  ...
  \node[phase, fill=paramgreen!10, dashed] (model) at (3, 0) {
    \textbf{Model}\\[2pt]
    \scriptsize ML preview
  };
  \node[phase, fill=violet1!10, dashed] (cause) at (6, 0) {
    \textbf{Cause}\\[2pt]
    \scriptsize ML preview
  };
...
\begin{tikzpicture}[
  ...
  \node[sbox] at (0, -0.5) {\textbf{L9--L10:} Is the pattern real? Hypothesis tests, ANOVA, A/B testing.};
  \node[sbox, dashed] at (0, -1.5) {\textbf{Preview:} How do variables relate? Regression, GLMs. \textit{(in ML course)}};
  \node[sbox, dashed] at (0, -2.5) {\textbf{Preview:} Did $X$ \textit{cause} $Y$? DAGs, potential outcomes, causal inference. \textit{(in ML course)}};
\end{tikzpicture}
\end{center}
\end{frame}
...
% ============================================================
\begin{frame}
\frametitle{L10 (cont.): ANOVA \& A/B Testing}
...
% ============================================================
\begin{frame}
\frametitle{Preview: Regression Inference \& GLMs \textit{(in ML course)}}
...
% ============================================================
\begin{frame}
\frametitle{Preview: Causal Inference \textit{(in ML course)}}
...
\begin{tikzpicture}[
  ...
  \node[lnode] (ak) at (1.8, -0.5) {$k$ groups:\\ANOVA (L10)};
  \draw[arr] (comp) -- (t2);
  \draw[arr] (comp) -- (ak);

  % Model branch
  \node[lnode, dashed] (ols) at (4, -0.5) {Continuous $Y$:\\OLS \scriptsize($\to$ ML)};
  \node[lnode, dashed] (glm) at (7, -0.5) {Binary/Count $Y$:\\GLM \scriptsize($\to$ ML)};
  \draw[arr] (rel) -- (ols);
  \draw[arr] (rel) -- (glm);

  % Causal layer
  \node[draw=violet1, fill=violet1!8, rounded corners=5pt, text width=11cm, align=center, inner sep=4pt, font=\small] (caus) at (0, -2.2) {
    \textbf{Is the question causal?} $\to$ Need RCT or causal assumptions \textit{($\to$ ML)}
  };
...
\begin{tikzpicture}
  \node[inner sep=0pt] {
    ...
    \begin{tabular}{l|l|l}
      ...
      OLS regression \textit{(ML)} & \texttt{statsmodels} & \texttt{OLS().fit().summary()} \\
      \rowcolor{popblue!8}
      GLMs \textit{(ML)} & \texttt{statsmodels} & \texttt{GLM(family=...).fit()} \\
      Logistic \textit{(ML)} & \texttt{statsmodels} & \texttt{Logit().fit()} \\
      \rowcolor{popblue!8}
      Causal inference \textit{(ML)} & \texttt{dowhy} & \texttt{CausalModel()} \\
      Prop.\ scores \textit{(ML)} & \texttt{causalml} & \texttt{PropensityScoreMatching()} \\
    \end{tabular}
  };
\end{tikzpicture}
\end{center}
...
\begin{center}
  ...
  \vspace{0.8cm}
  {\large 10 lectures $\cdot$ From data to inference (and a glimpse beyond)}
...
% ============================================================
% BONUS: DELTA METHOD (slides we never got to in L8 CI/Bootstrap session)
% ============================================================
\begin{frame}
\frametitle{Bonus: The Delta Method \textit{(slides we never got to in L8)}}

\begin{center}
\begin{tikzpicture}
  \node[draw=popblue!40, fill=popblue!4, rounded corners=5pt, text width=12cm, align=left, inner sep=8pt, font=\small] {
    \textbf{The setup.} You've estimated $\hat\theta$ with
    $\sqrt{n}\bigl(\hat\theta - \theta\bigr) \;\to\; N(0, \sigma^2)$.\\[4pt]
    \textbf{The question.} You actually care about a \emph{transformed} quantity $g(\hat\theta)$:
    \begin{itemize}\itemsep1pt
      \item $\hat p \;\to\;$ log-odds $\log\bigl(\hat p / (1-\hat p)\bigr)$
      \item $\bar X_n \;\to\;$ rate $1/\bar X_n$ (events per unit time)
      \item $(\hat\mu_1, \hat\mu_2) \;\to\;$ ratio $\hat\mu_1 / \hat\mu_2$
    \end{itemize}
...
\begin{center}
\begin{tikzpicture}
  \node[draw=orange1, fill=orange1!8, rounded corners=5pt, text width=12cm, align=center, inner sep=6pt, font=\small] {
    \textbf{Delta method.} If $g$ is differentiable at $\theta$ with $g'(\theta) \neq 0$,
    \[
      \sqrt{n}\bigl(g(\hat\theta) - g(\theta)\bigr) \;\xrightarrow{d}\; N\!\bigl(0,\; [g'(\theta)]^2 \sigma^2\bigr).
    \]
  };
\end{tikzpicture}
\end{center}
...
\vspace{0.1cm}
\begin{center}
\small\textit{Idea: Taylor-expand $g$ around $\theta$, keep the linear term, transfer the variance.}
\end{center}
\end{frame}

% ============================================================
\begin{frame}
\frametitle{Delta Method: CI on Log-Odds (Worked Example)}

\textbf{Problem.} Observe $X \sim \text{Bin}(n, p)$, $\hat p = X/n$. Build a 95\% CI on the \emph{log-odds} $\log\bigl(p/(1-p)\bigr)$.

\vspace{0.1cm}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\small
\textbf{Step 1.} Sampling distribution of $\hat p$:
\[
\hat p \;\dot\sim\; N\!\left(p,\; \frac{p(1-p)}{n}\right)
\]

\textbf{Step 2.} Compute $g'(p)$:
\[
...
\[
\widehat{\log\text{-odds}} \;\dot\sim\; N\!\left(\log\!\frac{p}{1-p},\; \frac{1}{n\,p(1-p)}\right)
\]
\end{column}
\begin{column}{0.5\textwidth}
\small
\textbf{Step 4.} Plug in $\hat p$, build the CI:
\[
\widehat{\text{SE}} = \sqrt{\tfrac{1}{n\,\hat p\,(1-\hat p)}}, \qquad \text{CI} = \log\tfrac{\hat p}{1-\hat p} \pm 1.96\,\widehat{\text{SE}}
\]

\textbf{Numbers.} $X = 30,\, n = 100 \;\Rightarrow\; \hat p = 0.3$:
\begin{align*}
\widehat{\log\text{-odds}} &= \log(0.3/0.7) \approx -0.847 \\
\widehat{\text{SE}} &= \sqrt{\tfrac{1}{100 \cdot 0.21}} \approx 0.218 \\
\text{95\% CI:} &\;\;-0.847 \pm 0.428 = (-1.28,\, -0.42)
\end{align*}
\end{column}
\end{columns}
...
\begin{center}
\begin{tikzpicture}
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=5pt, text width=12cm, align=center, inner sep=5pt, font=\small] {
    \textbf{Why bother?} Wald CI on log-odds, then back-transform $\to$ CI on odds, is more accurate than direct Wald CI on $\hat p$ when $\hat p$ is near 0 or 1. Same trick works for variance, ratios, $\exp/\log$ of any estimate.
  };
\end{tikzpicture}
\end{center}
\end{frame}

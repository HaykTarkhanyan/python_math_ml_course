...
\fcolorbox{violet1}{violet1!5}{\parbox{10cm}{\centering\small
  \textbf{$p$-hacking:} trying multiple analyses until $p < 0.05$. Examples:\\[2pt]
  Drop outliers selectively $\cdot$ Add/remove covariates $\cdot$ Test subgroups $\cdot$ Transform the outcome\\[2pt]
  Each ``researcher degree of freedom'' inflates the false positive rate.
}}
\end{center}
\end{frame}
...
{\footnotesize\textcolor{warnred}{\textbf{Warning:} Cohen's benchmarks (0.2/0.5/0.8) are \textbf{context-dependent}. In education, $d = 0.2$ can be large. Always define the \textbf{smallest effect size of interest} (SESOI) from domain knowledge, not from a table.}}
\end{frame}

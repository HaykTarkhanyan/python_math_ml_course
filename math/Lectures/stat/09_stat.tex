...
\begin{frame}
\frametitle{Reading the $z$-Table}
\framesubtitle{Standard Normal cumulative distribution: $\Phi(z) = P(Z \leq z)$}

\small
The table gives $\Phi(z)$: the area to the \textbf{left} of $z$ under $N(0,1)$.
...
\begin{tikzpicture}
  ...
  \node[font=\footnotesize\ttfamily, inner sep=0pt] {
    \begin{tabular}{c|cccccc}
      $z$ & \textbf{.00} & \textbf{.02} & \textbf{.04} & \textbf{.05} & \textbf{.06} & \textbf{.08} \\
      \hline
      \textbf{1.6} & .9452 & .9474 & .9495 & .9505 & .9515 & .9535 \\
      \textbf{1.7} & .9554 & .9573 & .9591 & \cellcolor{popblue!15}.9599 & .9608 & .9625 \\
      \rowcolor{yellow!20}
      \textbf{1.8} & .9641 & .9656 & .9671 & .9678 & .9686 & .9699 \\
      \textbf{1.9} & .9713 & .9726 & .9738 & .9744 & .9750 & .9761 \\
      \rowcolor{sampred!10}
      \textbf{2.0} & .9772 & .9783 & .9793 & .9798 & .9803 & .9812 \\
    \end{tabular}
  };
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
  ebox/.style={draw, rounded corners=5pt, text width=5.5cm, align=left, inner sep=6pt, font=\small}
]
  \node[ebox, fill=popblue!8] at (-4.2, 0) {
    \textbf{Example 1: Find $P(Z \leq 1.76)$}\\[3pt]
    Row \textbf{1.7} + column \textbf{.06}:\\
    $\Phi(1.76) = 0.9608$\\[3pt]
    So $P(Z > 1.76) = 1 - 0.9608$\\
    $= 0.0392$
  };

  \node[ebox, fill=orange1!8] at (4.2, 0) {
    \textbf{Example 2: Where is $z_{0.025}$?}\\[3pt]
    We need $P(Z > z) = 0.025$,\\
    i.e.\ $\Phi(z) = 0.975$.\\[3pt]
    Scan the table: $\Phi(1.96) = 0.9750$\\
    $\Rightarrow z_{0.025} = 1.96$ \;\checkmark
  };
\end{tikzpicture}
\end{center}
...
\vspace{0.1cm}
\begin{center}
\small\textbf{Drug trial p-value:} $p = 2 \times P(Z > 1.76) = 2 \times 0.0392 = 0.0784$. (Two-sided: multiply by 2.)
\end{center}
\end{frame}

\begin{frame}
\frametitle{Reading the $t$-Table}
\framesubtitle{Use when $\sigma$ is unknown and $n$ is small (replaces $z$)}

\small
The $t$-distribution has heavier tails than $N(0,1)$. Critical values depend on \textbf{degrees of freedom} (df).
...
\begin{tikzpicture}
  \node[font=\footnotesize\ttfamily, inner sep=0pt] {
    \begin{tabular}{c|ccccc}
      & \multicolumn{5}{c}{\textbf{Upper-tail probability}} \\
      \textbf{df} & \textbf{0.100} & \textbf{0.050} & \textbf{0.025} & \textbf{0.010} & \textbf{0.005} \\
      \hline
      5 & 1.476 & 2.015 & 2.571 & 3.365 & 4.032 \\
      10 & 1.372 & 1.812 & 2.228 & 2.764 & 3.169 \\
      \rowcolor{popblue!10}
      24 & 1.318 & 1.711 & \cellcolor{popblue!20}\textbf{2.064} & 2.492 & 2.797 \\
      30 & 1.310 & 1.697 & 2.042 & 2.457 & 2.750 \\
      60 & 1.296 & 1.671 & 2.000 & 2.390 & 2.660 \\
      $\infty$ & 1.282 & 1.645 & \textbf{1.960} & 2.326 & 2.576 \\
    \end{tabular}
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
  ...
  \node[ebox, fill=popblue!8] at (-4.2, 0) {
    \textbf{Coffee shop example}\\[3pt]
    $n = 25$ $\Rightarrow$ df $= 24$\\
    For 95\% CI (two-sided):\\
    $\alpha/2 = 0.025$ column\\
    $\Rightarrow t_{0.025, 24} = \mathbf{2.064}$\\[2pt]
    (wider than $z = 1.96$!)
  };

  \node[ebox, fill=paramgreen!8] at (4.2, 0) {
    \textbf{Key observations}\\[3pt]
    Small df $\Rightarrow$ larger critical values\\
    (more uncertainty $\Rightarrow$ wider intervals)\\[3pt]
    df $= \infty$ row $=$ the $z$-values\\
    (as $n \to \infty$, $t \to z$)
  };
\end{tikzpicture}
\end{center}

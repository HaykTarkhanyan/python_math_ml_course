...
\vspace{-0.1cm}
$$\hat\sigma^2 = \tfrac{1}{n}\textstyle\sum X_i^2 - \bar{X}^2 = \tfrac{1}{n}\textstyle\sum X_i^2 - 2\bar{X}^2 + \bar{X}^2 = \tfrac{1}{n}\textstyle\sum(X_i - \bar{X})^2$$
...
\fcolorbox{orange1}{orange1!5}{\parbox{11cm}{\centering\small
  ...
  it's using Fisher information under the hood. More in Lecture~9.
}}
\end{center}
\end{frame}
...
For latent variables: \textbf{EM algorithm} (Lecture~7)
\end{column}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[rbox] at (0, 2.7) {
    \textbf{\textcolor{popblue}{Interactive: MLE \& Likelihood}}\\[2pt]
    \footnotesize\texttt{rpsychologist.com/likelihood} --- drag sliders to see how likelihood, score, and Fisher info change in real time. The best interactive MLE demo available.
  };
  \node[rbox] at (0, 1.35) {
    \textbf{\textcolor{popblue}{Interactive: Seeing Theory (Brown University)}}\\[2pt]
    \footnotesize\texttt{seeing-theory.brown.edu/frequentist-inference} --- beautiful animations for point estimation, confidence intervals, and bootstrap. Chapter~5 covers MLE.
  };
  \node[rbox] at (0, 0.0) {
    \textbf{\textcolor{popblue}{Video: StatQuest --- MLE, clearly explained}}\\[2pt]
    \footnotesize\texttt{statquest.org/statquest-maximum-likelihood-clearly-explained} --- 6-minute visual walkthrough of MLE with the Normal distribution. Great for review.
  };
  \node[rbox] at (0, -1.35) {
    \textbf{\textcolor{popblue}{Interactive: MLU-Explain (Amazon)}}\\[2pt]
    \footnotesize\texttt{mlu-explain.github.io} --- visual articles on logistic regression, cross-validation, and neural networks. See how MLE powers these ML methods.
  };
  \node[rbox] at (0, -2.7) {
    \textbf{\textcolor{popblue}{Reading: Penn State STAT 415}}\\[2pt]

...
\begin{tikzpicture}[
  ...
  \node[sbox] at (0, -1.2) {\textbf{Admissibility \& Minimax:} Compare estimators by MSE across all $\theta$; minimize worst-case risk.};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!5, rounded corners=10pt, text width=11cm, align=center, inner sep=15pt] {
    A hospital records 30 patient recovery times (in days):\\[4pt]
    {\small $5,\; 12,\; 8,\; 3,\; 15,\; 7,\; 9,\; 11,\; 6,\; 14,\;\ldots$}\\[10pt]
    {\Large What distribution do these come from? Which parameters?}\\[10pt]
    In Lectures~3--4 we learned how to \textbf{judge} estimators (bias, variance, MSE, efficiency).\\[4pt]
    \textbf{But we never said how to actually \textit{find} an estimator!}\\
    Today: two systematic recipes for \textbf{constructing} estimators.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Method of Moments: The Key Insight}

\small
Every distribution's \textbf{population moments} are functions of its parameters:

\vspace{0.15cm}
\begin{center}
\begin{tikzpicture}[
  mombox/.style={draw, rounded corners=6pt, text width=5.2cm, align=center, font=\normalsize, inner sep=8pt}
]
  \node[mombox, fill=paramgreen!10, draw=paramgreen!60] (pop) at (-4, 0) {
    \textbf{\textcolor{paramgreen}{Population side}}\\[6pt]
    \small 1st moment: $\mu_1 = \mathbb{E}[X]$ \;\textcolor{gray}{(mean)}\\[2pt]
    2nd moment: $\mu_2 = \mathbb{E}[X^2]$ \;\textcolor{gray}{($\to$ variance)}\\[2pt]
    $k$-th moment: $\mu_k = \mathbb{E}[X^k]$
  };

  \node[mombox, fill=sampred!10, draw=sampred!60] (samp) at (4, 0) {
    \textbf{\textcolor{sampred}{Sample side}}\\[6pt]
    \small 1st moment: $\hat\mu_1 = \frac{1}{n}\sum X_i = \bar{X}$\\[2pt]
    2nd moment: $\hat\mu_2 = \frac{1}{n}\sum X_i^2$\\[2pt]
    $k$-th moment: $\hat\mu_k = \frac{1}{n}\sum X_i^k$
  };

  \draw[{Stealth[length=8pt]}-{Stealth[length=8pt]}, thick, popblue, dashed] (pop) -- (samp)
    node[midway, above, font=\small\bfseries] {set equal};
\end{tikzpicture}
\end{center}
...
\vspace{0.15cm}
\begin{center}
\fcolorbox{popblue}{popblue!5}{\parbox{12cm}{\centering\small
  \textbf{The MoM recipe:}\\[3pt]
  \textbf{1.}\;Write population moments as functions of parameters\quad
  \textbf{2.}\;Replace with sample moments\quad
  \textbf{3.}\;Solve for $\hat\theta$
}}
\end{center}

\vspace{0.1cm}
\centering\small $p$ unknown parameters $\Rightarrow$ need $p$ moment equations.

\vspace{0.05cm}
\centering\scriptsize\textcolor{gray}{Recall: \textbf{central} moments give variance ($k\!=\!2$), skewness ($k\!=\!3$), kurtosis ($k\!=\!4$). Raw moments work too.}
\end{frame}

\begin{frame}
\frametitle{MoM Example: Poisson (One Parameter)}

\small
\textbf{Setup:} $X_1, \ldots, X_n \sim \text{Pois}(\lambda)$. One unknown $\Rightarrow$ one equation.

\vspace{0.2cm}
\begin{center}
\begin{tikzpicture}[
  stp/.style={draw=popblue!60, fill=popblue!5, rounded corners=5pt, text width=12cm, align=left, inner sep=8pt, font=\small}
]
  \node[stp] (s1) at (0, 2.0) {
    \textbf{Step 1.}\;Population moment: $\;\mathbb{E}[X] = \lambda$ \quad{\footnotesize (the mean of a Poisson \textit{is} the parameter)}
  };
  \pause
  \node[stp] (s2) at (0, 0.8) {
    \textbf{Step 2.}\;Replace with sample moment: $\;\bar{X} = \hat\lambda$
  };
  \pause
  \node[stp] (s3) at (0, -0.4) {
    \textbf{Step 3.}\;Solve: $\;\boxed{\hat\lambda_{\text{MoM}} = \bar{X}}$ \quad\textcolor{paramgreen}{\checkmark} \quad Done --- nothing to solve!
  };
\end{tikzpicture}
\end{center}
...
\pause
\vspace{0.2cm}
\textbf{Concrete example:} A hospital records emergency arrivals per hour over $n = 30$ hours:

\vspace{0.05cm}
\centering\small $3,\; 5,\; 2,\; 4,\; 6,\; 1,\; 3,\; 4,\; 2,\; 5,\; \ldots$ \quad with $\bar{X} = 3.8$

\vspace{0.15cm}
$\Rightarrow\;\hat\lambda_{\text{MoM}} = 3.8$ arrivals per hour.

\vspace{0.15cm}
\begin{center}
\small \textcolor{popblue}{\textbf{One parameter, one moment, one line of algebra.}} That's MoM at its best.
\end{center}
\end{frame}

\begin{frame}
\frametitle{MoM Example: Normal (Two Parameters --- Full Derivation)}

\small
\textbf{Setup:} $X_1, \ldots, X_n \sim \mathcal{N}(\mu, \sigma^2)$. \;Two unknowns $\Rightarrow$ two equations.

\vspace{0.15cm}
\textbf{Step 1.} Write the first two population moments as functions of $\mu$ and $\sigma^2$:
\vspace{-0.1cm}
\begin{align*}
  \mu_1 &= \mathbb{E}[X] = \mu \\[-2pt]
  \mu_2 &= \mathbb{E}[X^2] = \text{Var}(X) + \bigl(\mathbb{E}[X]\bigr)^2 = \sigma^2 + \mu^2
\end{align*}

\vspace{-0.35cm}\pause
\textbf{Step 2.} Replace population moments with sample moments:
\vspace{-0.1cm}
\begin{align*}
  \bar{X} &= \hat\mu \\[-2pt]
  \tfrac{1}{n}\textstyle\sum X_i^2 &= \hat\sigma^2 + \hat\mu^2
\end{align*}

\vspace{-0.35cm}\pause
\textbf{Step 3.} Solve --- first equation gives $\hat\mu$ immediately; substitute into second:
\vspace{-0.1cm}
$$\hat\sigma^2 = \tfrac{1}{n}\textstyle\sum X_i^2 - \bar{X}^2 = \tfrac{1}{n}\textstyle\sum\bigl(X_i^2 - 2X_i\bar{X} + \bar{X}^2\bigr) = \tfrac{1}{n}\textstyle\sum(X_i - \bar{X})^2$$

\vspace{-0.1cm}
$$\boxed{\hat\mu_{\text{MoM}} = \bar{X}, \qquad \hat\sigma^2_{\text{MoM}} = \frac{1}{n}\sum_{i=1}^n(X_i - \bar{X})^2}$$

\vspace{0.05cm}
\centering\footnotesize Note: divides by $n$, not $n{-}1$ --- \textbf{biased!} Recall Bessel's correction from Lecture~3.
\end{frame}

\begin{frame}
\frametitle{MoM Example: Gamma Distribution}

\small
\textbf{Model:} $X_1, \ldots, X_n \sim \text{Gamma}(\alpha, \beta)$ \;\;(shape $\alpha$, rate $\beta$). \;Two unknowns.
...
\pause
\vspace{0.1cm}
$$\boxed{\hat\beta_{\text{MoM}} = \frac{\bar{X}}{S^2}, \qquad \hat\alpha_{\text{MoM}} = \frac{\bar{X}^2}{S^2}}$$

\vspace{0.1cm}
\small Simple algebra --- done! \textcolor{paramgreen}{\checkmark}

\vspace{0.1cm}
\footnotesize MLE for Gamma requires the digamma function $\psi(\alpha)$ --- \textbf{no closed form}, numerical optimization only.
\end{column}
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot[popblue, domain=0.05:8] {4*x^2*exp(-2*x)};
    \addlegendentry{$\alpha\!=\!3,\; \beta\!=\!2$}
    \addplot[sampred, domain=0.05:8] {x*exp(-x)};
...
\vspace{0.15cm}
\centering\small \textbf{Lesson:} MoM shines when MLE has no closed form. Quick, easy, often a good starting point.
\end{frame}
...
\vspace{0.1cm}
\centering\small
MoM uses only the \textbf{mean} of the data ($\bar{X}$), but for $\text{Uniform}(0, \theta)$ the mean\\
is \textit{not} the best summary. The \textbf{sufficient statistic} is $X_{(n)} = \max(X_i)$:\\[3pt]
knowing the maximum tells you almost exactly where $\theta$ is,\\
...
\begin{center}
  \vspace{1.5cm}
  {\Huge\bfseries\textcolor{popblue}{Maximum Likelihood}}\\[12pt]
  {\large Pick the parameter that makes the observed data most probable.}\\[8pt]
  {\normalsize Two worked examples, then the connection to machine learning.}
\end{center}
\end{frame}
...
\vspace{0.2cm}
\centering\small Let's work through two closed-form examples in detail.
\end{frame}
...
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!3, rounded corners=8pt, text width=12cm, align=left, inner sep=12pt] {
    ...
    \centering$\displaystyle\boxed{\hat{p}_{\text{MLE}} = \frac{k}{n} = \bar{X}}$\\[4pt]
...
\vspace{0.1cm}\pause
\textbf{Step 4.}\; Set $\dfrac{\partial\ell}{\partial(\sigma^2)} = 0$: \quad
$\underbrace{-\dfrac{n}{2\sigma^2}}_{\text{from }-\frac{n}{2}\log\sigma^2} + \underbrace{\dfrac{\sum(X_i - \bar{X})^2}{2\sigma^4}}_{\text{from }-\frac{1}{2\sigma^2}\sum(\cdots)^2} = 0$

\vspace{0.05cm}
\hfill Multiply both sides by $2\sigma^4$: \;$-n\sigma^2 + \sum(X_i - \bar{X})^2 = 0$\hspace{2.0cm}\null
...
$$y_i \mid \mathbf{x}_i \sim \mathcal{N}\!\bigl(f(\mathbf{x}_i;\mathbf{w}),\; \sigma^2\bigr)$$

\vspace{-0.3cm}\pause
Now treat $\mathbf{w}$ as the unknown parameter and write the log-likelihood --- same recipe as before, just with $f(\mathbf{x}_i;\mathbf{w})$ playing the role of $\mu$:
\vspace{-0.15cm}
...
\vspace{0.3cm}
\small
\textbf{The pattern:} write $\ell(\theta)$, differentiate, set to zero, solve.\\[6pt]
The same recipe works for Poisson ($\hat\lambda = \bar{X}$), Exponential ($\hat\lambda = 1/\bar{X}$), Gamma, etc.\\[6pt]
...
\begin{center}
  \vspace{1.5cm}
  {\Huge\bfseries\textcolor{popblue}{Properties of the MLE}}\\[12pt]
  {\large Invariance, identifiability, and what can go wrong.}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!8, rounded corners=10pt, text width=11cm, align=center, inner sep=12pt] {
    ...
    {\Large $\widehat{g(\theta)}_{\text{MLE}} = g(\hat{\theta}_{\text{MLE}})$}\\[8pt]
    \small\textbf{In words:} to get the MLE of a \textit{transformed} parameter,\\
    just transform the MLE. No need to re-derive from scratch.
  };
\end{tikzpicture}
\end{center}
...
\begin{itemize}\setlength{\itemsep}{3pt}
  \item You estimated $\hat{p}_{\text{MLE}} = 0.3$ for a coin
  \item Now you want the \textbf{odds}: $\frac{p}{1-p}$
  \item \textbf{Invariance:} just plug in!\\ $\widehat{\text{odds}} = \frac{0.3}{0.7} = 0.429$
  \item No need to re-derive the MLE of the odds from the likelihood
\end{itemize}

\vspace{0.1cm}
\small\textcolor{warnred}{MoM does \textbf{not} have this property.}
\end{column}
\begin{column}{0.48\textwidth}
\begin{center}
\begin{tikzpicture}[
...
\end{center}
\vspace{0.1cm}
\centering\footnotesize The two paths give the\\same answer --- the diagram commutes.
\end{column}
\end{columns}
\end{frame}

\begin{frame}
\frametitle{Identifiability: A Prerequisite for MLE}
\begin{center}
\begin{tikzpicture}
  \node[draw=warnred, fill=warnred!5, rounded corners=10pt, text width=11cm, align=center, inner sep=12pt] {
    \textbf{Before using MLE, ask: can we even recover $\theta$?}\\[8pt]
...
\vspace{0.2cm}
\textbf{When it fails} (the likelihood has \textbf{multiple maxima} with the same value):
\begin{itemize}\setlength{\itemsep}{2pt}\small
  \item \textbf{Mixtures:} $\frac{1}{2}N(-1, 1) + \frac{1}{2}N(3, 1)$ --- swap components, same distribution
  \item \textbf{Overparameterized:} $X \sim N(\alpha + \beta,\; 1)$ --- data reveals $\alpha + \beta$, not each separately
  \item \textbf{Neural nets:} Swap two hidden neurons and their weights --- same function, different $\theta$
\end{itemize}

\vspace{0.1cm}
\small Identifiability is a property of the \textbf{model}, not of MLE.\\
If the model is identifiable $\Rightarrow$ MLE has a unique maximizer (good).\\
If not $\Rightarrow$ MLE still finds \textit{a} maximum, but multiple $\theta$'s tie --- the answer is ambiguous.
\end{frame}
...
\vspace{0.1cm}
\footnotesize In Python: \texttt{scipy.optimize.minimize}\\[2pt]
For latent variables: \textbf{EM algorithm} (later lectures)
\end{column}
\end{columns}
\end{frame}

% ============================================================
\section{Why MLE Works}

\begin{frame}
\begin{center}
  \vspace{1.5cm}
  {\Huge\bfseries\textcolor{popblue}{Why MLE Works}}\\[12pt]
  {\large Connecting MLE to sufficiency, exponential families,}\\[4pt]
  {\large and the Cram\'er--Rao bound from Lectures~3--4.}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{MLE in Exponential Families: The General Recipe}

\small
Recall from Lecture~3: $\;f(x \mid \theta) = h(x)\exp\!\big(\eta(\theta)\,T(x) - A(\theta)\big)$

\vspace{0.1cm}
For $n$ i.i.d.\ observations, the log-likelihood depends on the data \textbf{only through} $\sum T(X_i)$:
\vspace{-0.15cm}
...
\vspace{-0.15cm}
$$\boxed{A'(\hat\theta_{\text{MLE}}) = \frac{1}{n}\sum_{i=1}^n T(X_i)}$$

\vspace{-0.15cm}\pause
\textbf{Let's verify with Poisson:}
\vspace{-0.1cm}
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}lllll@{}}
  \footnotesize Poisson: & \footnotesize $T(x) = x$, & \footnotesize $\eta = \log\lambda$, & \footnotesize $A(\eta) = e^\eta$, & \footnotesize $A'(\eta) = e^\eta = \lambda$ \\
\end{tabular}
\end{center}

\vspace{-0.1cm}
Formula says: $\hat\lambda = A'(\hat\eta) = \frac{1}{n}\sum X_i = \bar{X}$ \;\textcolor{paramgreen}{\checkmark} \;--- \textit{same answer we derived by hand!}

\vspace{0.1cm}
\begin{center}
\fcolorbox{paramgreen}{paramgreen!5}{\parbox{12cm}{\centering\small
  \textbf{One formula, every exponential family.} Plug in $T$ and $A$, get the MLE.\\[2pt]
  And since $\mathbb{E}[T(X)] = A'(\eta)$, the MLE is \textbf{exactly the MoM estimator}.
}}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Why MLE Works: Minimizing KL Divergence}

\small
There's a deeper reason MLE is optimal. Let $p_{\text{true}}$ be the true distribution.

\vspace{0.15cm}
\textbf{KL divergence} measures how ``far'' $p_\theta$ is from $p_{\text{true}}$:
\vspace{-0.1cm}
$$D_{\text{KL}}(p_{\text{true}} \,\|\, p_\theta) = \mathbb{E}_{p_{\text{true}}}\!\left[\log\frac{p_{\text{true}}(X)}{p_\theta(X)}\right] = \underbrace{\mathbb{E}[\log p_{\text{true}}(X)]}_{\text{const w.r.t.\ }\theta} - \underbrace{\mathbb{E}[\log p_\theta(X)]}_{\text{expected log-likelihood}}$$
...
\begin{center}
\fcolorbox{popblue}{popblue!5}{\parbox{11cm}{\centering\small
  \textbf{MLE finds the model closest to the truth} (in KL divergence).\\[3pt]
  Even if no $\theta$ gives the true distribution exactly,\\
  MLE finds the best approximation in the model family.
}}
\end{center}

\vspace{0.1cm}
\centering\small This is why MLE works well even when the model is \textbf{misspecified} --- it does the best it can.
\end{frame}
...
\small
\textbf{Example:} $X_i \sim \text{Bernoulli}(p_0)$, $p_0 = 0.3$, \;$I(p) = \frac{1}{p(1-p)}$. \;Var$(\hat{p}) = \frac{p(1-p)}{n}$.

\vspace{0.05cm}
As $n$ grows, the sampling distribution of $\hat{p}_{\text{MLE}}$ tightens around $p_0$:
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    % Bernoulli p=0.3: Var = 0.21/n
    % n=10: sigma^2 = 0.021, sigma = 0.1449
    \addplot[orange1!70, domain=0.01:0.59] {1/sqrt(2*pi*0.021)*exp(-(x-0.3)^2/(2*0.021))};
    \addlegendentry{$n = 10$\; ($\text{SE} = 0.14$)}

    % n=50: sigma^2 = 0.0042, sigma = 0.0648
    \addplot[popblue, domain=0.01:0.59] {1/sqrt(2*pi*0.0042)*exp(-(x-0.3)^2/(2*0.0042))};
    \addlegendentry{$n = 50$\; ($\text{SE} = 0.065$)}

    % n=200: sigma^2 = 0.00105, sigma = 0.0324
    \addplot[paramgreen, domain=0.01:0.59] {1/sqrt(2*pi*0.00105)*exp(-(x-0.3)^2/(2*0.00105))};
    \addlegendentry{$n = 200$\; ($\text{SE} = 0.032$)}

    % True value
    \draw[dashed, thick, sampred] (axis cs:0.3, 0) -- (axis cs:0.3, 16);
    \node[font=\small\bfseries, sampred, above left] at (axis cs:0.3, 15.5) {$p_0 = 0.3$};
  \end{axis}
\end{tikzpicture}
\end{center}
...
\vspace{-0.1cm}
\begin{center}
\small Variance shrinks as $\frac{1}{n\,I(p_0)} = \frac{p_0(1-p_0)}{n}$\,: more data $\Rightarrow$ tighter bell $\Rightarrow$ more precise estimate.
\end{center}
\end{frame}

\begin{frame}
\frametitle{From MLE to Standard Errors}

\small
Asymptotic normality says: $\;\hat\theta_{\text{MLE}} \;\dot\sim\; N\!\Big(\theta_0,\; \frac{1}{n\,I(\theta_0)}\Big)$

\vspace{0.15cm}
\textbf{Problem:} We don't know $\theta_0$ --- that's what we're estimating!

\pause
\vspace{0.15cm}
\textbf{Solution:} Plug in $\hat\theta$ to get the \textbf{standard error}:
\vspace{-0.1cm}
$$\boxed{\text{SE}(\hat\theta) = \frac{1}{\sqrt{n\,I(\hat\theta)}}} \qquad\text{or equivalently:}\qquad \boxed{\text{SE}(\hat\theta) = \frac{1}{\sqrt{J_n(\hat\theta)}}}$$

\vspace{-0.2cm}
where $J_n(\hat\theta) = -\ell''_n(\hat\theta)$ is the \textbf{observed} Fisher information (the actual curvature at the MLE).

\pause
\vspace{0.15cm}
\textbf{Example:} Bernoulli, $\hat{p} = 0.3$, $n = 100$.

\vspace{0.05cm}
\quad $I(p) = \frac{1}{p(1{-}p)}$ \;$\Rightarrow$\; $\text{SE} = \sqrt{\frac{\hat{p}(1{-}\hat{p})}{n}} = \sqrt{\frac{0.3 \cdot 0.7}{100}} = 0.046$

\vspace{0.15cm}
\begin{center}
\fcolorbox{orange1}{orange1!5}{\parbox{11cm}{\centering\small
  \textbf{This is how statistical software reports standard errors.}\\[2pt]
  Every time you see $\hat\theta \pm \text{SE}$ in R, Python, or a paper,\\
  it's using Fisher information under the hood. More in Lecture~7 (Confidence Intervals).
}}
\end{center}
\end{frame}

\begin{frame}
\frametitle{The Delta Method: SEs for Transformations}

\small
Often we want the SE of $g(\hat\theta)$, not $\hat\theta$ itself. \;(\textit{E.g.,} odds ratio from $\hat{p}$, or mean $1/\hat\lambda$ from Exp rate.)

\vspace{0.15cm}
\textbf{Delta method:} If $\hat\theta \;\dot\sim\; N(\theta_0, \sigma^2_{\hat\theta})$, then for smooth $g$:
\vspace{-0.1cm}
$$\boxed{g(\hat\theta) \;\dot\sim\; N\!\bigl(g(\theta_0),\; [g'(\theta_0)]^2 \cdot \sigma^2_{\hat\theta}\bigr)} \qquad \text{SE}(g(\hat\theta)) \approx |g'(\hat\theta)| \cdot \text{SE}(\hat\theta)$$

\vspace{-0.15cm}\pause
\textbf{Example:} $X_i \sim \text{Exp}(\lambda)$, $\hat\lambda = 1/\bar{X}$, want SE of the \textbf{mean lifetime} $\mu = 1/\lambda$.

\vspace{0.05cm}
\quad $g(\lambda) = 1/\lambda$, \;\;$g'(\lambda) = -1/\lambda^2$, \;\;$I(\lambda) = 1/\lambda^2$

\vspace{0.05cm}
\quad $\text{SE}(\hat\mu) = \frac{1}{\hat\lambda^2}\cdot\frac{1}{\sqrt{n\,I(\hat\lambda)}} = \frac{1}{\hat\lambda^2}\cdot\frac{\hat\lambda}{\sqrt{n}} = \frac{1}{\hat\lambda\sqrt{n}} = \frac{\bar{X}}{\sqrt{n}}$

\vspace{0.15cm}
\begin{center}
\fcolorbox{popblue}{popblue!5}{\parbox{11cm}{\centering\small
  \textbf{Delta method + invariance:} MLE gives you the estimate via invariance;\\
  the delta method gives you the SE. Together: point estimate + uncertainty for \textit{any} function of $\theta$.
}}
\end{center}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{4pt}
  ...
  \item \textbf{Boundary of parameter space:} $\text{Uniform}(0, \theta)$ $\Rightarrow$ $\hat\theta = X_{(n)}$.\\
    {\footnotesize Always underestimates: bias $= -\theta/(n{+}1)$. Regularity conditions fail (Lecture~4), CR doesn't apply.}
...
\begin{frame}
\frametitle{Summary: Constructing Estimators}
\begin{center}
\begin{tikzpicture}[
  sbox/.style={draw=popblue!60, fill=popblue!5, rounded corners=4pt, minimum width=12.5cm, minimum height=0.5cm, align=left, inner sep=5pt, font=\small}
]
  \node[sbox] at (0, 2.8) {\textbf{MoM:} Match sample moments to population moments. Simple but can give impossible values.};
  \node[sbox] at (0, 2.0) {\textbf{MLE:} Maximize $L(\theta) = \prod f(X_i \mid \theta)$. The go-to method for estimation.};
  \node[sbox] at (0, 1.2) {\textbf{Invariance:} MLE of $g(\theta)$ is $g(\hat\theta_{\text{MLE}})$. Transform freely.};
  \node[sbox] at (0, 0.4) {\textbf{Sufficiency:} MLE always uses the sufficient statistic --- no information wasted.};
  \node[sbox] at (0, -0.4) {\textbf{Consistency:} $\hat\theta_{\text{MLE}} \xrightarrow{P} \theta_0$. Correct in the long run.};
  \node[sbox] at (0, -1.2) {\textbf{Asymptotic normality:} $\sqrt{n}(\hat\theta - \theta_0) \to N(0, 1/I(\theta_0))$. Enables standard errors.};
  \node[sbox] at (0, -2.0) {\textbf{Efficiency:} MLE achieves the CR bound (exactly for exp.\ families, asymptotically otherwise).};
  \node[sbox] at (0, -2.8) {\textbf{ML connection:} Gaussian $\to$ MSE, Bernoulli $\to$ cross-entropy. Loss functions are MLE!};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{center}
\begin{tikzpicture}[
  rbox/.style={draw=popblue!40, fill=popblue!3, rounded corners=5pt, text width=12.5cm, align=left, inner sep=8pt, font=\small}
]
  \node[rbox] at (0, 2.2) {
    \textbf{\textcolor{popblue}{Interactive: MLE \& Likelihood (R Psychologist)}}\\[2pt]
    \footnotesize\texttt{rpsychologist.com/likelihood} --- drag sliders to see how likelihood, score, and Fisher info change in real time. The best interactive MLE demo.
  };
  \node[rbox] at (0, 0.55) {
    \textbf{\textcolor{popblue}{Interactive: Seeing Theory --- Point Estimation (Brown University)}}\\[2pt]
    \footnotesize\texttt{seeing-theory.brown.edu/frequentist-inference} --- Chapter 5 covers MLE with beautiful animations. Drag sample size to watch the MLE converge.
  };
  \node[rbox] at (0, -1.1) {
    \textbf{\textcolor{popblue}{Video: StatQuest --- MLE, clearly explained}}\\[2pt]
    \footnotesize\texttt{youtube.com/watch?v=XepXtl9YKwc} --- Josh Starmer's 6-minute visual walkthrough of MLE with the Normal distribution. Great for review.
  };
  \node[rbox] at (0, -2.75) {
    \textbf{\textcolor{popblue}{Reading: Penn State STAT 415 --- MoM \& MLE}}\\[2pt]
    \footnotesize\texttt{online.stat.psu.edu/stat415/lesson/1/1.4} --- worked MoM examples with step-by-step solutions for Exponential, Gamma, and more.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[rbox] at (0, 2.2) {
    \textbf{\textcolor{popblue}{Interactive: MLU-Explain (Amazon)}}\\[2pt]
    \footnotesize\texttt{mlu-explain.github.io} --- visual articles on logistic regression, cross-validation, and neural nets. See how MLE powers these ML methods.
  };
  \node[rbox] at (0, 0.55) {
    \textbf{\textcolor{popblue}{Textbook: Wasserman, \textit{All of Statistics}, Ch.~9}}\\[2pt]
    \footnotesize Clear, concise treatment of MLE with worked examples and proofs of consistency and asymptotic normality. Great for self-study alongside Casella \& Berger.
  };
  \node[rbox] at (0, -1.1) {
    \textbf{\textcolor{popblue}{Reading: Wikipedia --- Maximum Likelihood Estimation}}\\[2pt]
    \footnotesize\texttt{en.wikipedia.org/wiki/Maximum\_likelihood\_estimation} --- comprehensive article with derivations for many distributions, history, and connections to KL divergence.
  };
  \node[rbox] at (0, -2.75) {
    \textbf{\textcolor{popblue}{Textbook: Casella \& Berger, Ch.~7 (Point Estimation)}}\\[2pt]
    \footnotesize The standard reference for MoM, MLE, sufficiency, and efficiency. Covers everything in this lecture with rigorous proofs.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{center}
  ...
  {\large Next: Lecture~6 --- MAP estimation, priors, and the Bayesian perspective}
\end{center}
\end{frame}

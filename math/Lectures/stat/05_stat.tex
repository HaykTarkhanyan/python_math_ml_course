...
\begin{tikzpicture}[
  ...
  \node[mombox, fill=paramgreen!10, draw=paramgreen!60] (pop) at (-4, 0) {
    ...
    \small 1st moment: $\mu_1 = \mathbb{E}[X]$\\[2pt]
    2nd moment: $\mu_2 = \mathbb{E}[X^2]$\\[2pt]
...
\begin{center}
  ...
  {\normalsize Four closed-form examples, then the ML connection.}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  \node[draw=orange1, fill=orange1!3, rounded corners=8pt, text width=6.5cm, align=left, inner sep=8pt, font=\small] {
    ...
    \centering$\displaystyle\boxed{\hat{\lambda}_{\text{MLE}} = \bar{X}}$\\[-2pt]
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \node[font=\small\bfseries, sampred, above] at (axis cs:3, 0.5) {$\hat\lambda = 3$};
  \end{axis}
\end{tikzpicture}
\end{center}
...
\begin{column}{0.54\textwidth}
\begin{tikzpicture}
  \node[draw=sampred, fill=sampred!3, rounded corners=8pt, text width=6.5cm, align=left, inner sep=8pt, font=\small] {
    \textbf{Model:} $X_i \sim \text{Exp}(\lambda)$\\[3pt]
    {\scriptsize (time between arrivals, device lifetimes)}\\[6pt]
    $\ell(\lambda) = n\log\lambda - \lambda\sum X_i$\\[6pt]
    $\frac{\partial\ell}{\partial\lambda} = \frac{n}{\lambda} - \sum X_i = 0$\\[6pt]\pause
    \centering$\displaystyle\boxed{\hat{\lambda}_{\text{MLE}} = \frac{1}{\bar{X}}}$\\[-2pt]
    \small Reciprocal of sample mean, since $\mathbb{E}[X] = 1/\lambda$.
  };
\end{tikzpicture}
\end{column}
\begin{column}{0.44\textwidth}
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    % l(lambda) - l(0.5) = 20*(ln(lambda)-ln(0.5)) - (lambda-0.5)*40, n=20, sum=40, xbar=2
    \addplot[popblue, domain=0.1:1.5] {20*(ln(x)-ln(0.5)) - 40*(x-0.5)};

    % MLE marker
    \draw[very thick, sampred, dashed] (axis cs:0.5, -10) -- (axis cs:0.5, 0);
    \fill[sampred] (axis cs:0.5, 0) circle (3pt);
    \node[font=\small\bfseries, sampred, above] at (axis cs:0.5, 0.5) {$\hat\lambda = 0.5$};
  \end{axis}
\end{tikzpicture}
\end{center}
\footnotesize\centering Example: $n=20$, $\bar{X} = 2$ hrs
\end{column}
\end{columns}
\end{frame}

\begin{frame}
\frametitle{MLE: Uniform$(0, \theta)$ --- A Non-Regular Case}

\small
\textbf{Model:} $X_i \sim \text{Uniform}(0, \theta)$, \;$f(x \mid \theta) = \frac{1}{\theta}\,\mathbf{1}[0 \le x \le \theta]$.

\vspace{0.15cm}
\textbf{Likelihood:}
\vspace{-0.1cm}
$$L(\theta) = \prod_{i=1}^n \frac{1}{\theta}\,\mathbf{1}[X_i \le \theta] = \frac{1}{\theta^n}\,\mathbf{1}[\theta \ge X_{(n)}]$$

\vspace{-0.3cm}\pause
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\vspace{0.1cm}
$L(\theta)$ is \textbf{decreasing} in $\theta$ for $\theta \ge X_{(n)}$,\\
and \textbf{zero} for $\theta < X_{(n)}$.

\vspace{0.15cm}
No calculus needed --- the MLE is at the boundary:
$$\boxed{\hat\theta_{\text{MLE}} = X_{(n)} = \max(X_1, \ldots, X_n)}$$

\vspace{0.1cm}
\footnotesize \textcolor{warnred}{Biased:} $\mathbb{E}[X_{(n)}] = \frac{n}{n+1}\theta < \theta$.\\
\textcolor{warnred}{CR bound doesn't apply} (support depends on $\theta$).\\
But: \textbf{consistent}, and converges at rate $1/n$, not $1/\sqrt{n}$!
\end{column}
\begin{column}{0.48\textwidth}
\begin{center}
\begin{tikzpicture}
  \begin{axis}[
    ...
    xlabel={$\theta$},
    ylabel={$L(\theta)$},
    xmin=0, xmax=8, ymin=0, ymax=0.015,
    axis lines=left,
    grid=major, grid style={gray!15},
    xtick={4, 6},
    xticklabels={$X_{(n)}$, },
    ...
    % L(theta) = 1/theta^5 for theta >= 4, 0 otherwise. n=5
    \addplot[popblue, line width=2pt, domain=4:8, samples=60] {1/x^5};
    \addplot[popblue, line width=2pt, domain=0:3.98] {0};

    \fill[sampred] (axis cs:4, 0.000977) circle (3.5pt);
    \node[font=\small\bfseries, sampred, above right] at (axis cs:4, 0.001) {MLE};
  \end{axis}
\end{tikzpicture}
\end{center}
\footnotesize\centering $L(\theta)$ for $n=5$: max at boundary.
\end{column}
\end{columns}
\end{frame}
...
\begin{tabular}{lccc}
  ...
  Uniform$(0, \theta)$ & $\theta$ & $X_{(n)}$ & Range estimation \\
  \hline
\end{tabular}
\end{center}

\vspace{0.3cm}
\small Notice: for exponential families, MLE often equals MoM! We'll see why in the ``Why MLE Works'' section.
\end{frame}
...
\vspace{0.1cm}
\small If the model isn't identifiable, no amount of data will pin down $\theta$.
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[rbox] at (0, 0.55) {
    \textbf{\textcolor{popblue}{Interactive: Distill.pub --- Visual Exploration of MLE}}\\[2pt]
    \footnotesize\texttt{distill.pub} --- research articles with interactive diagrams. Search for ``maximum likelihood'' for excellent visualizations of loss landscapes.
  };
  \node[rbox] at (0, -1.1) {
    \textbf{\textcolor{popblue}{Video: 3Blue1Brown --- Probability Divergence}}\\[2pt]
    \footnotesize\texttt{youtube.com/watch?v=Ect65gRMTfc} --- why MLE = minimizing KL divergence. Connects Lecture~5 to information theory; beautiful animations.
  };
  \node[rbox] at (0, -2.75) {

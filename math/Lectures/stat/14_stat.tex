...
\begin{tikzpicture}[
  ...
  \node[rbox, fill=orange1!10] (c) at (-5, 0) {
    ...
    \begin{tikzpicture}[scale=0.5]
      ...
      \draw[arr, gray, scale=0.8] (x) -- (y);
    \end{tikzpicture}\\
    {\footnotesize $Z$ causes both\\(open backdoor path)}
  };
  % Reverse causation
...
\begin{center}
\begin{tikzpicture}
  \node[draw=warnred, fill=warnred!5, rounded corners=5pt, text width=11cm, align=center, inner sep=4pt, font=\small] {
    \textbf{The fundamental problem:} Each person is either treated or not --- never both.
    We can \textit{never} observe the individual causal effect $Y_i(1) - Y_i(0)$.\\[2pt]
    The missing ``?'' entries are \textbf{counterfactuals}: what \textit{would have happened}.
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!5, rounded corners=5pt, text width=11cm, align=center, inner sep=5pt, font=\small] {
    ...
    \textbf{SUTVA:} each person's outcome depends only on \textit{their own} treatment (no interference).
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[sbox] at (0, 0.0) {
    \textbf{Step 3:} This equals $\underbrace{E[Y(1)\!-\!Y(0) \mid T\!=\!1]}_{\text{ATT}}$ $+$ $\underbrace{E[Y(0) \mid T\!=\!1] - E[Y(0) \mid T\!=\!0]}_{\textcolor{sampred}{\text{selection bias}}}$.
  };
  \node[sbox] at (0, -1.2) {
    \textbf{Step 4:} Randomization ensures $T \perp\!\!\!\perp (Y(1), Y(0))$, so selection bias $= 0$ and ATT $=$ ATE.
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
  ...
  \node[font=\scriptsize, text width=2.5cm, align=center] at (0, -0.7) {$X \not\!\perp\!\!\!\perp Y$\\Control $M$: blocks path\\(estimates \textit{direct} effect only)};
...
\begin{tikzpicture}[
  ...
  % Control (ordered so matches are visually close)
  \node[font=\small\bfseries, sampred] at (5, 2.0) {Control};
  \foreach \i/\a/\s in {1/36/M, 2/41/F, 3/29/M, 4/48/F, 5/56/F, 6/22/F, 7/39/M, 8/34/F} {
    \node[person, fill=sampred!15] (c\i) at (5, 1.5 - \i*0.45) {\a\s};
  }

  % Matches (same sex, close age)
  \draw[thick, paramgreen, -{Stealth}] (t1) -- (c1) node[midway, above, font=\tiny, paramgreen] {match};
  \draw[thick, paramgreen, -{Stealth}] (t2) -- (c2);
  \draw[thick, paramgreen, -{Stealth}] (t3) -- (c3);
  \draw[thick, paramgreen, -{Stealth}] (t4) -- (c5);
  \draw[thick, paramgreen, -{Stealth}] (t5) -- (c7);

  % Unmatched
  \draw[gray, dashed] (c4.east) -- ++(0.3, 0) node[right, font=\tiny, gray] {$\times$};
  \draw[gray, dashed] (c6.east) -- ++(0.3, 0) node[right, font=\tiny, gray] {$\times$};

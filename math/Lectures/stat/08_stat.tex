...
\pause
\vspace{0.15cm}
\textbf{Example: Poisson $\lambda$.} \; $I(\lambda) = 1/\lambda$, MLE $= \hat\lambda = \bar{X}$.
$\;\text{SE} = \sqrt{\hat\lambda/n}$.\\
$n = 50$ earthquakes/year, $\hat\lambda = 3.2$:
...
\begin{tikzpicture}[
  ...
  \node[mbox, fill=paramgreen!8] at (5, 0) {
    ...
    \footnotesize Corrects for bias and skewness.\\
    Best coverage.\\[4pt]
    \textbf{Gold standard.} Use by default.
  };
...
\begin{tikzpicture}[
  ...
  \node[cbox, fill=popblue!8] at (-4.2, 0) {
    \textbf{\large\textcolor{popblue}{Analytical (Wald)}}\\[6pt]
    $\hat\theta \pm z_{\alpha/2} \cdot \text{SE}$\\[6pt]
    \textcolor{paramgreen}{+} Fast, no simulation\\
    \textcolor{paramgreen}{+} Exact SE formulas for common cases\\
    \textcolor{paramgreen}{+} Easy to interpret\\[4pt]
    \textcolor{warnred}{--} Needs a formula for SE\\
    \textcolor{warnred}{--} Assumes approximate normality\\
    \textcolor{warnred}{--} Always symmetric
  };

  \node[cbox, fill=violet1!8] at (4.2, 0) {
    \textbf{\large\textcolor{violet1}{Bootstrap}}\\[6pt]
    Resample $\Rightarrow$ percentiles\\[6pt]
    \textcolor{paramgreen}{+} Works for \textit{any} statistic\\
    \textcolor{paramgreen}{+} No formulas needed\\
    \textcolor{paramgreen}{+} Respects asymmetry (percentile/BCa)\\[4pt]
    \textcolor{warnred}{--} Computationally expensive\\
    \textcolor{warnred}{--} Fails for extremes, small $n$\\
    \textcolor{warnred}{--} Results vary across runs
  };
\end{tikzpicture}
\end{center}
...
\fcolorbox{orange1}{orange1!5}{\parbox{10cm}{\centering\small
  ...
  Use bootstrap when: the statistic is complex (median, correlation, ratio),\\
  no SE formula is available, or you suspect the distribution is skewed.\\[2pt]
  When both are available, they should give similar answers for large $n$.
}}
\end{center}
\end{frame}

...
\pause
\centering\small
Why can we drop $P(\text{data})$?  It doesn't depend on $\theta$ --- it's a \textbf{normalizing constant}.\\[2pt]
For MAP we only need $\arg\max_\theta$, so constants don't change the answer.
\end{frame}
...
\begin{tikzpicture}[scale=0.85]
  % L1 (Lasso) - left
  \begin{scope}[xshift=-4.5cm]
    \node[font=\bfseries, paramgreen] at (0, 3.5) {L1 (Lasso): diamond};
    % Diamond constraint
    ...
    \node[font=\small\bfseries, sampred, right] at (0.15, 1.8) {$\hat{\theta}$};
    % Axes
    \draw[->] (-2.5,0) -- (2.5,0) node[right, font=\small] {$\theta_1$};
    \draw[->] (0,-2.5) -- (0,2.8) node[above, font=\small] {$\theta_2$};
    \node[font=\small, align=center] at (0, -3.2) {Contours hit a \textbf{corner}\\$\Rightarrow$ $\theta_1 = 0$ exactly};
  \end{scope}
  % L2 (Ridge) - right
  \begin{scope}[xshift=4.5cm]
    \node[font=\bfseries, popblue] at (0, 3.5) {L2 (Ridge): circle};
    % Circle constraint
    ...
    \node[font=\small\bfseries, sampred, right] at (1.0, 1.58) {$\hat{\theta}$};
    % Axes
    \draw[->] (-2.5,0) -- (2.5,0) node[right, font=\small] {$\theta_1$};
    \draw[->] (0,-2.5) -- (0,2.8) node[above, font=\small] {$\theta_2$};
    \node[font=\small, align=center] at (0, -3.2) {Contours hit a \textbf{smooth edge}\\$\Rightarrow$ both $\theta_j \neq 0$};
  \end{scope}
\end{tikzpicture}
\end{center}

\vspace{-0.1cm}
\small The diamond has \textbf{corners on the axes} --- the loss contours are much more likely to first touch a corner, setting one or more coefficients to exactly zero. The circle has no corners, so the solution is almost never exactly sparse.
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[pbox, fill=paramgreen!10] at (5, 0) {
    ...
    Predictions average\\over all $\theta$'s\\[4pt]
    Expensive (MCMC)\\[4pt]
    \footnotesize $P(\tilde{x} \mid \text{data}) {=} \int P(\tilde{x} \mid \theta)\, P(\theta \mid \text{data})\, d\theta$
  };

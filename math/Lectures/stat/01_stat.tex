...
\title{Lecture 0: Foundations}
\subtitle{What Statistics Is and Why It's Hard}
\date{}
...
\begin{frame}
\frametitle{The Choice of Loss Reflects Your Values}
\begin{center}
\begin{tikzpicture}[
  exbox/.style={draw, rounded corners=4pt, minimum height=1.4cm, text width=5cm, align=left, inner sep=6pt, font=\small}
]
  \node[exbox, fill=popblue!8] at (-4, 2) {
    \textbf{Medical dosing}\\
    Overdose vs underdose have\\
    very different consequences\\
    $\Rightarrow$ asymmetric loss
  };
  \node[exbox, fill=paramgreen!8] at (4, 2) {
    \textbf{House prices}\\
    Mansions in the data?\\
    MAE $\ne$ MSE answer\\
    $\Rightarrow$ median vs mean
  };
  \node[exbox, fill=sampred!8] at (-4, -0.7) {
    \textbf{Spam filter}\\
    Blocking real email vs\\
    letting spam through\\
    $\Rightarrow$ different error costs
  };
  \node[exbox, fill=popblue!8] at (4, -0.7) {
    \textbf{Flood planning}\\
    Under-predicting flood level\\
    is catastrophic\\
    $\Rightarrow$ conservative (high quantile)
  };

  \node[font=\normalsize\bfseries, text=warnred] at (0, -2.5) {There is no ``correct'' loss --- it depends on context.};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{What Statistics Will Give Us}
\begin{center}
\begin{tikzpicture}[
  roadbox/.style={draw=popblue, fill=popblue!8, rounded corners=6pt, minimum width=4cm, minimum height=1.3cm, align=center, font=\normalsize},
  arrow/.style={-{Stealth}, thick, popblue!60}
]
  \node[roadbox] (est) at (0, 3) {\textbf{Estimation}\\{\small What's our best guess for $\theta$?}};
  \node[font=\footnotesize, right=0.3cm of est] {Lectures 2--3};

  \node[roadbox] (unc) at (0, 1.2) {\textbf{Uncertainty}\\{\small How confident are we?}};
  \node[font=\footnotesize, right=0.3cm of unc] {Lectures 4--5};

  \node[roadbox] (dec) at (0, -0.6) {\textbf{Decisions}\\{\small Is the effect real or noise?}};
  \node[font=\footnotesize, right=0.3cm of dec] {Lectures 6--7};

  \node[roadbox] (mod) at (0, -2.4) {\textbf{Models}\\{\small How do variables relate?}};
  \node[font=\footnotesize, right=0.3cm of mod] {Lecture 8};
  ...
  % Today marker
  \node[draw=sampred, fill=sampred!10, rounded corners=6pt, minimum width=4cm, minimum height=1cm, align=center] (today) at (-6, 3) {\textbf{Today}\\{\small Foundations}};
  \draw[arrow, sampred] (today) -- (est);
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Practical: Loss, Risk, and Robustness}
\begin{center}
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!3, rounded corners=10pt, text width=12cm, align=left, inner sep=15pt] {
    ...
    \begin{enumerate}
      \item Compute the sample \textbf{mean}, \textbf{median}, and \textbf{mode}
      \item Compute empirical risk under each loss for each summary
      \item Verify: mean minimizes squared-error risk, median minimizes absolute-error risk
      \item Add one extreme outlier and repeat
      \item Observe: how much does each summary move?
    \end{enumerate}
    \vspace{0.3cm}
    \textbf{Discuss:} Which summary would you report, and why?\\

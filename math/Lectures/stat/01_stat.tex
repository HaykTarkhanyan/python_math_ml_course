...
\begin{column}{0.38\textwidth}
  \begin{center}
    \includegraphics[width=\columnwidth, height=5.5cm, keepaspectratio]{../../assets/survivorship_airplane.pdf}
  \end{center}
\end{column}
...
\frametitle{Homework}
\begin{enumerate}
  \item A factory claims its light bulbs last 1000 hours on average ($\mu = 1000$).
    You test 50 bulbs and find $\bar{X} = 980$.\\
    Identify: (a) the population, (b) the sample, (c) the parameter,
    (d) the statistic, (e) the estimator, (f) the estimate.

  \vspace{0.15cm}
  \item For data $X = \{1, 2, 3, 4, 100\}$:\\
    (a) Find the value $c$ that minimizes the empirical risk under $L_2$ loss.\\
    (b) Find the value that minimizes $L_1$ loss.\\
    (c) Explain which loss function is more appropriate here and why.

  \vspace{0.15cm}
  \item For data $X = \{2, 3, 5, 7, 8\}$, use the plug-in principle to estimate:\\
    (a) the mean, (b) the variance (using $\hat{F}_n$, dividing by $n$),
    (c) the median, (d) the 75th percentile.
\end{enumerate}
\end{frame}

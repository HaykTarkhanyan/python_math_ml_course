...
\begin{center}
\fcolorbox{orange1}{orange1!5}{\parbox{11.5cm}{\centering\small
  \textbf{Why the Gaussian is everywhere in ML:} if you only know the mean and variance, the Gaussian is the \textbf{least informative} distribution compatible with that knowledge --- any other choice smuggles in extra assumptions.
}}
\end{center}
\end{frame}
...
% ============================================================
\begin{frame}
\frametitle{The CLT, Information-Theoretically}

\small
\textbf{Classical CLT (recall, Module 20).} Let $X_1, X_2, \ldots$ be i.i.d.\ with mean $0$ and variance $\sigma^2$. The normalized sum
\vspace{-0.1cm}
$$Z_n \;:=\; \frac{X_1 + X_2 + \cdots + X_n}{\sqrt{n}} \;\xrightarrow{d}\; N(0,\, \sigma^2).$$
\vspace{-0.15cm}

The \emph{distribution} converges to a Gaussian. \;What does the \emph{entropy} do?

\vspace{0.1cm}
\textbf{Use the differential-entropy scaling rule} $\;h(aX) = h(X) + \log_2 |a|$.

With $S_n = X_1 + \cdots + X_n$ (so $\text{Var}(S_n) = n\sigma^2$) and $a = 1/\sqrt{n}$:
\vspace{-0.1cm}
...
\vspace{0.05cm}
\begin{center}
\fcolorbox{violet1}{violet1!5}{\parbox{11.7cm}{\centering\small
  \textbf{Entropic CLT (Barron, 1986):} $h(Z_n)$ is \emph{monotonically increasing in $n$} and converges to the Gaussian's entropy,
  $$h(Z_n) \;\nearrow\; h\!\big(N(0,\sigma^2)\big) = \tfrac{1}{2}\log_2(2\pi e\, \sigma^2).$$
}}
\end{center}
...
\fcolorbox{paramgreen}{paramgreen!5}{\parbox{11.7cm}{\centering\footnotesize
  Among distributions with variance $\sigma^2$, the Gaussian has \textbf{maximum} differential entropy (we proved this). Summation \textbf{climbs that hill}: convolution is entropy-increasing, and the Gaussian is its \textbf{fixed point} --- the entropy attractor.
}}
\end{center}
\end{frame}

...
\textbf{(A) The 20 Questions / Wordle game.}
\begin{center}
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=11.5cm, align=left, inner sep=6pt, font=\small] {
    20 Questions: with $N$ possible answers, you need $\lceil\log_2 N\rceil$ yes/no questions
    if you play optimally. Why \emph{exactly} that number?\\[3pt]
    Wordle: $\sim$2300 valid answers, good players average \textbf{4--5 guesses}.
    Each guess reveals $\sim$2--3 bits. Why \emph{those} numbers?
  };
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}
  \node[draw=sampred, fill=sampred!5, rounded corners=6pt, text width=11.5cm, align=center, inner sep=6pt] {
    $\displaystyle\mathcal{L}(\theta) = -\sum_{i=1}^n \sum_{k} y_{ik}\,\log \hat{p}_k(x_i; \theta)$\\[3pt]
    \small Logistic regression, softmax, neural nets, LLMs --- all minimize this.\\
    By the end of \textbf{info\_02}, you'll know why it's this formula and not another.
  };
\end{tikzpicture}
\end{center}
...
\vspace{0.1cm}
\begin{center}
\small Both problems are solved by one quantity: \textbf{entropy}.
\end{center}
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[font=\small, violet1] at (0, -2.9) {\textbf{Next lecture (info\_02):} loss functions, MLE, mutual information, decision trees.};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, text width=12cm, align=center, inner sep=7pt] {
    ...
    $H(X) \;:=\; \mathbb{E}\!\left[-\log_2 p(X)\right] \;=\; -\displaystyle\sum_{x} p(x)\,\log_2 p(x)$\\[3pt]
    \footnotesize Convention: $0 \cdot \log_2 0 = 0$ \;($\lim_{t\to 0^+} t\log t = 0$).
  };
\end{tikzpicture}
\end{center}

\vspace{0.05cm}
\textbf{Worked example:} $X \in \{a, b, c, d\}$, $p = (\tfrac{1}{2}, \tfrac{1}{4}, \tfrac{1}{8}, \tfrac{1}{8})$. \;\;\textit{\small Predict first: how many bits?}
...
\vspace{-0.1cm}
\textbf{Other distributions on $\{a, b, c, d\}$:}
...
\vspace{-0.05cm}
\begin{center}
\footnotesize Higher entropy $=$ less predictable. Uniform maximizes; concentrated distributions minimize.
\end{center}
\end{frame}
...
\vspace{0.05cm}
The function $h(t) = -t\log_2 t$ is \textbf{concave} on $[0,1]$:
$\;h''(t) = -1/(t \ln 2) < 0$. So Jensen's inequality applies with uniform weights $1/g$:
...
\small
A shaurma stand's cashier sends order codes to the kitchen. Each order is a symbol from $\mathcal{X} = \{\text{cheese, shaurma, potato, chocho}\}$, encoded as a \textbf{binary string}. The probabilities reflect how often each item is ordered.
...
\vspace{0.05cm}
\begin{center}
\small Half the orders going through the cashier are cheese --- shouldn't it get a \textbf{shorter} code than rare items like chocho?
\end{center}
\end{frame}
...
\vspace{0.05cm}
Suppose: \;cheese$\to$\texttt{0}, \;shaurma$\to$\texttt{1}, \;potato$\to$\texttt{01}, \;chocho$\to$\texttt{11}.
...
\begin{tikzpicture}[scale=0.85,
  ...
  \node[lf=sampred]    (chocho)  at (6,-5.4)    {chocho \;\texttt{111}};
  % Edges
  ...
  \draw[thick] (n2) -- node[above right, font=\footnotesize\bfseries] {1} (chocho);
  % Probabilities
...
\begin{tabular}{lccccc}
  ...
  chocho  & $1/8$ & $1/4$ & 3 & 2 & \textcolor{paramgreen}{$-1$} \\
  \hline
\end{tabular}
\end{center}
...
\fcolorbox{warnred}{warnred!5}{\parbox{11.5cm}{\centering\footnotesize
  \textbf{Wait, what?} Potato and chocho use \emph{fewer} bits under the wrong code.
...
\fcolorbox{popblue}{popblue!5}{\parbox{11.5cm}{\centering\footnotesize
  \textbf{Note the units gap:} discrete $H(X) \geq 0$ measures bits;
  continuous $h(X)$ measures bits \emph{relative} to a unit-width interval --- it can be \textbf{negative}.
}}
\end{center}
\end{frame}
...
\begin{center}
\fcolorbox{warnred}{warnred!5}{\parbox{11.5cm}{\centering\footnotesize
  \textbf{The scaling property is the source of all weirdness:} if you measure $X$ in meters vs.\ centimeters, $h$ changes by $\log_2 100 \approx 6.6$ bits, even though the random variable is ``the same.''
}}
\end{center}

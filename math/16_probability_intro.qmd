...
*See also.* [**Problem 39**](#weighted-dart-scoring-sequel-to-problem-10) extends this setup by assigning a point value to each ring and asking for the expected score per throw.
...
### 39 Weighted Dart Scoring (sequel to Problem 10) {data-difficulty="2"}

Same dart target as Problem 10: five concentric circles with radii $1, 2, 3, 4, 5$ m, dart lands uniformly at random somewhere on the board. But this time each ring is worth points:

| Zone | Radii (m) | Points |
|:----:|:---------:|:------:|
| central disk | $[0, 1]$ | **25** (bullseye) |
| 2nd donut | $[1, 2]$ | $20$ |
| 3rd donut | $[2, 3]$ | $15$ |
| 4th donut | $[3, 4]$ | $10$ |
| 5th donut | $[4, 5]$ | **5** (outer) |

Let $X$ denote the score from one throw.

a. Write the PMF of $X$.
b. Compute $E[X]$.
c. Compute $\operatorname{Var}(X)$.
d. Vardan offers to play this game with you: pay him $12$ dram per throw, you collect $X$ dram per throw. Should you accept?

::: {.callout-tip collapse="true" title="Solution"}

**Setup.** From Problem 10, the donut areas (in units of $\pi$) are $1, 3, 5, 7, 9$, summing to $25$. Probabilities are area ratios, so $P(X = s_i) = \text{area}_i / 25$.
...
Sanity: probabilities sum to $1 + 3 + 5 + 7 + 9 = 25$ over $25$. ✓

**b) Expectation.** $E[X] = \sum_i s_i \cdot P(X = s_i)$:

$$\begin{aligned}
E[X] &= \tfrac{1}{25}\big(25 \cdot 1 + 20 \cdot 3 + 15 \cdot 5 + 10 \cdot 7 + 5 \cdot 9\big) \\
&= \tfrac{1}{25}(25 + 60 + 75 + 70 + 45) \\
&= \tfrac{275}{25} = 11
\end{aligned}$$

**c) Variance.** First $E[X^2]$:

$$\begin{aligned}
E[X^2] &= \tfrac{1}{25}\big(625 \cdot 1 + 400 \cdot 3 + 225 \cdot 5 + 100 \cdot 7 + 25 \cdot 9\big) \\
&= \tfrac{1}{25}(625 + 1200 + 1125 + 700 + 225) \\
&= \tfrac{3875}{25} = 155
\end{aligned}$$

Then $\operatorname{Var}(X) = E[X^2] - E[X]^2 = 155 - 11^2 = 155 - 121 = 34$.

Standard deviation: $\sqrt{34} \approx 5.83$ points per throw, a sizeable spread relative to $E[X] = 11$.

**d) Pay 12 dram per throw, gain $X$ per throw?** Net per throw is $X - 12$. By linearity of expectation:
$$E[X - 12] = E[X] - 12 = 11 - 12 = -1 \text{ dram per throw}$$

You lose $1$ dram on average per throw. **Don't accept** (unless you're paying for entertainment). To break even Vardan should charge exactly $11$ dram; anything higher is a profit for him.

**Why $E[X]$ is so much lower than the bullseye's $25$.** The naive guess "average over the five scores" gives $(25 + 20 + 15 + 10 + 5)/5 = 15$, but the actual expectation is only $11$. The reason is that the rings are *not* equally likely: the outer donut is **9 times** more probable than the bullseye, because area scales with $r^2$ and the outer donut covers a much larger fraction of the board. The expectation gets pulled toward the high-area, low-score outer rings.

Quantitatively, the outer donut alone contributes $5 \cdot \tfrac{9}{25} = \tfrac{45}{25} = 1.8$ points to $E[X]$, while the bullseye contributes only $25 \cdot \tfrac{1}{25} = 1$. The outer donut adds *more* to the average than the bullseye, despite being worth $5\times$ less per hit — pure area effect.

*Connecting back to Problem 10.* That problem gave you the PMF of which color the dart hits. This problem treats which *ring* as a random variable (with five values) and computes its expectation. Same setup, one level of abstraction up: from "what's the probability of event $A$?" to "what's the expected value of a function of the outcome?"

*Same idea as the Die Game (Problem 27).* That was a discrete RV with $6$ outcomes (die faces) and per-face payoffs. This is a discrete RV with $5$ outcomes (rings) and per-ring payoffs. The PMF here just happens to come from a geometric setup (area ratios) instead of equal-likely die faces.

*A note on player skill.* This solution assumes the dart lands *uniformly* on the board, which models a complete beginner who throws randomly. A skilled player concentrates throws near the center, so the distribution becomes non-uniform (e.g., a 2D Gaussian centered on the bullseye). Then $P(\text{bullseye})$ rises sharply and $E[X]$ shoots up — that's why real dart-game scoring rewards center hits so heavily: it makes the game payoff-positive for good players and payoff-negative for bad ones.

:::

## Random Variables (Chapter 9 of the Armenian notes)

### 40 Max of Two Dice: PMF and CDF (textbook 9.1) {data-difficulty="2"}
...
### 41 Broken Stick: 10 cm Triangle (textbook 9.3) {data-difficulty="2"}
...
### 42 Walking, Sleeping, or Studying: Piecewise PDF (textbook 9.6) {data-difficulty="2"}

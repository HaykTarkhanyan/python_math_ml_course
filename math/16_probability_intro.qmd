...
- [🛠️🗂️ Գործնականի PDF-ը](Homeworks/hw_18_prob_1.pdf)
...
Pick three numbers independently and uniformly from the interval [1, 10]. What is the probability that they can be the lengths of the sides of a triangle?

[📹 Solution](https://youtu.be/ccNSU_TsUnQ)

::: {.callout-tip collapse="true" title="Solution"}

Let $a, b, c$ be i.i.d. Uniform$[1, 10]$. We need all three triangle inequalities:

$$a + b > c, \quad a + c > b, \quad b + c > a$$

**Reduction to a single inequality.** Sort the three values so that $a \leq b \leq c$. The first two inequalities are then automatic ($c$ is largest, so $a + c > c \geq b$ and $b + c > c \geq a$). Only the third is in question:

$$a + b > c$$

So we want $P(a + b > c)$ where $a, b, c$ are the **sorted** values of three uniform draws on $[1, 10]$.

**Setting up the integral.** Sample the unsorted triple uniformly on the cube $[1, 10]^3$ (volume $9^3 = 729$). The triangle condition $\max(a,b,c) < \text{sum of other two}$ depends only on the unordered values, so we can compute:

$$P(\text{triangle}) = \frac{\text{volume}\{(a,b,c) \in [1,10]^3 : \max < a+b+c - \max\}}{729}$$

By symmetry, condition on $c$ being the maximum (this has probability $\tfrac{1}{3}$, and we'll multiply by $3$ at the end). The "$c$ is max and triangle" event becomes:

$$1 \leq a, b \leq c, \quad a + b > c, \quad c \in [1, 10]$$

**Inner area for fixed $c$.** For fixed $c$, the $(a, b)$ region is the square $[1, c]^2$ intersected with the half-plane $a + b > c$. The square has area $(c-1)^2$. The "fail" region $a + b \leq c$ is the triangle with vertices $(1, 1), (1, c-1), (c-1, 1)$ (assuming $c \geq 2$), which has legs of length $c - 2$ and area $\tfrac{(c-2)^2}{2}$. (For $c < 2$, the square is so small that $a + b > c$ is automatic since $a, b \geq 1$.)

So for $c \in [2, 10]$:

$$\text{good area}(c) = (c - 1)^2 - \frac{(c-2)^2}{2}$$

For $c \in [1, 2]$: $\text{good area}(c) = (c-1)^2$.

**Integrate over $c$:**

$$V_{\max=c} = \int_1^2 (c-1)^2\, dc + \int_2^{10}\left[(c-1)^2 - \tfrac{(c-2)^2}{2}\right] dc$$

First piece: $\int_1^2 (c-1)^2 dc = \tfrac{1}{3}$.

Second piece: $\int_2^{10}(c-1)^2 dc = \big[\tfrac{(c-1)^3}{3}\big]_2^{10} = \tfrac{729 - 1}{3} = \tfrac{728}{3}$.

And $\int_2^{10} \tfrac{(c-2)^2}{2} dc = \tfrac{1}{2}\big[\tfrac{(c-2)^3}{3}\big]_2^{10} = \tfrac{512}{6} = \tfrac{256}{3}$.

Total volume with $c$ as maximum:

$$V_{\max=c} = \tfrac{1}{3} + \tfrac{728}{3} - \tfrac{256}{3} = \tfrac{473}{3}$$

By symmetry, multiply by $3$ for the cases where $a$ or $b$ is the max:

$$V_{\text{triangle}} = 3 \cdot \tfrac{473}{3} = 473$$

$$P(\text{triangle}) = \frac{473}{729} \approx 0.649$$

*Sanity-check.* When the lower bound is $0$ instead of $1$ (the classic $[0,1]^3$ version), the answer is exactly $\tfrac{1}{2}$. Here the lower bound of $1$ helps — it forces all sides to be at least $1$, ruling out near-zero "needles" that fail the inequality, so the probability is higher than $\tfrac{1}{2}$. That matches our $\approx 0.649$.

*Connection to ML.* Geometric probability problems like this teach you to set up multivariate integrals over constraint regions — the same setup that drives the **volume-of-feasible-region** intuition behind Bayesian posteriors, MCMC sampling, and rejection sampling. Whenever you write "$P(X \in A) = \int_A f(x)\,dx$", you're doing exactly this kind of geometric reasoning.
...
$$P(D) = P(D \mid W)P(W) + P(D \mid \neg W)P(\neg W) = 0.15 \cdot 0.9 + 0.8 \cdot 0.1 = 0.135 + 0.08 = 0.215$$
...
**c) After Heads then Tails.** A Tails *immediately* rules out the two-headed coin (it can't ever show Tails). So:

$$P(\neg T \mid H, T_{\text{ails}}) = 1$$

The probability it's one of the nine ordinary coins is $\boxed{1}$.

*The lesson.* When some hypothesis $H$ assigns probability **zero** to an observation, observing that observation rules $H$ out completely. Bayes' rule does this automatically: $P(H \mid \text{obs}) \propto P(\text{obs} \mid H) \cdot P(H) = 0 \cdot P(H) = 0$.
...
### 26 Modified Die: Probability and Moments {data-difficulty="2"}
...
### 27 Die Game: Expected Value {data-difficulty="2"}
...
### 28 PDF 2x on [0,1]: Moments {data-difficulty="2"}
...
### 33 Find Normalizing Constant {data-difficulty="3"}

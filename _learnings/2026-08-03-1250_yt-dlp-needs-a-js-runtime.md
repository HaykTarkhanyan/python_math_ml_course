...
## Update, 2026-08-08: the fix was applied five days late, after it bit again

**This note diagnosed the cause and gave the exact one-flag fix on 2026-08-03, and the script was
never changed.** On 2026-08-08 the same 403 hit the first video of a new chapter. The
"Consequences" section above had even predicted it verbatim: *"`yt_fetch.sh` is currently broken
for any new video"*.

`yt_fetch.sh` now carries the fix, plus two things this note argued for:

- `--js-runtimes node` on the download line, with a comment pointing back here.
- **An assertion that `video.mp4` exists and is non-empty, exiting 1 if not.** This is the more
  important half. The reason the bug survived five days is the *partial success* described
  above - metadata, description and subtitles all land normally, so the folder looks healthy and
  the script exits 0 with only the media missing. It can no longer fail quietly.
- An optional `HEIGHT` argument (default 720, unchanged) so a deck wanting full-bleed stills can
  ask for 1080 without editing the script.

**The general lesson, which is worth more than the yt-dlp detail:** a learning that identifies a
fix but does not *apply* it is a diary entry, not a repair. When a note's Consequences section
names a specific broken file, either fix the file in the same session or the note has not
finished doing its job. Verified working: 1080p AV1, 179 MB and 176 MB, `ffprobe` confirms
`av1,1920,1080`.

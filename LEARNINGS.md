...
- Beamer **silently clips** overflowing content - no overfull-vbox warnings. A clean log and correct page count prove nothing about layout. **2026-08-08:** visual inspection is no longer the only tool - `non_essential/detect_clipped_slides.py` diffs the prose in each frame against the prose in the rendered PDF and reports what never arrived. It caught L34's recap frame losing the tail of its last list item, on the final slide of the chapter, after a self-review had already passed the deck. It flags false positives (figure/table/column boundaries, accented characters), so every flag needs a look - but it turns "eyeball all 36 pages" into "check these 9". Still blind to clipped *figures* and overlapping boxes, so `beamer-overflow-check` and eyeballing remain necessary.

...
`ml/07_classic_methods/28_svm_and_classic_methods.tex`. Hoisting to `ml/preamble.tex` with
...
- [ ] **Deferred cleanup (own task, all 3 chapters):** hoist to preamble, remove all 5 locals,
  recompile [17]-[20] + `09_regression_metrics` + `28_svm_and_classic_methods`, verify each.
...
**STATUS 2026-07-10:** Data-driven figures DONE and committed for all four decks (13 of the 14
rows; only the item-13 TikZ schematics remain). Delivered, all overflow-checked, 0 errors:
- [17]: tree_staircase, tree_extrapolation, tree_greedy_xor, readable depth-2 titanic_tree.
- [18]: rf_boundary (blocky vs smooth), rf_max_features (rho decorrelation curve - honest: shows
  the mechanism rho, not a staged error drop), rf_instability legend fix.
- [19]: adaboost_rounds (SAMME weights + stump per round), boost_gb_classification (log-odds ->
  sigmoid), boost_anim_50 (round-50 overlay).
- [20]: xgb_monotonic (wiggly vs clean), 12_xgb_importance repointed to Titanic, xgb_stacking
  receipt (honest: stack ties best base, does not beat it).
- **catboost is NOT installed** in the `ma` venv; not needed (CatBoost figure is item-13 TikZ).
- **Remaining:** item 13 - 4 TikZ schematics in [20] (sparsity arrow, EFB merge, CatBoost
  ordered-TS timeline, DART greyed trees).

...
\begin{frame}{The BatchNorm gotcha}
  ...
  \vskip 6pt
  \begin{itemize}\small
    \item In \texttt{train()} mode BatchNorm normalizes each batch with \textbf{that
...
# then: the usual training loop, unchanged - only the head learns
\end{lstlisting}
  \vskip 2pt
  \fcolorbox{paramgreen}{paramgreen!8}{\parbox{0.97\linewidth}{\small
...
\begin{frame}{2026: whose shoulders you stand on}
  Same two recipes - but the pretrained trunk is increasingly \textbf{not} an ImageNet
  classifier:
  \vskip 4pt
  \begin{itemize}\small
    \item \textbf{Self-supervised backbones} learn to see from photos with \textbf{no
      labels at all}: shown two random crops of the same photo, the net must give them
      matching features - so it learns what stays the same when the view changes.
      Meta's \textbf{DINOv3} (2025): a 7-billion-parameter vision transformer trained
      this way on 1.7 billion unlabeled images, built to be used \textbf{frozen} - and
      distilled into laptop-sized versions, including ConvNeXt CNNs (L17).
    \item \textbf{Image-text backbones} (CLIP, 2021 - L17's epilogue) learned from
      captions, so their features already line up with words.
    \item Papers call Recipe 1 a \textbf{linear probe}: freeze the trunk, fit one linear
      layer - logistic regression, [11] - on its features. It is how these backbones are
      graded.
  \end{itemize}
  \vskip 6pt
  \fcolorbox{paramgreen}{paramgreen!8}{\parbox{0.97\linewidth}{\small
...
\begin{frame}{Grad-CAM: where is the net looking?}
  ...
  \end{columns}
  \vskip 2pt
  {\scriptsize\color{gray} Selvaraju et al., "Grad-CAM", ICCV 2017.}
\end{frame}
...
\begin{frame}{The verdict on our photo}
  \begin{columns}[T]
    ...
    case. The audit that matters is on your fine-tuned grader (HW3).}
    \vskip 4pt
    \fcolorbox{armblue}{armblue!8}{\parbox{0.95\linewidth}{\small

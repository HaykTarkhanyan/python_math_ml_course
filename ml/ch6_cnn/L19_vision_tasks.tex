...
\begin{frame}{The task zoo}
  \begin{center}
    \includegraphics[height=0.66\textheight]{task_zoo.pdf}
  \end{center}
  \vskip 2pt
...
\begin{frame}{NMS: from a flood of boxes to a clean answer}
  ...
  \vskip 1pt
  \begin{center}
    \only<1>{\includegraphics[width=0.38\linewidth]{nms_anim_0.pdf}}%
    \only<2>{\includegraphics[width=0.38\linewidth]{nms_anim_1.pdf}}%
    \only<3>{\includegraphics[width=0.38\linewidth]{nms_anim_2.pdf}}%
    \only<4>{\includegraphics[width=0.38\linewidth]{nms_anim_3.pdf}}%
    \only<5>{\includegraphics[width=0.38\linewidth]{nms_anim_4.pdf}}%
  \end{center}
  \vskip 1pt
  \fcolorbox{armblue}{armblue!8}{\parbox{0.97\linewidth}{\small
  ...
  Ultralytics' YOLO26 (Jan 2026). The
  duplicates did not go away; the model learned to suppress them itself.}
\end{frame}
...
% reversed ranking (FP, FP, TP, TP) gives AP = .5.
\begin{frame}{mAP by hand}
  ...
  \fcolorbox{armblue}{armblue!8}{\parbox{0.97\linewidth}{\small
    ...
    give AP $= 0.5$. mAP rewards finding every object \emph{and} being most confident about
...
\begin{frame}{Decoding one YOLO cell, by hand}
  \small
  YOLO resizes every photo to $448\times448$; a $7\times7$ grid makes each cell $64$ px
  wide. Take the cell in row 3, column 2 (counting from 0). One of its two boxes outputs:
  \vskip 3pt
  \begin{center}
    $x = 0.5,\; y = 0.25$ {\footnotesize (centre, relative to the \emph{cell})}\quad
    $w = 0.2,\; h = 0.3$ {\footnotesize (relative to the \emph{image})}\quad
    confidence $0.8$
  \end{center}
  and the cell's class head says $P(\text{pomegranate} \mid \text{object}) = 0.9$.
  \vskip 4pt
  \begin{itemize}
    \item centre: $\big((2 + 0.5)\cdot 64,\ (3 + 0.25)\cdot 64\big) = (160, 208)$ px;
    \item size: $0.2 \cdot 448 \approx 90$ px wide, $0.3 \cdot 448 \approx 134$ px tall;
    \item score for NMS: $0.8 \times 0.9 = \mathbf{0.72}$ - "is there an object here"
      times "which class is it".
  \end{itemize}
  \vskip 4pt
  \fcolorbox{armblue}{armblue!8}{\parbox{0.97\linewidth}{\small
    Why 2 boxes per cell? In training, whichever of the two already overlaps the true box
    best is made "responsible" for it, so the two specialize in different shapes. Later
    YOLOs predict offsets from a few preset box shapes ("anchors") instead.}}
\end{frame}
...
\begin{frame}{Separable convolutions: the idea}
  ...
  \vskip 4pt
  {\small \textbf{Two} things share the name "separable": \emph{spatially} separable factors
  a 2D kernel into two 1D passes (Sobel $=$ column $\times$ row, L16); the speed win here is
  \emph{depthwise} separable - factor across \textbf{channels}:}
  \vskip 4pt
  \begin{enumerate}
...
\begin{frame}{Transposed convolution: the output must be an image}
  \begin{columns}[T]
    ...
    \fcolorbox{armorange}{armorange!12}{\parbox{0.95\linewidth}{\small
      ...
      with a checker pattern (L16's 1D toy: the middle cell got two stamps). Fixes:
      kernel size divisible by the stride, or upsample-then-convolve (Odena et al.,
      2016).}}
  \end{columns}
\end{frame}
...
\begin{frame}{U-Net in action: roads from the sky}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item loss: [11]'s log-loss, applied \emph{once per pixel} and summed.
    \end{itemize}
    \vskip 6pt
    \fcolorbox{paramgreen}{paramgreen!8}{\parbox{0.95\linewidth}{\small
      ...
      $=$ missed, red $=$ false alarms - [12]'s confusion matrix, \emph{painted onto the
      map}.}}
    \column{0.56\textwidth}
    \begin{center}
...
\begin{frame}{Same idea, other shapes: 1D and 3D}
  \begin{columns}[T]
    ...
    Nothing about convolution says "image". The window slides over whatever grid the
    data lives on:
    \vskip 4pt
    \begin{itemize}\small
      \item \textbf{1D} - sequences: accelerometer traces for activity recognition,
        audio, character-level text (right). L16 \emph{opened} with a 1D kernel sliding
        over a sequence - that was the real thing, not a warm-up.
    ...
    \fcolorbox{armblue}{armblue!8}{\parbox{0.96\linewidth}{\small
        ...
        works.}}
    \column{0.46\textwidth}
    \begin{center}\includegraphics[width=0.92\linewidth]{text_1d_conv.png}\end{center}
    \vskip 2pt
    {\scriptsize A movie review as a \textbf{1D signal}: each row is a character of
    ``dummy review'', one-hot across the alphabet; a 1D kernel (blue) slides down it -
    a \textbf{character-level text-CNN}.}
  \end{columns}
\end{frame}
...
\begin{frame}{The inductive-bias ledger}
  ...
  \vskip 6pt
  \begin{itemize}
    \item CNNs \textbf{assume} locality and translation equivariance (L16's two wishes:

...
\begin{frame}{Grouped convolution}
  \begin{columns}[T]
    ...
    \fcolorbox{paramgreen}{paramgreen!8}{\parbox{0.95\linewidth}{\small
      $g{=}2$ (AlexNet) to $g{=}C$ (depthwise): one operation, two extremes.
      \emph{Deformable} conv (Dai et al., 2017) goes further - it learns \emph{where} to
      sample.}}
  \end{columns}
\end{frame}
...
\begin{frame}{The pioneers today}
  ...
  TechCrunch, Mar 2026 (\$1.03B seed round).}
\end{frame}
...
\begin{frame}{One paper, three fates}
  ...
  \begin{itemize}
    ...
    \item \textbf{Ilya Sutskever} - co-founded \textbf{OpenAI}, chief scientist
      ...
      (SSI) - through August 2026 no product, paper or model at all, by choice (a first
      model is rumoured).
  ...
  \begin{columns}[T]
    ...
    \begin{itemize}\small
      \item A shared \textbf{benchmark} (ImageNet) turned architecture design into
        measurable progress.
      \item \textbf{LeNet} wrote the blueprint; \textbf{AlexNet} = [43]-[45]'s tricks
        at GPU scale; \textbf{VGG} = small filters, stacked deep.

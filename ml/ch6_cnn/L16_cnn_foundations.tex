...
\begin{frame}{Three numbers, but which three?}
  \small
  Nothing in RGB means \emph{how bright} or \emph{which color} - both are spread across all
  three numbers. \textbf{HSV} keeps the same pixel and changes the axes: \textbf{H}ue (angle
  - which color), \textbf{S}aturation (radius - how pure), \textbf{V}alue (height - how
  bright).
  \begin{center}\includegraphics[width=0.78\linewidth]{hsv_space.pdf}\end{center}
  \vskip -4pt
  \fcolorbox{armorange}{armorange!12}{\parbox{0.965\linewidth}{\small
    In RGB, "darker" is ambiguous - subtract and the hue drifts, multiply and it survives.
    In HSV, brightness \emph{is} an axis: moving $V$ cannot do anything else.}}
\end{frame}
...
\begin{frame}{Aside: from Taylor to Fourier}
  \begin{center}\includegraphics[width=0.97\linewidth]{taylor_vs_fourier.pdf}\end{center}
  \vskip 4pt
  \begin{columns}[T]
    \column{0.60\textwidth}
...
\begin{frame}{Dilation and erosion: kernels that are not sums}
  ...
  \vskip 2pt
  \begin{center}\includegraphics[width=0.97\linewidth]{morphology_demo.pdf}\end{center}
  \vskip 2pt
  \fcolorbox{armorange}{armorange!12}{\parbox{0.97\linewidth}{\small
...
\begin{frame}{Dilated convolution: a kernel with gaps}
  \begin{center}\includegraphics[width=0.97\linewidth]{dilated_conv_demo.pdf}\end{center}
  \vskip 2pt
  ...
  \vskip 3pt
  \fcolorbox{armblue}{armblue!8}{\parbox{0.97\linewidth}{\small
    L17 grew the receptive field by \emph{stacking} layers; dilation grows it \emph{per
    layer}. It returns in \textbf{L19} for segmentation, where you need wide context but
    must keep every pixel.}}
\end{frame}

\begin{frame}{Transposed convolution: run it backwards to upsample}
  \begin{center}\includegraphics[width=0.80\linewidth]{transposed_conv_demo.pdf}\end{center}
  \vskip 2pt
  A normal conv \textbf{shrinks} the map; a \textbf{transposed} conv \textbf{grows} it -
  each input cell \emph{stamps} the kernel onto a larger output, and overlaps add.
  \vskip 3pt
  \fcolorbox{armblue}{armblue!8}{\parbox{0.97\linewidth}{\small

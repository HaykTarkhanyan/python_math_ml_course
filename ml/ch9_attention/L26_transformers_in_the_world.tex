...
\begin{frame}{Read that graph carefully}
  ...
  \vskip 10pt
  With $d_{\text{model}} = 4096$, attention only overtakes the feed-forward at
  \textbf{16,384 tokens}. Below that, most of the compute is in the per-token MLP, not in
  attention at all.

  \vskip 12pt
  \watchbox{So for a 2,048-token context, optimising attention is optimising the smaller half.
  The quadratic term is real and it is what caps long context -- but ``quadratic'' describes how
  the cost \emph{grows}, not where the cost \emph{is} at every size. The crossover is worth
  knowing before you optimise the wrong thing.}

  \vskip 6pt
  {\small FlashAttention, sliding-window and sparse variants all attack the $n^2$ term; they

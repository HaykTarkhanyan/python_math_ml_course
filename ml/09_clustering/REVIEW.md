# L13 Clustering (ml/09_clustering) - content & teaching review

> **STATUS 2026-08-20.** A second pedagogical review was run (fresh read of the `.tex` plus a
> visual pass on the rendered pages). It raised 11 items; the instructor selected **6**, now
> applied - see `DECISIONS.md` #23. Added: a **running example** (toy loyalty-card customers,
> `customers()` in the generator) used twice, a **scaling-trap predict-first**
> (`clu_scaling_trap`, ARI 0.42 raw vs 0.84 standardized), a **responsibility** frame for the
> E-step (`clu_responsibility`), an **elbow-vs-silhouette** frame (`clu_pick_k`), a
> **single-linkage chaining** frame (`clu_chaining`), a **centroid-profiling** frame
> (`clu_segments`), and **originating citations** on all ten named methods. The scale-first box on
> the distance frame was reworded to pose the question rather than answer it, so the new
> predict-first is not spoiled.
>
> Also fixed while verifying: the **Lloyd animation had been silently clipping its own footnote**
> on all 6 overlay pages since the deck was written (figure too tall at `0.6\textwidth`, now
> `0.49`; bisected against the original - width alone caused it). Beamer emits no overfull-vbox
> warning, but `non_essential/detect_clipped_slides.py` names the missing text in seconds; it had
> just never been run here, which is why this survived the 2026-07-07 and 2026-08-09 reviews -
> even though `WORKFLOWS.md` already lists it in the definition of done for a deck. Both detectors
> have now been run on this deck and every flag checked against the rendered page. See
> `_learnings/2026-08-20-1745_the-clipped-slide-detector-works-nobody-ran-it.md`.
>
> **Later the same day, from an instructor question** ("why is that top-left point noise when the
> one beside it is a border point?"): the deck stated the three point types but never showed the
> rule that separates them. New frame + `clu_border_vs_noise.pdf` zooms on the closest border/noise
> pair - `p` (3 in its eps-ball, but a **core** point `c` at 0.55 inside) joins; `q` (2 in its ball,
> and the only thing inside is `p`, a *border* point) is noise. **Reachability propagates only
> through core points**; `q` misses `c` by 0.04. Walkthrough steps 5-6 now annotate that same pair
> instead of two unrelated points, which is what made the contrast look arbitrary.
>
> **Instructor pass 2, same day.** (a) **Acronyms** brought in line with `ml/SLIDE_STYLE.md` -
> DBSCAN and HDBSCAN had never been expanded at all, and the **H** (a hierarchy over *every*
> density, rather than one `eps`) is the point of the algorithm; GMM, BIC/AIC and NMI expanded too.
> (b) **Three more section transitions**: the catch-all `Other algorithms` became **Medoids**,
> **Density-based clustering** and **Hierarchical clustering**, and `Cluster evaluation` split into
> **without labels** / **against labels**. The Outline frame now names 8 real topics instead of 5
> vague ones. (c) The silhouette frame now answers **what `s = -1` means** (only `b = 0` produces
> it - the point sits exactly on the nearest cluster; verified numerically, a genuinely misassigned
> point scores about `-0.4`). (d) The **Rand/ARI worked example split into two frames** - "compare
> pairs, not labels" then "Rand looks fine, ARI does not"; numbers re-verified against sklearn
> (Rand `0.5636`, ARI `0.1258`).
>
> **REMOVED at instructor request:** the pure-noise predict-first frame (which the 2026-08-09 pass
> had added as its item #4). Its punchline - k-means returns four tidy clusters and a silhouette of
> 0.41 from uniform random points - survives as a self-contained bullet on the "Easy to get wrong"
> frame. `fig_kmeans_noise()` and `clu_kmeans_noise.pdf` are deliberately retained so the frame can
> be restored in one paste.
>
> Deck is now **56 frames / 75 pages**; both detectors run and every flag checked.
>
> **Raised but NOT applied** (instructor's call, still open): the DBSCAN predict-first prints its
> answer above the question; the elbow figure is unrealistically clean against prose calling elbows
> "often fuzzy"; the hook's "how many groups?" question never pays off; the two closing frames
> (evaluation-in-practice / easy-to-get-wrong) overlap. Item #9 below (computed curse-of-dim curve)
> was independently re-raised and also remains open.
...
Reviewed 2026-07-07 (Claude). Scope: `32_clustering.tex` (46 frame-numbers / 63 physical
pages), `32_clustering_OUTLINE.md`, all figures in `fig/`, generator `py_src/cluster_demos.py`.

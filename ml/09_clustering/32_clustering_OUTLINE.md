...
- Examples: **per-frame toy 2D blobs** (no single threaded example) + the full **sklearn gallery**
  image, framed as "the wider zoo". PCA = one pointer frame (`35_dimensionality_reduction`). Caveats frame kept.
- Code: one `fit_predict` snippet (note `random_state`). Practical teaser: image compression.
- File: overwrite `32_clustering.tex`.
...
27. **Cluster after PCA.** Reduce then cluster; pointer to `35_dimensionality_reduction`.

...
`ml/11_neural_networks/L17_rnns_lstm.tex` is a skeleton — 7 frames, every content frame a `% TODO: port from lecture_i2dl/rnn` comment, with 5 sections (sequential data + vanilla RNN, BPTT, LSTM/GRU, attention, applications) crammed into one deck and no hands-on path or homework.
...
Two lecture units in `ml/11_neural_networks/`:
...
**Homework `.qmd`:** CNN and RNN homeworks share the NN-chapter `.qmd` at `ml/11_neural_networks/05_neural_networks__concepts.qmd` (created by whichever block lands first), following course conventions: `{data-difficulty="1|2|3"}` 🧀 levels and `{.content-visible when-profile="solution"}`.

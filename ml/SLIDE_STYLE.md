...
- **Full-bleed / full-screen frames — an approved default, not an exception (added 2026-07-21).** When a frame's payload is one strong visual — a big architecture diagram, a photo, an animation still, a borrowed video still, a rich generated figure — prefer a `[plain]` **full-screen** frame that lets the image fill the slide over the usual title+body+footer structure. Use it **sometimes**, by judgment: go full-bleed when the picture should do the talking (visualization-rich moments); keep the standard structure when the frame needs surrounding prose. Good practice: set up a full-bleed still with a short *preceding* normal frame, and keep only a small source/caption line (and optionally suppress the footer for the cleanest look).
  - **Mechanism.** 16:9 deck: `\usebackgroundtemplate{\includegraphics[width=\paperwidth,height=\paperheight]{fig/...}}` wrapped around a `[plain]` frame (see the welchlabs full-bleed pattern in `ml/11_neural_networks`). 4:3 deck (e.g. the `dl_*` LMU-embed set): letterbox the image on black so it is not distorted — see the `\bbslide` macro in `ml/ch6_cnn/dl_cnn_conv_math.tex`. Put the attribution/caption in a small `tikz` overlay node.
  - **Embedding external slides (e.g. LMU IML, CC BY 4.0) — added 2026-07-24.** Taking whole slides from an openly-licensed course is allowed; do **not** re-caveat licensing. Pull the *compiled* slide PDF from `github.com/slds-lmu/lecture_iml` (`slides-pdf/`, find via the GitHub API tree), extract the single page with `pypdf` into the deck's `fig/` (e.g. `lmu_shapley_value.pdf`), then drop it on a `[plain]` frame: `\includegraphics[width=\paperwidth,height=0.9\paperheight,keepaspectratio]{fig/lmu_*.pdf}` with a `{\scriptsize Source: LMU Interpretable Machine Learning (CC BY 4.0)}` line beneath. LMU's slides are 16:9 like ours, so they fill cleanly with just that strip. Pattern used for the SHAP-computation, worked-PFI, and husky slides in `ml/05_interpretability`. Embed when LMU's asset is strictly better (a real figure, a canonical worked table); author natively when the value is narrative fit (running example, controlled numbers).
...
- **Abbreviations (convention).** Spell the **full name on first use, then introduce the abbreviation**, e.g. "Gradient-based One-Side Sampling (GOSS)", "Exclusive Feature Bundling (EFB)", "DART (Dropouts meet Multiple Additive Regression Trees)". After that, the abbreviation alone is fine.
  - **Exempt: obvious and already-taught acronyms (added 2026-08-02, instructor).** Do **not** expand terms that every reader of these decks already knows, or that the course itself has taught in an earlier chapter. Expanding them is noise, not rigor. The exempt set:
    - **Universal technical:** `ML`, `AI`, `CPU`, `GPU`, `API`, `PDF`, `HTML`, `CSV`, `SQL`, `IID`, `US`.
    - **Standard metrics and math:** `MSE`, `MAE`, `RMSE`, `MAPE`, `AUC`, `ROC`, `KL`, `MLE`, `MAP`, `ELBO`, `SGD`, `PDF`/`CDF`.
    - **Methods the course has already taught:** `PCA`, `CNN`, `RNN`, `LSTM`, `GRU`, `VAE`, `AE`, `SAE`, `SVM`, `KNN`, `NLP`, `LLM` - once the chapter that introduced them has been delivered. Signpost with the chapter on first cross-chapter use ("the RNN from ch7") instead of re-expanding.
    - **Named entities, venues, licenses, datasets:** `GPT`, `BERT`, `LMU`, `ICLR`, `NeurIPS`, `CC BY`, `MNIST`, `CIFAR`, `IMDB`.
  - The rule exists for terms a reader **cannot** look up from context - method names lifted from a paper (`SAGE`, `LOCO`, `GOSS`, `EFB`), or coined in the deck. Apply it there and nowhere else.
  - **This rule keeps getting broken, so verify it mechanically** - don't trust a read-through. Before a deck is done, run:
    ...
    Then, for every acronym in that list **that is not in the exempt set above**, confirm the deck expands it *somewhere* (`grep -i "full name" DECK.tex`). Ignore LaTeX markup and code keywords. If the check flags only exempt terms, the deck passes - do not "fix" it.

...
\begin{frame}{What about a fixed window?}
  ...
  \begin{itemize}
    \item L19's \textbf{1D convolution} - a $k$-token kernel sliding along the text;
  ...
  tell whether one pomegranate or many was sent - the word that decides it is 9 words
  back.
  \vskip 6pt
  \fcolorbox{armblue}{armblue!8}{\parbox{0.97\linewidth}{\small
    Stacking conv layers widens the window (L17's receptive field), but the width is
...
\begin{frame}{Forward pass, summarized}
  \begin{columns}[T]
    ...
    \vskip 8pt
    {\footnotesize In practice $d_h=64$-$512$, not $2$ - but the recipe is identical. And
    it is \textbf{fixed}: 3 words or 3,000, the whole past must fit into those $d_h$
    numbers - a lossy summary L21 will come back to.}
  \end{columns}
\end{frame}
...
\begin{frame}{The taxonomy}
  ...
  diagram's "many-to-X" naming = our "seq-to-X": same taxonomy. Diagram: Karpathy
  (2015), "The Unreasonable Effectiveness of Recurrent Neural Networks" - L21 returns
  to that essay.}
\end{frame}
...
\begin{frame}{Backprop through time}
  \begin{columns}[T]
    ...
    needs nothing new - [42]'s \texttt{loss.backward()} (one backward sweep, any graph),
    kept again. A tied weight collects the \textbf{sum} of its gradients from every step
    - the same rule as a shared conv kernel (L16).
    \vskip 6pt
    ...
    \vskip 6pt
    {\footnotesize Formalized as "backpropagation through time" by Werbos (1990) -
    four years after backprop itself (Rumelhart, Hinton \& Williams, 1986).}
...
\begin{frame}{The full derivation, part 1}
  ...
  $(\mathbf{V}^\top)^{k}\mathbf{v} = \lambda^{k}\mathbf{v}$. In that direction the matrix
  power \emph{is} the scalar story, $0.8^{29}$ again - and after many steps the direction
  with the largest $|\lambda|$ dominates everything else.}
\end{frame}
...
\begin{frame}{Empirical proof}
  \begin{columns}[T]
    ...
    \vskip 6pt
    \fcolorbox{armred}{armred!8}{\parbox{0.95\linewidth}{\small
      Not asserted - \textbf{measured}. Note the log scale: the gradient reaching step 1
      is over \textbf{300,000$\times$} smaller than at the end - and it has already lost
      $1000\times$ within the last 15 steps.}}

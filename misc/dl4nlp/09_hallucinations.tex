...
% ============================================================
% CITATION / ATTRIBUTION SYSTEMS
% ============================================================
\begin{frame}
\frametitle{Trend: citation \& attribution systems}

\begin{center}
\begin{tikzpicture}
  % Key idea
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, text width=11cm, align=center, inner sep=8pt, font=\small] at (0, 3) {
    \textbf{2024--25 trend:} production LLMs increasingly provide \textbf{inline citations}\\
    for their claims, grounding outputs in retrievable, verifiable sources.
  };
  ...
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (-4.5, 1.2) {
    \textbf{\textcolor{paramgreen}{Perplexity}}\\[3pt]
    {\scriptsize Every answer includes\\numbered source links.\\Users can verify each claim.\\Search-augmented generation.}
  };

  \node[draw=popblue, fill=popblue!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (-0.5, 1.2) {
    \textbf{\textcolor{popblue}{Gemini + Google\\Search grounding}}\\[3pt]
    {\scriptsize Queries Google Search\\at inference time.\\Returns answers with\\linked source snippets.}
  };

  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (3.5, 1.2) {
    \textbf{\textcolor{orange1}{GPT-4 + Browse /\\Bing integration}}\\[3pt]
    {\scriptsize Web retrieval with\\inline citations.\\Sources shown as\\clickable footnotes.}
  };

  % How it works
  \node[draw=violet1, fill=violet1!5, rounded corners=8pt, text width=11cm, align=center, inner sep=6pt, font=\small] at (0, -0.7) {
    \textbf{Typical pipeline:} query $\to$ retrieve documents $\to$ generate answer\\
    with \textbf{per-sentence source annotations} $\to$ post-hoc NLI verification of each citation.
  };

  % Limitations
  \node[draw=warnred, fill=warnred!5, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\scriptsize] at (-3, -2.5) {
    \textbf{Open challenges:}\\
    Citations may not support the claim\\(``citation hallucination'').\\Source quality varies.
  };
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\scriptsize] at (3.5, -2.5) {
    \textbf{Why it helps:}\\
    Shifts burden to \emph{verification}.\\Users can spot-check sources.\\Builds appropriate trust.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
% ============================================================
\section{Faithfulness, benchmarks \& wrap-up}
\begin{frame}
...
\begin{tabular}{>{\bfseries}l l c l}
  ...
  \textcolor{violet1}{SimpleQA} & Factual QA accuracy & 4,326 Q & OpenAI 2024; short-fact Qs \\
  \hline
\end{tabular}
}
\end{center}
...
% ============================================================
\begin{frame}
\frametitle{Benchmark spotlight: SimpleQA}

\begin{center}
\begin{tikzpicture}
  % What is it
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, text width=11cm, align=center, inner sep=8pt, font=\small] at (0, 3) {
    \textbf{SimpleQA} (OpenAI, 2024): 4,326 short-fact questions with verified ground-truth answers.\\[3pt]
    Designed to measure \textbf{factual accuracy} and \textbf{calibration} --- does the model\\
    answer correctly \emph{or} admit it doesn't know?
  };

  % Three metrics
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (-4.5, 1) {
    \textbf{\textcolor{paramgreen}{Correct}}\\[3pt]
    {\scriptsize Model's answer matches\\the verified fact.}
  };
  \node[draw=sampred, fill=sampred!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (-0.5, 1) {
    \textbf{\textcolor{sampred}{Incorrect}}\\[3pt]
    {\scriptsize Model gives a confident\\but wrong answer.}
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (3.5, 1) {
    \textbf{\textcolor{orange1}{Not attempted}}\\[3pt]
    {\scriptsize Model declines to\\answer (``I don't know'').}
  };

  % Key finding
  \node[draw=warnred, fill=warnred!5, rounded corners=8pt, text width=11cm, align=center, inner sep=6pt, font=\small] at (0, -0.7) {
    \textbf{Key finding:} even frontier models (GPT-4o, Claude 3.5, Gemini) score only\\
    \textbf{30--40\% correct} on SimpleQA. Models that attempt more answers tend to\\
    hallucinate more --- highlighting the \textbf{accuracy--coverage trade-off}.
  };

  % Why it matters
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=11cm, align=center, inner sep=5pt, font=\small] at (0, -2.3) {
    \textbf{Why it matters:} SimpleQA penalizes both hallucination \emph{and} over-refusal,\\
    encouraging well-calibrated models that know when to say ``I don't know.''
  };

  \node[font=\scriptsize, text=gray] at (0, -3.2) {Wei et al., ``Measuring short-form factuality in large language models,'' OpenAI 2024};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.88, transform shape]
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 2.5) {
    \textbf{\textcolor{popblue}{Hallucination Surveys}}\\[4pt]
    {\small
    \textbullet~Ji et al.\ (2023), ``Survey of Hallucination in Natural Language Generation''\\[2pt]
    \textbullet~Huang et al.\ (2023), ``A Survey on Hallucination in Large Language Models''\\[2pt]
    \textbullet~Tonmoy et al.\ (2024), ``A Comprehensive Survey of Hallucination Mitigation Techniques in LLMs''
    }
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 0.2) {
    \textbf{\textcolor{paramgreen}{Detection \& Evaluation}}\\[4pt]
    {\small
    \textbullet~Manakul et al.\ (2023), ``SelfCheckGPT: Zero-Resource Black-Box Hallucination Detection''\\[2pt]
    \textbullet~Min et al.\ (2023), ``FActScore: Fine-grained Atomic Evaluation of Factual Precision''
    }
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -1.8) {
    \textbf{\textcolor{orange1}{Mitigation Approaches}}\\[4pt]
    {\small
    \textbullet~Lewis et al.\ (2020), ``Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks''\\[2pt]
    \textbullet~Kadavath et al.\ (2022), ``Language Models (Mostly) Know What They Know''
    }
  };
\end{tikzpicture}
\end{center}

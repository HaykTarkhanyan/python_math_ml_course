...
% TIMELINE
% ============================================================
\section{The 2018 breakthroughs}
\begin{frame}
...
\begin{tikzpicture}
  ...
  \node[font=\small\bfseries, text=popblue] at (0, 3.3) {Parameters scaled 1500$\times$ in just two years};
...
\begin{tikzpicture}
  ...
  % Architecture
  \node[draw=violet1, fill=violet1!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.5, 0.95) {
    \textbf{Architecture:}\\[3pt]
    Character CNN $\to$ 2-layer\\biLSTM (4096 units)\\[3pt]
    Forward + backward LMs\\[3pt]
    Final: learned per-task\\weighted sum of layers
  };

  % The innovation
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.5, 0.95) {
  ...
  % Example
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -1.55) {
  ...
  % Impact
  \node[draw=orange1, fill=orange1!5, rounded corners=6pt, text width=12cm, align=center, inner sep=4pt, font=\scriptsize] at (0, -2.95) {
...
\begin{tikzpicture}
  ...
  % Stage 1
  \node[draw=popblue, fill=popblue!10, rounded corners=8pt, minimum width=4.5cm, minimum height=1.9cm, text width=4.2cm, align=center, inner sep=6pt, font=\small] at (-3.5, 1.85) {
  ...
  % Stage 2
  \node[draw=paramgreen, fill=paramgreen!10, rounded corners=8pt, minimum width=4.5cm, minimum height=1.9cm, text width=4.2cm, align=center, inner sep=6pt, font=\small] at (3.5, 1.85) {
  ...
  \draw[-Stealth, very thick, orange1] (0, 1.85) -- (1, 1.85);
  ...
  % Results
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.5, -1.95) {
    \textbf{Results:}\\[3pt]
    SOTA on 9/12 tasks\\+8.9\% commonsense\\+5.7\% QA (RACE)\\Beat specialized models
  };

  % Key insight
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.5, -1.95) {
...
\begin{tikzpicture}
  ...
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (3.5, 0.5) {
  ...
  \draw[-Stealth, very thick, paramgreen] (-0.3, 0.5) -- (0.5, 0.5);

  % Architecture table
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt] at (0, -1.05) {
    {\small
  ...
  % Training data
  \node[font=\small, text=gray] at (0, -2.25) {
    Trained on BooksCorpus (800M words) + English Wikipedia (2.5B words) = 3.3B words total
  };

  % Bottom
  \node[draw=popblue, fill=popblue!10, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt, font=\small] at (0, -3.0) {
...
\begin{tikzpicture}
  ...
  % Three embeddings
  \node[font=\small\bfseries, text=orange1] at (0, -0.4) {Input = Token Embedding + Segment Embedding + Position Embedding};
...
\begin{tikzpicture}
  ...
  \node[font=\small, text width=4.5cm, align=center] at (-3.5, 1.0) {
    Randomly mask 15\% of tokens\\[4pt]
    Model predicts original token\\using bidirectional context\\[4pt]
    Of the 15\% masked: 80\% $\to$ [MASK],\\10\% random, 10\% kept
  };
  ...
  % Training cost
  \node[font=\scriptsize, text=gray] at (0, -3.35) {
    Training: BERT-Base on 4 TPUs (16 chips), 4 days, $\sim$\$500 $\cdot$ BERT-Large used 16 TPUs (64 chips)
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  % GLUE
  \node[draw=popblue, fill=popblue!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (-3.5, 1.95) {
  ...
  % SQuAD
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (3.5, 1.95) {
  ...
  % NLI
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (-3.5, -0.2) {
    \textbf{MultiNLI:}\\[2pt]
  ...
  % NER
  \node[draw=violet1, fill=violet1!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (3.5, -0.2) {
    \textbf{CoNLL NER:}\\[2pt]
  ...
  % Impact
  \node[draw=popblue, fill=popblue!10, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -2.0) {
    \textbf{The BERT effect:} one pre-trained model + a linear head beat \emph{years} of\\
    task-specific engineering --- the default NLP starting point, 2018--2020.
  };
...
\begin{tikzpicture}
  ...
  \node[draw=popblue, fill=popblue!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (-3.5, -0.45) {
    \textbf{vs.\ GPT-1:}\\[2pt]
    13$\times$ more parameters\\Pre-LayerNorm (more stable)\\Context: 1024 tokens (vs.\ 512)\\WebText: 40 GB (vs.\ 800M words)
  };

  % Zero-shot
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (3.5, -0.45) {
    \textbf{Zero-shot breakthrough:}\\[2pt]
    SOTA on 7/8 LM benchmarks\\with \textbf{no fine-tuning at all}\\[1pt]
    {\scriptsize Coherent multi-paragraph text}
  };

  % "Too dangerous to release"
  \node[draw=warnred, fill=warnred!8, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -2.1) {
    \textbf{``Too dangerous to release'':} OpenAI withheld the full model for 9 months.\\
    Staged 2019 release: Small $\to$ Medium $\to$ Large $\to$ XL.\\[1pt]
    {\scriptsize First major debate about responsible AI release. Mostly a PR event in hindsight.}
  };

  % Key lesson
  \node[draw=orange1, fill=orange1!5, rounded corners=6pt, text width=12cm, align=center, inner sep=4pt, font=\scriptsize] at (0, -3.25) {
...
\begin{tikzpicture}
  ...
  % Task examples
  \node[draw=popblue, fill=popblue!8, rounded corners=4pt, text width=5cm, align=left, inner sep=4pt, font=\scriptsize] at (-3.5, 1.6) {
    \textbf{Translation:}\\
    ``translate English to German: Hello''\\$\to$ ``Hallo''
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=4pt, text width=5cm, align=left, inner sep=4pt, font=\scriptsize] at (3.5, 1.6) {
    \textbf{Summarization:}\\
    ``summarize: [long article]''\\$\to$ ``[short summary]''
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=4pt, text width=5cm, align=left, inner sep=4pt, font=\scriptsize] at (-3.5, 0.35) {
    \textbf{Classification:}\\
    ``sst2 sentence: The movie was great''\\$\to$ ``positive''
  };
  \node[draw=violet1, fill=violet1!8, rounded corners=4pt, text width=5cm, align=left, inner sep=4pt, font=\scriptsize] at (3.5, 0.35) {
  ...
  \node at (0, -1.25) {
    ...
    \begin{tabular}{l r r r}
      \textbf{Variant} & \textbf{Params} & \textbf{Enc/Dec layers} & \textbf{$d_{\text{ff}}$ (width)} \\
      \hline
      Small & 60M & 6 / 6 & 2{,}048 \\
      Base & 220M & 12 / 12 & 3{,}072 \\
      Large & 770M & 24 / 24 & 4{,}096 \\
      \textbf{11B} & \textbf{11B} & \textbf{24 / 24} & \textbf{65{,}536} \\
    \end{tabular}
    }
  };

  % Key contributions
  \node[draw=sampred, fill=sampred!8, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt, font=\small] at (0, -2.9) {
    \textbf{Encoder-decoder} + span-corruption denoising objective.\\
    Introduced \textbf{C4} (750 GB) $\cdot$ a landmark systematic transfer study.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  % Scale comparison
  \node[font=\small\bfseries, text=warnred] at (0, 1.5) {100$\times$ larger than GPT-2, 1500$\times$ larger than GPT-1};

  % Training data
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.5, -0.15) {
    \textbf{Training data:}\\[3pt]
    300B tokens total\\Common Crawl (60\%)\\WebText2 (22\%)\\Books (16\%) + Wikipedia (3\%)
  };

  % Cost
  \node[draw=warnred, fill=warnred!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.5, -0.15) {
    \textbf{Training cost:}\\[3pt]
    355 GPU-years\\$\sim$\textbf{\$4.6 million}\\[2pt]
    {\scriptsize Third-party estimate ---\\OpenAI never disclosed compute}
  };

  % Model family
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt, font=\small] at (0, -1.9) {
    \textbf{8 model sizes} spanning 3 orders of magnitude:\\[2pt]
    {\small Ada (350M) $\to$ Babbage (1.3B) $\to$ Curie (6.7B) $\to$ \textbf{Davinci (175B)}}
  };

  % API
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt, font=\small] at (0, -3.0) {
...
\begin{tikzpicture}
  ...
  % Key finding
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt, font=\small] at (0, -1.4) {
  ...
  % Results
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=12cm, align=center, inner sep=4pt, font=\small] at (0, -2.85) {
...
\begin{center}
\begin{tikzpicture}
  \node[font=\small\bfseries, text=popblue] at (0, 3.5) {Accuracy vs.\ model size --- schematic (shape of the GPT-3 result)};
  ...
  \node[font=\scriptsize, text=gray] at (0, -3.2) {Model size (log scale)};
...
% THE EVOLUTION
% ============================================================
\section{Lessons \& what came next}
\begin{frame}
...
\begin{tikzpicture}
  ...
  \node at (0, 1.1) {
    ...
    \begin{tabular}{l l r l}
      ...
      LLaMA & Mixed (7 sources) & 1.4T tokens & Publicly available data only \\
  ...
  % Key lessons
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (-3.5, -1.75) {
  ...
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (3.5, -1.75) {
  ...
  \node[font=\scriptsize, text=gray] at (0, -3.1) {
    From $\sim$1B words (2018) to $\sim$2T tokens (2023) --- $\sim$1000$\times$ more training data (words vs.\ tokens)
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
% ============================================================
\begin{frame}
\frametitle{What came next: the post-2020 landscape}

\vspace{-0.3cm}
\begin{center}
\begin{tikzpicture}[scale=0.88, transform shape]
  % Timeline axis
  \draw[thick, -Stealth, popblue!40] (-7, 2.8) -- (7, 2.8);
  \node[font=\tiny, text=gray] at (-6, 3.1) {2022};
  \node[font=\tiny, text=gray] at (-1, 3.1) {2023};
  \node[font=\tiny, text=gray] at (4, 3.1) {2024};

  % Row 1: 2022
  \node[draw=orange1, fill=orange1!15, rounded corners=4pt, inner sep=4pt, font=\scriptsize, text width=2.2cm, align=center] at (-6, 2.0) {
    \textbf{Chinchilla}\\{\tiny 70B, DeepMind}\\{\tiny Compute-optimal\\scaling laws}
  };

  % Row 2: 2023
  \node[draw=paramgreen, fill=paramgreen!15, rounded corners=4pt, inner sep=4pt, font=\scriptsize, text width=2.2cm, align=center] at (-3.5, 2.0) {
    \textbf{LLaMA}\\{\tiny 7--65B, Meta}\\{\tiny Open-source\\revolution}
  };
  \node[draw=popblue, fill=popblue!15, rounded corners=4pt, inner sep=4pt, font=\scriptsize, text width=2.2cm, align=center] at (-1, 2.0) {
    \textbf{GPT-4}\\{\tiny OpenAI}\\{\tiny Multimodal; ``sparks\\of AGI'' (MSR)}
  };
  \node[draw=violet1, fill=violet1!15, rounded corners=4pt, inner sep=4pt, font=\scriptsize, text width=2.2cm, align=center] at (1.5, 2.0) {
    \textbf{Mixtral}\\{\tiny 8$\times$7B $\cdot$ Dec 2023}\\{\tiny Open MoE,\\near GPT-3.5}
  };

  % Row 3: 2023-24
  \node[draw=sampred, fill=sampred!15, rounded corners=4pt, inner sep=4pt, font=\scriptsize, text width=2.2cm, align=center] at (4, 2.0) {
    \textbf{LLaMA 3}\\{\tiny 8--405B, Meta}\\{\tiny Open-weight\\frontier model}
  };
  \node[draw=warnred, fill=warnred!15, rounded corners=4pt, inner sep=4pt, font=\scriptsize, text width=2.2cm, align=center] at (6.2, 2.0) {
    \textbf{DeepSeek-V3}\\{\tiny 671B MoE}\\{\tiny Efficient training\\open-weight}
  };

  % Key themes
  \node[font=\small\bfseries, text=popblue] at (0, 0.55) {Key themes of the post-GPT-3 era};

  \node[draw=orange1, fill=orange1!8, rounded corners=4pt, text width=3.4cm, align=center, inner sep=5pt, font=\scriptsize] at (-5, -1.15) {
    \textbf{\textcolor{orange1}{Compute-optimal scaling}}\\[2pt]
    Chinchilla showed models were\\over-parameterized \& under-trained.\\Optimal: scale data \& params equally.
  };

  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=4pt, text width=3.4cm, align=center, inner sep=5pt, font=\scriptsize] at (-1.5, -1.15) {
    \textbf{\textcolor{paramgreen}{Open-source/weight}}\\[2pt]
    LLaMA sparked an explosion\\of open models: Alpaca,\\Vicuna, Mistral, Phi, Qwen, \ldots
  };

  \node[draw=popblue, fill=popblue!8, rounded corners=4pt, text width=3.4cm, align=center, inner sep=5pt, font=\scriptsize] at (2, -1.15) {
    \textbf{\textcolor{popblue}{Multimodal + reasoning}}\\[2pt]
    GPT-4, Gemini, Claude:\\vision, audio, tool use,\\extended thinking
  };

  \node[draw=violet1, fill=violet1!8, rounded corners=4pt, text width=3.4cm, align=center, inner sep=5pt, font=\scriptsize] at (5.5, -1.15) {
    \textbf{\textcolor{violet1}{Mixture of Experts}}\\[2pt]
    Mixtral, DeepSeek-V3:\\large capacity with efficient\\inference via sparse activation
  };

  % Bridge note
  \node[draw=popblue, fill=popblue!10, rounded corners=6pt, text width=13cm, align=center, inner sep=5pt, font=\small] at (0, -2.95) {
    \textbf{We will cover these models, RLHF/alignment, and advanced prompting in upcoming lectures.}
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{center}
\begin{tikzpicture}[scale=0.88, transform shape]
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 2.7) {
    \textbf{\textcolor{popblue}{GPT Series}}\\[4pt]
    {\small
    \textbullet~Radford et al.\ (2018), ``Improving Language Understanding by Generative Pre-Training'' (GPT-1)\\[2pt]
    \textbullet~Radford et al.\ (2019), ``Language Models are Unsupervised Multitask Learners'' (GPT-2)\\[2pt]
    \textbullet~Brown et al.\ (2020), ``Language Models are Few-Shot Learners'' (GPT-3)
    }
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 0.1) {
    \textbf{\textcolor{paramgreen}{BERT \& Variants}}\\[4pt]
    {\small
    \textbullet~Devlin et al.\ (2019), ``BERT: Pre-training of Deep Bidirectional Transformers''\\[2pt]
    \textbullet~Liu et al.\ (2019), ``RoBERTa: A Robustly Optimized BERT Pretraining Approach''
    }
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -2.1) {
    \textbf{\textcolor{orange1}{T5 \& Encoder-Decoder}}\\[4pt]
    {\small
    \textbullet~Raffel et al.\ (2020), ``Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer'' (T5)\\[2pt]
    \textbullet~Lewis et al.\ (2020), ``BART: Denoising Sequence-to-Sequence Pre-training''
    }
  };
\end{tikzpicture}
\end{center}

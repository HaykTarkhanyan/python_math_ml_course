...
\subtitle{CLM $\cdot$ MLM $\cdot$ NSP $\cdot$ SFT $\cdot$ RLHF $\cdot$ DPO $\cdot$ GRPO $\cdot$ LoRA}
\date{}
...
\begin{tikzpicture}
  ...
  % Arrows
  \draw[-Stealth, very thick, popblue] (pt) -- (ft);
  \draw[-Stealth, very thick, paramgreen] (ft) -- (dep);
  \node[font=\scriptsize, text=popblue] at (-2.25, 2.75) {base model};
  \node[font=\scriptsize, text=paramgreen] at (2.25, 2.75) {aligned model};
...
\begin{tikzpicture}
  ...
  \node[font=\scriptsize, text=gray] at (0, 0.2) {Each token can only see tokens to its \textbf{left} (causal mask)};

  % Pros and cons
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=left, inner sep=6pt, font=\small] at (-3.5, -1.7) {
    \textbf{\textcolor{paramgreen}{Advantages:}}\\[3pt]
    Natural for text generation\\[2pt]
    Every token contributes to loss\\[2pt]
    Simple and scalable
  };
  \node[draw=warnred, fill=warnred!8, rounded corners=6pt, text width=5cm, align=left, inner sep=6pt, font=\small] at (3.5, -1.7) {
...
\begin{tikzpicture}
  ...
  \node[draw, rounded corners=4pt, fill=sampred!10, minimum width=2.5cm, font=\small, text width=2.3cm, align=center] at (-4, -1.35) {
    \textbf{80\%}\\replace with [MASK]
  };
  \node[draw, rounded corners=4pt, fill=orange1!10, minimum width=2.5cm, font=\small, text width=2.3cm, align=center] at (0, -1.35) {
    \textbf{10\%}\\replace with random token
  };
  \node[draw, rounded corners=4pt, fill=paramgreen!10, minimum width=2.5cm, font=\small, text width=2.3cm, align=center] at (4, -1.35) {
    \textbf{10\%}\\keep unchanged
  };

  % Why: prevent mismatch
  \node[font=\scriptsize, text=gray, text width=12cm, align=center] at (0, -2.5) {
    Why the 10\%+10\%? [MASK] never appears at fine-tune time --- prevents a train/test mismatch
  };

  % Models
  \node[font=\small, text=gray] at (0, -3.05) {
...
\begin{tikzpicture}
  ...
  \fill[sampred] (3, -0.1) circle (5pt);
  \node[font=\scriptsize, anchor=west] at (3.3, -0.1) {``She plays piano''};
  \draw[sampred, thick, dashed] (-2.5, 1.3) -- (1.8, 1.9);
  \draw[sampred, thick, dashed] (-2.3, 0.7) -- (2.8, 0.1);
  \node[font=\tiny, text=sampred] at (0, 2.2) {push};

  % InfoNCE loss
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, inner sep=8pt, text width=10cm, align=center] at (0, -1.25) {
    {\normalsize $\displaystyle \mathcal{L}_{\text{InfoNCE}} = -\log \frac{\exp(\text{sim}(z_i, z_i^+) / \tau)}{\sum_{j=1}^{N} \exp(\text{sim}(z_i, z_j) / \tau)}$}
  };

  % Applications
  \node[draw=violet1, fill=violet1!8, rounded corners=4pt, text width=3.5cm, align=center, inner sep=5pt, font=\small] at (-4.5, -2.95) {
    \textbf{\textcolor{violet1}{SimCSE}}\\[2pt]
    {\scriptsize Sentence embeddings\\(same input, different dropout)}
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=4pt, text width=3.5cm, align=center, inner sep=5pt, font=\small] at (0, -2.95) {
    \textbf{\textcolor{orange1}{CLIP}}\\[2pt]
    {\scriptsize Image--text alignment\\(match image to caption)}
  };
  \node[draw=popblue, fill=popblue!8, rounded corners=4pt, text width=3.5cm, align=center, inner sep=5pt, font=\small] at (4.5, -2.95) {
...
\begin{tikzpicture}
  ...
  \node[draw=sampred, fill=sampred!5, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.5, 0) {
    \textbf{\textcolor{sampred}{ELECTRA --- RTD (bonus):}}\\[3pt]
    \textbf{R}eplaced \textbf{T}oken \textbf{D}etection: a small generator makes fake tokens; a discriminator flags them.\\All tokens give signal.
  };
...
% FROM BASE TO USEFUL
% ============================================================
\section{Fine-tuning \& instruction tuning}
\begin{frame}
...
% ============================================================
\begin{frame}
\frametitle{Constitutional AI --- alignment from AI feedback}

\begin{center}
\begin{tikzpicture}
  % Intro
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, 3.2) {
    \textbf{Bai et al.\ (2022), Anthropic:} replace (most) human feedback with \textbf{AI feedback}.\\
    Define a ``constitution'' of principles; let the model critique and revise its own outputs.
  };

  % Two phases
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.5, 1.0) {
    \textbf{\textcolor{paramgreen}{Phase 1: Supervised (RLAIF)}}\\[3pt]
    {\scriptsize 1.\ Model generates response\\
    2.\ Model \emph{critiques} it using a principle\\
    \quad (e.g., ``Is this harmful?'')\\
    3.\ Model \emph{revises} its response\\
    4.\ Train SFT on revised outputs}
  };

  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.5, 1.0) {
    \textbf{\textcolor{orange1}{Phase 2: RL (RLAIF)}}\\[3pt]
    {\scriptsize Train a reward model using\\
    \textbf{AI-generated} preference labels\\
    instead of human annotators.\\[2pt]
    Then run RLHF as usual.}
  };

  % Constitution example
  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=12cm, align=left, inner sep=6pt, font=\scriptsize] at (0, -0.8) {
    \textbf{Example constitutional principle:} ``Choose the response that is most helpful while being \emph{harmless} and \emph{honest}. Avoid responses that are toxic, racist, sexist, or encourage illegal activity.''
  };

  % Advantages
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=5.5cm, align=center, inner sep=5pt, font=\small] at (-3, -2.3) {
    \textbf{Advantages:}\\[2pt]
    {\scriptsize Scales cheaply (no human labelers)\\
    Principles are explicit and auditable\\
    Less harmful than pure RLHF}
  };
  \node[draw=warnred, fill=warnred!5, rounded corners=6pt, text width=5.5cm, align=center, inner sep=5pt, font=\small] at (3.5, -2.3) {
    \textbf{Limitations:}\\[2pt]
    {\scriptsize AI judge inherits model biases\\
...
\begin{tikzpicture}
  ...
  % Key insight
  \node[draw=warnred, fill=warnred!5, rounded corners=6pt, text width=11cm, align=center, inner sep=5pt, font=\small] at (0, -2.75) {
...
\begin{tikzpicture}
  ...
  % What clipping does
  \node[font=\small\bfseries, text=popblue] at (-3.5, 1.4) {What does clipping do?};
  ...
  % Explanation boxes
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=4.5cm, align=center, inner sep=6pt, font=\small] at (3, 0.55) {
  ...
  \node[draw=warnred, fill=warnred!8, rounded corners=6pt, text width=4.5cm, align=center, inner sep=6pt, font=\small] at (3, -1.55) {
  ...
  % Typical epsilon
  \node[font=\scriptsize, text=gray] at (-2.7, -2.75) {Typical: $\epsilon = 0.2$ \quad $\cdot$ \quad $\hat{A}_t$ computed via GAE (generalized advantage estimation)};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\scriptsize] at (3.5, -0.9) {
    ...
    Often adapted to hit a target KL
  };

  % Per-token KL
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=11cm, align=center, inner sep=5pt, font=\small] at (0, -2.35) {
    In practice, KL is computed \textbf{per token}: $\displaystyle D_{\text{KL}} = \sum_{t=1}^{T} \log \frac{\pi_\theta(y_t|x,y_{<t})}{\pi_{\text{ref}}(y_t|x,y_{<t})}$
  };

  % Takeaway
  \node[font=\scriptsize, text=gray, text width=13cm, align=center] at (0, -3.25) {
...
% DPO
% ============================================================
\section{Preference optimization: DPO \& GRPO}
\begin{frame}
...
\begin{tikzpicture}
  ...
  % DPO loss
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, inner sep=5pt, align=center] at (0, 1.55) {
  ...
  % Comparison
  \node[draw=warnred, fill=warnred!8, rounded corners=6pt, text width=4.5cm, align=center, inner sep=5pt, font=\small] at (-3, -0.55) {
    \textbf{\textcolor{warnred}{RLHF}}\\[2pt]
    3 stages\\4 models in memory\\Unstable RL training\\Complex hyperparameters
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=4.5cm, align=center, inner sep=5pt, font=\small] at (3, -0.55) {
  ...
  \draw[-Stealth, very thick, paramgreen] (-0.5, -0.55) -- (0.5, -0.55);

  % Results
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=10cm, align=center, inner sep=5pt, font=\small] at (0, -2.4) {
    DPO matches or exceeds PPO quality: 61\% win rate vs.\ PPO's 57\% on summarization
  };

  % Models
  \node[font=\small, text=gray] at (0, -3.1) {
    \textbf{Used by:} Zephyr, Tulu, many open-source models \quad |\quad \textbf{Same optimum, simpler in practice}
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  % Step 2
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=12cm, align=left, inner sep=5pt, font=\small] at (0, 2.25) {
    \textbf{Step 2:} Closed-form optimal policy \quad $\displaystyle \pi^*(y|x) = \frac{1}{Z(x)}\, \pi_{\text{ref}}(y|x)\, \exp\!\Big(\frac{R(x,y)}{\beta}\Big)$ \quad {\scriptsize ($Z(x)$: normalizer)}
  };

  % Step 3
  \node[draw=orange1, fill=orange1!5, rounded corners=6pt, text width=12cm, align=left, inner sep=5pt, font=\small] at (0, 1.3) {
    \textbf{Step 3:} Solve for reward: \quad $R(x,y) = \beta \log \dfrac{\pi^*(y|x)}{\pi_{\text{ref}}(y|x)} + \beta \log Z(x)$
  };

  % Step 4
  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=12cm, align=left, inner sep=5pt, font=\small] at (0, 0.2) {
  ...
  % Final result
  \node[draw=sampred, fill=sampred!8, rounded corners=8pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -1.85) {
    \textbf{Result --- DPO loss:}\\[6pt]
    {\normalsize $\displaystyle \mathcal{L}_{\text{DPO}} = -\mathbb{E}\!\left[\log \sigma\!\left(\beta \log \frac{\pi_\theta(y_w|x)}{\pi_{\text{ref}}(y_w|x)} - \beta \log \frac{\pi_\theta(y_l|x)}{\pi_{\text{ref}}(y_l|x)}\right)\right]$}
  };

  % Intuition
  \node[draw=popblue, fill=lightbg, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt, font=\small] at (0, -3.25) {
...
\begin{tikzpicture}
  ...
  \node[font=\scriptsize, text=violet1] at (-0.5, 2.85) {$G$ samples};
  ...
  % Formula
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, inner sep=8pt, text width=8cm, align=center] at (0, -0.6) {
    {\normalsize $\displaystyle \hat{A}_i = \frac{r_i - \text{mean}(r_1, \ldots, r_G)}{\text{std}(r_1, \ldots, r_G)}$}\\[6pt]
    {\small No value network needed --- advantage from group statistics}\\[2pt]
    {\scriptsize \textcolor{warnred}{Caveat:} if all $G$ rewards are equal, std $= 0$ (no signal)}
  };

  % Key results
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (-3.5, -2.9) {
    \textbf{DeepSeek-R1-Zero:}\\[3pt]
    GRPO with \emph{no SFT at all}\\$\Rightarrow$ emergent reasoning!
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (3.5, -2.9) {
...
\begin{tabular}{>{\bfseries}l c c c c}
  ...
  \textcolor{orange1}{GRPO} & 2 (policy, ref) & Medium & Verifiable & Math/code reasoning \\[3pt]
  \textcolor{violet1}{CAI (RLAIF)} & 2 (policy, ref) + AI judge & Stable & AI-generated & Scalable safety \\
  \hline
\end{tabular}
}
\end{center}
...
\begin{tikzpicture}
  % Model size → VRAM
  \node[font=\small\bfseries, text=popblue] at (-3.3, 3.5) {Full fine-tuning needs enormous memory};
...
\begin{tikzpicture}
  ...
  % Formula
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, inner sep=8pt, text width=8cm, align=center] at (0, -1.0) {
...
% ============================================================
\begin{frame}
\frametitle{QLoRA --- Quantized LoRA}
...
\begin{tikzpicture}
  ...
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, text width=11cm, align=center, inner sep=6pt, font=\small] at (0, 3) {
    \textbf{Dettmers et al.\ (NeurIPS 2023):} store frozen weights $W_0$ in \textbf{4-bit} precision,\\
    train LoRA adapters in 16-bit. Fine-tune a \textbf{65B} model on a \textbf{single 48GB GPU}.
  };

  % Three innovations
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (-4.5, 1.0) {
    \textbf{\textcolor{paramgreen}{4-bit NormalFloat}}\\[3pt]
    {\scriptsize Quantization type\\optimized for normally\\distributed weights.\\Better than INT4.}
  };

  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (0, 1.0) {
    \textbf{\textcolor{orange1}{Double quantization}}\\[3pt]
    {\scriptsize Quantize the quantization\\constants themselves.\\Saves $\sim$0.37 bits per\\parameter extra.}
  };

  \node[draw=violet1, fill=violet1!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=5pt, font=\small] at (4.5, 1.0) {
    \textbf{\textcolor{violet1}{Paged optimizers}}\\[3pt]
    {\scriptsize Offload optimizer states\\to CPU RAM on OOM.\\Uses NVIDIA unified\\memory.}
  };

  % Memory comparison
  \node[font=\small\bfseries, text=popblue] at (0, -0.4) {Memory comparison (65B model fine-tuning):};

  \node at (0, -1.4) {
    {\small
    \renewcommand{\arraystretch}{1.05}
    \begin{tabular}{l c c}
      \textbf{Method} & \textbf{VRAM} & \textbf{GPUs needed} \\
      \hline
      Full fine-tuning (16-bit) & $>$780 GB & $\sim$10$\times$ A100 \\
      LoRA (16-bit base) & $\sim$130 GB & 2$\times$ A100 \\
      \textbf{QLoRA (4-bit base)} & \textbf{$\sim$33 GB} & \textbf{1$\times$ A100} \\
    \end{tabular}
    }
  };

  % Result
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=12.8cm, align=center, inner sep=5pt, font=\small] at (0, -3.05) {
    \textbf{Key result:} QLoRA matches 16-bit LoRA quality on all benchmarks.\\
    Guanaco (QLoRA-tuned LLaMA 65B) reached \textbf{99.3\%} of ChatGPT quality on Vicuna.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tabular}{>{\bfseries}l l c c c}
  ...
  \textcolor{paramgreen}{QLoRA} & LoRA + 4-bit base weights & $\sim$0.01\% & No overhead & Excellent \\[2pt]
...
\begin{tikzpicture}[scale=0.85]
  ...
  % ChatGPT
  \node[draw, rounded corners=4pt, fill=popblue!8, text width=13cm, align=left, inner sep=5pt, font=\scriptsize] at (0, -1.1) {
    \textbf{ChatGPT:} \quad Pre-train (GPT-3.5/4) $\to$ SFT (demos) $\to$ RLHF (PPO, human prefs)
  };
  % Llama
  \node[draw, rounded corners=4pt, fill=paramgreen!8, text width=13cm, align=left, inner sep=5pt, font=\scriptsize] at (0, -1.9) {
    \textbf{Llama 2 Chat:} \quad Pre-train (2T tokens) $\to$ SFT (27.5k examples) $\to$ RLHF (iterative, 5 rounds)
  };
  % DeepSeek
  \node[draw, rounded corners=4pt, fill=orange1!8, text width=13cm, align=left, inner sep=5pt, font=\scriptsize] at (0, -2.7) {
    \textbf{DeepSeek-R1:} \quad Pre-train $\to$ cold-start SFT $\to$ GRPO $\to$ SFT (distilled) $\to$ GRPO again
  };
  % Zephyr
  \node[draw, rounded corners=4pt, fill=violet1!8, text width=13cm, align=left, inner sep=5pt, font=\scriptsize] at (0, -3.5) {
...
\begin{tikzpicture}
  ...
  \node[draw=popblue, fill=popblue!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.5, 2) {
    ...
    $\Rightarrow$ \textbf{DPO} or \textbf{RLHF} (max control)\\[2pt]
  ...
  \node[draw=violet1, fill=violet1!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.5, -0.3) {
    ...
    $\Rightarrow$ \textbf{MLM} (BERT-style)\\[2pt]
  ...
  \node[draw=sampred, fill=sampred!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.5, -2.5) {
    ...
    $\Rightarrow$ \textbf{CLM} (GPT-style)\\[2pt]
...
\begin{tikzpicture}[scale=0.88, transform shape]
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 2.75) {
    \textbf{\textcolor{popblue}{Pre-training}}\\[4pt]
    {\footnotesize
    \textbullet~Devlin et al.\ (2019), ``BERT'' --- masked language modeling\\[2pt]
    \textbullet~Radford et al.\ (2019), ``GPT-2'' --- causal language modeling
    }
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -0.1) {
    \textbf{\textcolor{paramgreen}{RLHF \& Alignment}}\\[4pt]
    {\footnotesize
    \textbullet~Ouyang et al.\ (2022), ``Training LMs to follow instructions'' (InstructGPT)\\[2pt]
    \textbullet~Rafailov et al.\ (2023), ``Direct Preference Optimization'' (DPO)\\[2pt]
    \textbullet~Shao et al.\ (2024), ``DeepSeekMath'' (GRPO) \quad \textbullet~Bai et al.\ (2022), ``Constitutional AI'' (RLAIF)
    }
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -2.65) {
    \textbf{\textcolor{orange1}{Parameter-Efficient Fine-Tuning}}\\[4pt]
    {\footnotesize
    \textbullet~Hu et al.\ (2022), ``LoRA: Low-Rank Adaptation of Large Language Models''\\[2pt]
    \textbullet~Dettmers et al.\ (2023), ``QLoRA: Efficient Finetuning of Quantized Language Models''\\[2pt]
    \textbullet~Houlsby et al.\ (2019), ``Parameter-Efficient Transfer Learning for NLP'' (Adapters)
    }
  };
\end{tikzpicture}
\end{center}

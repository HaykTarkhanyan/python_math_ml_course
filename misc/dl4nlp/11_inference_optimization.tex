...
\begin{tabular}{>{\bfseries}l c c c c}
  ...
  \textcolor{orange1}{AWQ} & Required & 3--4 & Fast & Production, multi-modal \\[2pt]
  \textcolor{violet1}{GGUF} & None & 2--8 & Fast (CPU) & llama.cpp, local/edge \\
  \hline
\end{tabular}
}
\end{center}
...
\begin{tikzpicture}
  ...
  % AWQ
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=3.4cm, align=center, inner sep=5pt, font=\small] at (3.2, 0.5) {
  ...
  % GGUF
  \node[draw=violet1, fill=violet1!8, rounded corners=6pt, text width=3.4cm, align=center, inner sep=5pt, font=\small] at (6.8, 0.5) {
    \textbf{\textcolor{violet1}{GGUF}}\\[3pt]
    {\scriptsize File format for llama.cpp.\\CPU + GPU hybrid inference.\\Many quant levels (Q2--Q8).\\Best for local/edge deployment.}
  };

  % Bottom recommendation
  \node[draw=popblue, fill=lightbg, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -1.5) {
    \textbf{Quick rule:} bitsandbytes to try $\to$ GPTQ/AWQ to deploy (GPU) $\to$ GGUF for CPU/edge $\to$ QLoRA to fine-tune
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -1.5) {
    ...
    {\scriptsize Key enabler: \textbf{PagedAttention} --- manages KV-cache like OS virtual memory pages}\\[2pt]
    {\scriptsize First introduced as ``iteration-level scheduling'' in \textbf{Orca} (Yu et al., 2022)}
  };
  ...
  \node[draw=violet1, fill=violet1!8, rounded corners=4pt, text width=3cm, align=center, inner sep=5pt, font=\small] at (6, -3) {
    ...
    {\scriptsize RadixAttention for\\prefix caching\\Structured output\\(Zheng et al., 2024)}
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.88, transform shape]
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 2.5) {
    \textbf{\textcolor{popblue}{Quantization}}\\[4pt]
    {\small
    \textbullet~Frantar et al.\ (2023), ``GPTQ: Accurate Post-Training Quantization for Generative Pretrained Transformers''\\[2pt]
    \textbullet~Lin et al.\ (2024), ``AWQ: Activation-aware Weight Quantization for LLM Compression''\\[2pt]
    \textbullet~Dettmers et al.\ (2023), ``QLoRA: Efficient Finetuning of Quantized Language Models''
    }
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 0.3) {
    \textbf{\textcolor{paramgreen}{Efficient Serving}}\\[4pt]
    {\small
    \textbullet~Kwon et al.\ (2023), ``Efficient Memory Management for LLM Serving with PagedAttention'' (vLLM)\\[2pt]
    \textbullet~Yu et al.\ (2022), ``Orca: A Distributed Serving System for Transformer-Based Generative Models''\\[2pt]
    \textbullet~Zheng et al.\ (2024), ``SGLang: Efficient Execution of Structured Language Model Programs''\\[2pt]
    \textbullet~Dao et al.\ (2022), ``FlashAttention: Fast and Memory-Efficient Exact Attention''
    }
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -1.7) {
    \textbf{\textcolor{orange1}{Distillation \& Pruning}}\\[4pt]
    {\small
    \textbullet~Hinton et al.\ (2015), ``Distilling the Knowledge in a Neural Network''\\[2pt]
    \textbullet~Sanh et al.\ (2019), ``DistilBERT, a distilled version of BERT: smaller, faster, cheaper and lighter''
    }
  };
\end{tikzpicture}
\end{center}

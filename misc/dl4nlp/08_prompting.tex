...
\begin{tikzpicture}
  ...
  % Direct injection
  \node[draw=sampred, fill=sampred!8, rounded corners=6pt, text width=5.2cm, align=left, inner sep=5pt, font=\scriptsize] at (-3.5, 1.2) {
  ...
  % Indirect injection
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=5.2cm, align=left, inner sep=5pt, font=\scriptsize] at (3.5, 1.2) {
  ...
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=3.4cm, align=center, inner sep=5pt, font=\scriptsize] at (-4.5, -1.7) {
    \textbf{Input sanitization}\\[2pt]
    Use delimiters, XML tags\\to separate instructions\\from user content
  };

  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=3.4cm, align=center, inner sep=5pt, font=\scriptsize] at (0, -1.7) {
    \textbf{Output filtering}\\[2pt]
    Post-process outputs\\to detect leaked system\\prompts or harmful text
  };

  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=3.4cm, align=center, inner sep=5pt, font=\scriptsize] at (4.5, -1.7) {
    \textbf{Least privilege}\\[2pt]
    Limit what tools and\\data the model can\\access per request
  };

  \node[draw=warnred, fill=warnred!8, rounded corners=6pt, text width=12cm, align=center, inner sep=4pt, font=\small] at (0, -2.9) {
    \textbf{Open problem:} no complete defense exists. Prompt injection is to LLMs\\
    what SQL injection was to web apps --- a fundamental architectural challenge.
  };
\end{tikzpicture}
\end{center}
\end{frame}

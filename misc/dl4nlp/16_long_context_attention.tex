...
% ============================================================
% NATIVE LONG CONTEXT & INFINI-ATTENTION
% ============================================================
\begin{frame}
\frametitle{Native long context training}
\vspace{-0.2cm}
\begin{center}
\begin{tikzpicture}
  % Post-hoc vs native
  \node[draw=warnred, fill=warnred!5, rounded corners=8pt, text width=4.8cm, align=center, inner sep=6pt] at (-3.5, 2.2) {
    {\small\bfseries\textcolor{warnred}{Post-hoc extension}}\\[4pt]
    {\scriptsize Train on short context (4K--8K),\\
    then extend via interpolation\\
    (PI, YaRN, LongRoPE).\\[3pt]
    Quality degrades at the\\
    long-context boundary.}
  };

  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=8pt, text width=4.8cm, align=center, inner sep=6pt] at (3.5, 2.2) {
    {\small\bfseries\textcolor{paramgreen}{Native long context}}\\[4pt]
    {\scriptsize Train \textbf{from scratch} on long sequences.\\
    Gemini 1.5/2.0: up to \textbf{2M tokens}.\\
    Claude 3/3.5: \textbf{200K} natively.\\[3pt]
    Better long-range coherence;\\
    no interpolation artifacts.}
  };

  \draw[-Stealth, very thick, orange1] (-0.5, 2.2) -- (0.5, 2.2);
  \node[font=\scriptsize, text=orange1, above] at (0, 2.2) {trend};

  % Infini-attention
  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=10cm, align=center, inner sep=5pt, font=\scriptsize] at (0, -0.2) {
    \textbf{Infini-attention} (Munkhdalai et al., Google, 2024): compressive memory within standard attention.\\[3pt]
    Divide sequence into segments; compress earlier segments into a fixed-size buffer.\\
    Bounded memory for unbounded context. \textbf{114$\times$} less memory; 1M-token passkey retrieval.\\[3pt]
    {\tiny Promising idea, but Ring Attention and native long context remain dominant in practice.}
  };

  % Bottom insight
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=11cm, align=center, inner sep=5pt, font=\small] at (0, -1.8) {
    \textbf{The trend:} move from ``train short, extend later'' to ``train long natively.''\\
    {\scriptsize This requires efficient training (FlashAttention + Ring Attention) but avoids quality degradation.}
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.88, transform shape]
  ...
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -1.7) {
    ...
    \textbullet~Liu et al.\ (TACL 2024), ``Lost in the Middle: How LLMs Use Long Contexts''\\[2pt]
    \textbullet~Munkhdalai et al.\ (2024), ``Leave No Context Behind: Efficient Infinite Context Transformers''
    }
  };
\end{tikzpicture}
\end{center}

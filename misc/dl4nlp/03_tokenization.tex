...
\begin{tikzpicture}[scale=0.82, transform shape]
  ...
  % Problems
  \node[draw=warnred, fill=warnred!5, rounded corners=6pt, text width=4.5cm, align=left, inner sep=6pt, font=\small] at (-4, -1.4) {
    \textbf{\textcolor{warnred}{Drawbacks:}}\\[3pt]
    \textbullet\ Sequences $\sim$4--5$\times$ longer\\[1pt]
  ...
  % Advantages
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=4.5cm, align=left, inner sep=6pt, font=\small] at (4, -1.4) {
    \textbf{\textcolor{paramgreen}{Advantages:}}\\[3pt]
    \textbullet\ Tiny vocabulary ($\sim$256)\\[1pt]
  ...
  % Verdict
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=11cm, align=center, inner sep=6pt, font=\small] at (0, -3.3) {
...
\begin{tikzpicture}[scale=0.82, transform shape]
  ...
  % Insight
  \node[font=\scriptsize, text=gray, text width=8cm, align=center] at (0, -4) {
...
\begin{tikzpicture}[scale=0.82, transform shape]
  % Setup
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=13cm, align=left, inner sep=6pt, font=\small] at (0, 3.3) {
    \textbf{Corpus:}\; cheese ($\times$10),\; cheek ($\times$8),\; cheap ($\times$5),\; seep ($\times$7) \quad\quad
    Current state: all split to characters
  };

  % Two candidate pairs
  \node[font=\small\bfseries, text=gray] at (0, 2.2) {Which pair to merge: (\texttt{e},\,\texttt{e}) or (\texttt{e},\,\texttt{p})\;?};

  % BPE column
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, minimum width=6cm, minimum height=4.5cm] at (-4, -0.3) {};
  \node[font=\normalsize\bfseries, text=popblue] at (-4, 1.5) {BPE: count frequency};

  \node[font=\small, text width=5cm, align=left] at (-4, 0.3) {
    (\texttt{e},\,\texttt{e}): cheese($\times$10) + cheek($\times$8)\\
    $= \textbf{18}$\\[6pt]
    (\texttt{e},\,\texttt{p}): cheap($\times$5) + seep($\times$7)\\
    $= 12$
  };

  \node[draw=popblue, fill=popblue!15, rounded corners=4pt, font=\small, inner sep=5pt] at (-4, -1.5) {
    Winner: (\texttt{e},\,\texttt{e}) --- higher count
  };

  % WordPiece column
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, minimum width=6cm, minimum height=4.5cm] at (4, -0.3) {};
  \node[font=\normalsize\bfseries, text=paramgreen] at (4, 1.5) {WordPiece: likelihood score};

  \node[font=\small, text width=5cm, align=left] at (4, 0.3) {
    $\dfrac{\text{freq}(\texttt{ee})}{\text{freq}(\texttt{e}) \times \text{freq}(\texttt{e})} = \dfrac{18}{43 \times 43} = 0.0097$\\[10pt]
    $\dfrac{\text{freq}(\texttt{ep})}{\text{freq}(\texttt{e}) \times \text{freq}(\texttt{p})} = \dfrac{12}{43 \times 12} = \textbf{0.0233}$
  };

  \node[draw=paramgreen, fill=paramgreen!15, rounded corners=4pt, font=\small, inner sep=5pt] at (4, -1.5) {
    Winner: (\texttt{e},\,\texttt{p}) --- higher score
  };

  % Insight
  \node[draw=orange1, fill=orange1!5, rounded corners=6pt, text width=13cm, align=center, inner sep=6pt, font=\small] at (0, -3.2) {
    WordPiece prefers merging pairs whose co-occurrence is \textbf{surprising} relative to individual frequencies.\\[2pt]
    ``\texttt{e}'' is very common, so ``\texttt{ee}'' is not surprising. But ``\texttt{ep}'' appearing together is more informative.
  };

  \node[font=\scriptsize, text=gray] at (0, -4.2) {
    freq(\texttt{e}) = 43 (summing all \texttt{e}'s in the corpus), \;freq(\texttt{p}) = 12 (from ``cheap'' and ``seep'')
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.82, transform shape]
  ...
  \node[font=\small\bfseries, orange1] at (7.2, -1.2) {39};
...
\begin{tikzpicture}[
  ...
  \node[trendbox, fill=paramgreen!8, draw=paramgreen, line width=1.5pt] at (-4, 0.8) {
    ...
    \textbullet\ No brittle pre-tokenization\\
    \quad (regex, Unicode tables)\\[2pt]

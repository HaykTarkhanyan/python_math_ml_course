...
% ============================================================
\begin{frame}
\frametitle{Q, K, V --- where do they come from?}
\vspace{-0.3cm}
\begin{center}
\begin{tikzpicture}[scale=0.88, transform shape]
  % Input X
  \node[draw=popblue, fill=popblue!10, rounded corners=6pt, minimum width=2.8cm, minimum height=1.8cm, font=\small, align=center] (X) at (0, 2.5) {
    \textbf{Input $X$}\\[2pt]
    {\scriptsize $(n \times d_{\text{model}})$}
  };

  % Three projection arrows
  \draw[-Stealth, very thick, popblue] (X.south) -- ++(-4.5, -2) node[midway, above, sloped, font=\scriptsize] {$\times\; W_Q$};
  \draw[-Stealth, very thick, orange1] (X.south) -- ++(0, -2) node[midway, right, font=\scriptsize] {$\times\; W_K$};
  \draw[-Stealth, very thick, paramgreen] (X.south) -- ++(4.5, -2) node[midway, above, sloped, font=\scriptsize] {$\times\; W_V$};

  % Q, K, V boxes
  \node[draw=popblue, fill=popblue!15, rounded corners=6pt, minimum width=2.5cm, minimum height=1.4cm, font=\small, align=center] (Q) at (-4.5, -0.8) {
    \textbf{Query $Q$}\\[2pt]
    {\scriptsize $(n \times d_k)$}
  };
  \node[draw=orange1, fill=orange1!15, rounded corners=6pt, minimum width=2.5cm, minimum height=1.4cm, font=\small, align=center] (K) at (0, -0.8) {
    \textbf{Key $K$}\\[2pt]
    {\scriptsize $(n \times d_k)$}
  };
  \node[draw=paramgreen, fill=paramgreen!15, rounded corners=6pt, minimum width=2.5cm, minimum height=1.4cm, font=\small, align=center] (V) at (4.5, -0.8) {
    \textbf{Value $V$}\\[2pt]
    {\scriptsize $(n \times d_v)$}
  };

  % Weight matrix annotations
  \node[font=\scriptsize, text=popblue] at (-4.5, -1.9) {$W_Q \in \mathbb{R}^{d_{\text{model}} \times d_k}$};
  \node[font=\scriptsize, text=orange1] at (0, -1.9) {$W_K \in \mathbb{R}^{d_{\text{model}} \times d_k}$};
  \node[font=\scriptsize, text=paramgreen] at (4.5, -1.9) {$W_V \in \mathbb{R}^{d_{\text{model}} \times d_v}$};

  % Key insight
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12.5cm, align=center, inner sep=6pt, font=\small] at (0, -2.8) {
    \textbf{Same input, three different views} --- the model \textbf{learns} what to query, what to advertise, and what to send
  };

  % Self vs cross note
  \node[draw=orange1, fill=orange1!5, rounded corners=6pt, text width=5.5cm, align=center, inner sep=5pt, font=\scriptsize] at (-3.5, -4) {
    \textbf{Self-attention:} $Q$, $K$, $V$ all from same $X$
  };
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=5.5cm, align=center, inner sep=5pt, font=\scriptsize] at (3.5, -4) {
    \textbf{Cross-attention:} $Q$ from decoder,\\$K$, $V$ from encoder
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.75, transform shape]
  ...
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=4pt, text width=4.2cm, align=left, inner sep=5pt, font=\scriptsize, anchor=west] at (3.5, 1) {
    \textbf{Feed-Forward (per token)}\\
    $\text{FFN}(x) = W_2 \cdot \text{GELU}(W_1 x + b_1) + b_2$\\
    Hidden dim: $4 \times d_{\text{model}}$\\[2pt]
    {\tiny Modern: \textbf{SwiGLU} (LLaMA, Mistral)}
  };

  \node[draw=sampred, fill=sampred!5, rounded corners=4pt, text width=4.2cm, align=left, inner sep=5pt, font=\scriptsize, anchor=west] at (-7.5, -0.5) {
    \textbf{Residual connections}\\
    $\text{output} = x + \text{sublayer}(x)$\\
    Each layer reads from and writes to\\a shared \textbf{residual stream}
  };

  \node[draw=orange1, fill=orange1!5, rounded corners=4pt, text width=4.2cm, align=left, inner sep=5pt, font=\scriptsize, anchor=west] at (-7.5, 2.5) {
    \textbf{Layer normalization}\\
    Stabilizes training\\
    Pre-norm (GPT) vs post-norm (original)\\[2pt]
    {\tiny Modern: \textbf{RMSNorm} (faster, no mean)}
  };
...
% ============================================================
\begin{frame}
\frametitle{Modern Transformer block variants}
\vspace{-0.4cm}
\begin{center}
\begin{tikzpicture}[scale=0.82, transform shape]
  % === Pre-LN vs Post-LN ===
  \node[font=\normalsize\bfseries, text=popblue] at (0, 4) {Three key changes in modern LLMs};

  % Post-LN (original)
  \node[draw=sampred, fill=sampred!5, rounded corners=8pt, minimum width=3.5cm, minimum height=4.5cm] at (-5, 1) {};
  \node[font=\small\bfseries, text=sampred] at (-5, 3) {Post-LN {\scriptsize(original)}};

  \node[draw, rounded corners=3pt, fill=lightbg, font=\tiny, minimum width=2.2cm] (po_in) at (-5, 2.2) {Input $x$};
  \node[draw, rounded corners=3pt, fill=popblue!12, font=\tiny, minimum width=2.2cm] (po_attn) at (-5, 1.5) {Sublayer};
  \node[draw, rounded corners=3pt, fill=lightbg, font=\tiny, minimum width=2.2cm] (po_add) at (-5, 0.7) {$x + $ Sublayer};
  \node[draw, rounded corners=3pt, fill=orange1!15, font=\tiny, minimum width=2.2cm] (po_ln) at (-5, -0.1) {LayerNorm};
  \draw[-Stealth, thin] (po_in) -- (po_attn);
  \draw[-Stealth, thin] (po_attn) -- (po_add);
  \draw[-Stealth, thin] (po_add) -- (po_ln);
  \node[font=\tiny, text=sampred] at (-5, -0.8) {Unstable at large scale};

  % Pre-LN (modern)
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=8pt, minimum width=3.5cm, minimum height=4.5cm] at (-1, 1) {};
  \node[font=\small\bfseries, text=paramgreen] at (-1, 3) {Pre-LN {\scriptsize(GPT, LLaMA)}};

  \node[draw, rounded corners=3pt, fill=lightbg, font=\tiny, minimum width=2.2cm] (pr_in) at (-1, 2.2) {Input $x$};
  \node[draw, rounded corners=3pt, fill=orange1!15, font=\tiny, minimum width=2.2cm] (pr_ln) at (-1, 1.5) {LayerNorm};
  \node[draw, rounded corners=3pt, fill=popblue!12, font=\tiny, minimum width=2.2cm] (pr_attn) at (-1, 0.7) {Sublayer};
  \node[draw, rounded corners=3pt, fill=lightbg, font=\tiny, minimum width=2.2cm] (pr_add) at (-1, -0.1) {$x + $ Sublayer};
  \draw[-Stealth, thin] (pr_in) -- (pr_ln);
  \draw[-Stealth, thin] (pr_ln) -- (pr_attn);
  \draw[-Stealth, thin] (pr_attn) -- (pr_add);
  \node[font=\tiny, text=paramgreen] at (-1, -0.8) {Stable, no warmup needed};

  % === RMSNorm ===
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=4.8cm, align=left, inner sep=6pt, font=\small] at (3.5, 2.5) {
    \textbf{\textcolor{orange1}{RMSNorm}} {\scriptsize(Zhang \& Sennrich, 2019)}\\[4pt]
    $\text{RMSNorm}(x) = \dfrac{x}{\text{RMS}(x)} \cdot \gamma$\\[6pt]
    $\text{RMS}(x) = \sqrt{\frac{1}{d}\sum_{i=1}^{d} x_i^2}$\\[6pt]
    {\scriptsize Drops the mean-centering of LayerNorm.\\Faster: saves one reduction op per layer.}\\[3pt]
    {\scriptsize\textcolor{orange1}{Used by: LLaMA, Mistral, PaLM, Gemma}}
  };

  % === SwiGLU ===
  \node[draw=violet1, fill=violet1!8, rounded corners=6pt, text width=4.8cm, align=left, inner sep=6pt, font=\small] at (3.5, -1.5) {
    \textbf{\textcolor{violet1}{SwiGLU activation}} {\scriptsize(Shazeer, 2020)}\\[4pt]
    $\text{SwiGLU}(x) = \text{Swish}(xW_1) \odot (xW_3)$\\[4pt]
    {\scriptsize Replaces ReLU/GELU in FFN. Uses a \textbf{gating} mechanism (element-wise product of two projections). Third weight matrix $W_3$ added; hidden dim adjusted to $\frac{8}{3}d$ to match param count.}\\[3pt]
    {\scriptsize\textcolor{violet1}{Used by: LLaMA, PaLM, Mistral, Gemma}}
  };

  % Summary
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12.5cm, align=center, inner sep=5pt, font=\small] at (0, -3.7) {
    Modern recipe: \textbf{Pre-LN} + \textbf{RMSNorm} + \textbf{SwiGLU} + \textbf{RoPE} --- small changes, large impact on training stability and performance
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  % Dropout note
  \node[font=\scriptsize, text=gray, text width=8cm, align=center] at (0, -3.2) {
    \textbf{Dropout} ($p{=}0.1$) applied after attention weights and after each sublayer during training
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
% ============================================================
\section{Why it works \& practice}
\begin{frame}
\frametitle{How Transformers learn}
\vspace{-0.3cm}
\begin{center}
\begin{tikzpicture}[scale=0.88, transform shape]
  % CLM column (left)
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, minimum width=6cm, minimum height=6.5cm] at (-3.8, -0.2) {};
  \node[font=\normalsize\bfseries, text=orange1] at (-3.8, 2.8) {Causal LM (decoder)};

  % CLM example
  \node[font=\small, text width=5cm, align=center] at (-3.8, 1.8) {
    ``The cat sat on the \rule{1cm}{0.4pt}''
  };
  \draw[-Stealth, thick, orange1] (-3.8, 1.3) -- (-3.8, 0.7);
  \node[draw, rounded corners=4pt, fill=orange1!15, font=\small, minimum width=3cm] at (-3.8, 0.4) {Transformer};
  \draw[-Stealth, thick, orange1] (-3.8, 0.05) -- (-3.8, -0.5);
  \node[draw, rounded corners=4pt, fill=lightbg, font=\small, minimum width=3cm] at (-3.8, -0.8) {softmax over vocab};

  % Prediction
  \node[font=\small] at (-3.8, -1.5) {$P(\text{``mat''}) = 0.42$};

  % Loss
  \node[draw=warnred, fill=warnred!5, rounded corners=4pt, text width=4.5cm, align=center, inner sep=4pt, font=\scriptsize] at (-3.8, -2.4) {
    $\mathcal{L} = -\displaystyle\sum_{t=1}^{T} \log P(x_t \mid x_{<t})$\\[3pt]
    Cross-entropy on \textbf{every} position
  };

  % Teacher forcing note
  \node[font=\tiny, text=gray, text width=4.5cm, align=center] at (-3.8, -3.3) {
    Teacher forcing: use true tokens as input, not model's predictions
  };

  % MLM column (right)
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, minimum width=6cm, minimum height=6.5cm] at (3.8, -0.2) {};
  \node[font=\normalsize\bfseries, text=popblue] at (3.8, 2.8) {Masked LM (encoder)};

  % MLM example
  \node[font=\small, text width=5cm, align=center] at (3.8, 1.8) {
    ``The \textbf{\textcolor{sampred}{[MASK]}} sat on the mat''
  };
  \draw[-Stealth, thick, popblue] (3.8, 1.3) -- (3.8, 0.7);
  \node[draw, rounded corners=4pt, fill=popblue!15, font=\small, minimum width=3cm] at (3.8, 0.4) {Transformer};
  \draw[-Stealth, thick, popblue] (3.8, 0.05) -- (3.8, -0.5);
  \node[draw, rounded corners=4pt, fill=lightbg, font=\small, minimum width=3cm] at (3.8, -0.8) {softmax over vocab};

  % Prediction
  \node[font=\small] at (3.8, -1.5) {$P(\text{``cat''}) = 0.67$};

  % Loss
  \node[draw=warnred, fill=warnred!5, rounded corners=4pt, text width=4.5cm, align=center, inner sep=4pt, font=\scriptsize] at (3.8, -2.4) {
    $\mathcal{L} = -\displaystyle\sum_{i \in \mathcal{M}} \log P(x_i \mid x_{\setminus \mathcal{M}})$\\[3pt]
    Loss only on \textbf{masked} positions
  };
  ...
  % Dominant note
  \node[draw=orange1, fill=orange1!5, rounded corners=6pt, text width=12.5cm, align=center, inner sep=5pt, font=\scriptsize] at (0, -4.3) {
    Since GPT-3: \textbf{causal LM is the dominant paradigm} --- one training objective, scales to any task via prompting
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.82, transform shape]
  % Two main costs
  \node[draw=popblue, fill=popblue!10, rounded corners=6pt, minimum width=5.5cm, minimum height=2cm, align=center, inner sep=8pt] at (-3.5, 2.5) {
    \textbf{\textcolor{popblue}{\large Self-Attention}}\\[4pt]
    {\normalsize $O(n^2 \cdot d)$ \quad time}\\[2pt]
    {\normalsize $O(n^2)$ \quad memory}\\[2pt]
    {\scriptsize ($QK^\top$ is an $n \times n$ matrix)}
  };

  \node[draw=paramgreen, fill=paramgreen!10, rounded corners=6pt, minimum width=5.5cm, minimum height=2cm, align=center, inner sep=8pt] at (3.5, 2.5) {
    \textbf{\textcolor{paramgreen}{\large Feed-Forward}}\\[4pt]
    {\normalsize $O(n \cdot d^2)$ \quad time}\\[2pt]
    {\normalsize $O(d^2)$ \quad memory}\\[2pt]
    {\scriptsize (two $d \times 4d$ weight matrices)}
  };

  % Which dominates?
  \node[font=\small\bfseries, text=gray] at (0, 0.6) {Which dominates?};

  % Regime comparison
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.5, -0.5) {
    \textbf{Short context} ($n < d$)\\[3pt]
    FFN dominates\\[2pt]
    {\scriptsize E.g., $n{=}512$, $d{=}4096$}\\
    {\scriptsize Attention: $0.5\text{M} \cdot d$ \;vs\; FFN: $512 \cdot 16\text{M}$}
  };

  \node[draw=sampred, fill=sampred!5, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.5, -0.5) {
    \textbf{Long context} ($n > d$)\\[3pt]
    \textcolor{sampred}{\textbf{Attention dominates}}\\[2pt]
    {\scriptsize E.g., $n{=}128\text{K}$, $d{=}4096$}\\
    {\scriptsize Attention: $16\text{B} \cdot d$ \;vs\; FFN: $128\text{K} \cdot 16\text{M}$}
  };

  % The quadratic wall visual
  \node[font=\small\bfseries, text=sampred] at (-4.5, -2.2) {The quadratic wall};

  \draw[thick, ->] (-6.5, -4.2) -- (-2.5, -4.2) node[right, font=\scriptsize] {Sequence length $n$};
  \draw[thick, ->] (-6.5, -4.2) -- (-6.5, -2.5) node[above, font=\scriptsize] {Cost};

  % Quadratic curve
  \draw[very thick, sampred] (-6.3, -4.1) .. controls (-5.5, -3.8) and (-4.5, -3.2) .. (-3, -2.6);
  \node[font=\tiny, text=sampred] at (-3, -2.3) {$n^2$};

  % Linear curve
  \draw[very thick, paramgreen] (-6.3, -4.1) .. controls (-5, -3.7) and (-4, -3.5) .. (-3, -3.3);
  \node[font=\tiny, text=paramgreen] at (-3, -3.0) {$n$};

  % Concrete numbers
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=5.5cm, align=left, inner sep=6pt, font=\scriptsize] at (3.5, -2.5) {
    \textbf{\textcolor{orange1}{Doubling sequence length:}}\\[3pt]
    \textbullet~Attention: $4\times$ more compute\\[1pt]
    \textbullet~KV cache memory: $2\times$ more\\[1pt]
    \textbullet~Attention matrix: $4\times$ more memory\\[4pt]
    {\footnotesize $128\text{K tokens} \rightarrow 16$ \textbf{billion} entries per head per layer}
  };

  % Bridge to long context deck
  \node[font=\scriptsize, text=violet1, text width=12cm, align=center] at (0, -4.7) {
...
\begin{center}
\begin{tikzpicture}[scale=0.9, transform shape]
  % Boxes stack top-down with fixed gaps (below=of), so they never overlap however text wraps.
  % Attention & Transformers
  \node[draw=popblue, fill=popblue!8, rounded corners=6pt, text width=13cm, align=left, inner sep=6pt, anchor=north] (pb) at (0, 4.15) {
    \textbf{\textcolor{popblue}{Attention, Transformers \& visual explainers}}\\[2pt]
    {\footnotesize
    \textbullet~Vaswani et al.\ (2017), ``Attention Is All You Need'' --- the original Transformer paper\\
    ...
    \textbullet~Brendan Bycroft, ``LLM Visualization'' --- watch a GPT run in interactive 3D --- \texttt{bbycroft.net/llm}
    }
  };

  % Positional Encodings
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=13cm, align=left, inner sep=6pt, below=0.28cm of pb] (pg) {
    \textbf{\textcolor{paramgreen}{Positional Encodings}}\\[2pt]
    {\footnotesize
    \textbullet~Su et al.\ (2021), ``RoFormer: Enhanced Transformer with Rotary Position Embedding'' (RoPE)\\
    \textbullet~Press et al.\ (2022), ``Train Short, Test Long: Attention with Linear Biases'' (ALiBi)
    }
  };

  % Architecture Variants & Surveys
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=13cm, align=left, inner sep=6pt, below=0.28cm of pg] {
    \textbf{\textcolor{orange1}{Architecture Variants \& Surveys}}\\[2pt]
    {\footnotesize
    \textbullet~Devlin et al.\ (2019), ``BERT: Pre-training of Deep Bidirectional Transformers'' (encoder-only)\\
    \textbullet~Radford et al.\ (2018/2019), ``Improving/Language Models are Unsupervised Multitask Learners'' (GPT/GPT-2)\\

...
\begin{tikzpicture}[scale=0.88, transform shape]
  ...
  \node at (3.5, -0.5) {
    ...
    \begin{tabular}{l l}
      ...
      \hline
      In-context learning & $\sim$175B (GPT-3) \\[1pt]
...
% ============================================================
% IN-CONTEXT LEARNING AS EMERGENT
% ============================================================
\begin{frame}
\frametitle{In-context learning: emergence without weight updates}
\vspace{-0.3cm}
\begin{center}
\begin{tikzpicture}[scale=0.88, transform shape]
  % The phenomenon
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, text width=12.5cm, align=center, inner sep=8pt, font=\normalsize] at (0, 3) {
    \textbf{In-context learning (ICL):} a model learns a new task from examples\newline
    \textbf{in the prompt alone} --- no gradient updates, no fine-tuning
  };

  % Example
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5.5cm, align=left, inner sep=6pt] at (-3.5, 0.8) {
    {\small\bfseries\textcolor{paramgreen}{Prompt:}}\par\vspace{3pt}
    {\small ``happy'' $\to$ ``triste''\newline
    ``cat'' $\to$ ``gato''\newline
    ``blue'' $\to$ }
  };

  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=5.5cm, align=left, inner sep=6pt] at (3.5, 0.8) {
    {\small\bfseries\textcolor{orange1}{Theoretical explanations:}}\par\vspace{3pt}
    {\small \textbullet\ \textbf{Implicit Bayesian inference}\newline
    {\scriptsize (Xie et al., ICLR 2022): Transformers\newline
    implicitly infer the latent concept}\par\vspace{2pt}
    \textbullet\ \textbf{Mesa-optimization}\newline
    {\scriptsize (von Oswald et al., ICML 2023):\newline
    Transformers implement gradient\newline
    descent in their forward pass}}
  };

  % Why emergent
  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=12.5cm, align=center, inner sep=6pt, font=\small] at (0, -1.8) {
    ICL was \textbf{never explicitly trained for} --- it emerged from next-token prediction at scale.\newline
    {\scriptsize GPT-2 (1.5B): weak ICL. GPT-3 (175B): strong ICL. The gap is striking.}
  };

  % Bottom
  \node[font=\scriptsize, text=gray, text width=12cm, align=center] at (0, -3) {
    Brown et al.\ (2020) first demonstrated ICL systematically. It remains one of the\newline
    most consequential and least understood emergent abilities of large language models.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.88, transform shape]
  ...
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=6pt] at (0, 0.5) {
    ...
    \textbullet~Wei et al.\ (2022), ``Chain-of-Thought Prompting Elicits Reasoning in LLMs''\\[2pt]
    \textbullet~Xie et al.\ (ICLR 2022), ``An Explanation of In-context Learning as Implicit Bayesian Inference''
    }
  };

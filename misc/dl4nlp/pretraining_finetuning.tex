...
\begin{tikzpicture}
  % Embedding space
  \node[font=\small\bfseries, text=popblue] at (0, 3.5) {Learn by comparing: pull similar pairs together, push dissimilar apart};
  ...
  % Applications
  \node[draw=violet1, fill=violet1!8, rounded corners=4pt, text width=3.5cm, align=center, inner sep=5pt, font=\small] at (-4.5, -3) {
    \textbf{\textcolor{violet1}{SimCSE}}\\[2pt]
    {\scriptsize Sentence embeddings\\(same input, different dropout)}
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=4pt, text width=3.5cm, align=center, inner sep=5pt, font=\small] at (0, -3) {
    \textbf{\textcolor{orange1}{CLIP}}\\[2pt]
    {\scriptsize Image--text alignment\\(match image to caption)}
  };
  \node[draw=popblue, fill=popblue!8, rounded corners=4pt, text width=3.5cm, align=center, inner sep=5pt, font=\small] at (4.5, -3) {
...
\begin{tikzpicture}
  ...
  % Key insight
  \node[draw=warnred, fill=warnred!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -2.8) {
...
\begin{tikzpicture}
  % Prompt
  \node[draw, rounded corners=6pt, fill=lightbg, minimum width=3cm, font=\small] (prompt) at (-5.5, 2.5) {Prompt $x$};

  % Model generates responses
  \node[draw, rounded corners=6pt, fill=popblue!15, minimum width=2.5cm, font=\small] (model) at (-2.5, 2.5) {SFT Model};
  \draw[-Stealth, thick] (prompt) -- (model);

  % Multiple responses
  \node[draw, rounded corners=3pt, fill=lightbg, font=\scriptsize, text width=2cm, align=center] (y1) at (0.5, 3.3) {Response A};
  \node[draw, rounded corners=3pt, fill=lightbg, font=\scriptsize, text width=2cm, align=center] (y2) at (0.5, 2.5) {Response B};
  \node[draw, rounded corners=3pt, fill=lightbg, font=\scriptsize, text width=2cm, align=center] (y3) at (0.5, 1.7) {Response C};
  \draw[-Stealth, thick] (model.east) -- (y1.west);
  \draw[-Stealth, thick] (model.east) -- (y2.west);
  \draw[-Stealth, thick] (model.east) -- (y3.west);

  % Human ranking
  \node[draw, rounded corners=6pt, fill=orange1!15, minimum width=2.5cm, font=\small, text width=2.5cm, align=center] (human) at (3.5, 2.5) {Human ranks:\\B $>$ A $>$ C};
  \draw[-Stealth, thick] (1.7, 2.5) -- (human.west);

  % Reward model
  \node[draw, rounded corners=6pt, fill=sampred!15, minimum width=3cm, font=\small, text width=2.8cm, align=center] (rm) at (6.5, 2.5) {Reward Model\\$R_\phi(x, y) \to \mathbb{R}$};
  \draw[-Stealth, thick] (human.east) -- (rm.west);

  % Bradley-Terry
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, inner sep=8pt, text width=10cm, align=center] at (0, 0.2) {
    {\normalsize Bradley-Terry model: $P(y_w \succ y_l) = \sigma\!\big(R_\phi(x, y_w) - R_\phi(x, y_l)\big)$}\\[4pt]
    {\small Train to predict which response humans prefer}
  };

  % Details
  \node[draw=orange1, fill=orange1!8, rounded corners=4pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (-3.5, -1.5) {
    \textbf{Pairwise preferences}\\[2pt]
    Rankings are less noisy than absolute scores
  };
  \node[draw=violet1, fill=violet1!8, rounded corners=4pt, text width=5cm, align=center, inner sep=5pt, font=\small] at (3.5, -1.5) {
...
\begin{tikzpicture}
  % The PPO objective
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, inner sep=10pt, text width=12cm, align=center] at (0, 3) {
  ...
  % Typical epsilon
  \node[font=\scriptsize, text=gray] at (0, -2.8) {Typical: $\epsilon = 0.2$ \quad $\cdot$ \quad $\hat{A}_t$ computed via GAE (generalized advantage estimation)};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  % Problem statement
  \node[draw=warnred, fill=warnred!5, rounded corners=8pt, text width=12cm, align=center, inner sep=8pt, font=\small] at (0, 3.2) {
  ...
  % Per-token KL
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -2.2) {
    In practice, KL is computed \textbf{per token}: $\displaystyle D_{\text{KL}} = \sum_{t=1}^{T} \log \frac{\pi_\theta(y_t|x,y_{<t})}{\pi_{\text{ref}}(y_t|x,y_{<t})}$\\[4pt]
    {\scriptsize This keeps the policy close to the SFT model --- it can improve but not drift too far}
  };

  % Takeaway
  \node[font=\scriptsize, text=gray, text width=12cm, align=center] at (0, -3.3) {
...
\begin{tikzpicture}
  ...
  % DPO loss
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, inner sep=8pt, text width=12cm, align=center] at (0, 1.5) {
    {\small $\displaystyle \mathcal{L}_{\text{DPO}} = -\mathbb{E}\!\left[\log \sigma\!\left(\beta \log \frac{\pi_\theta(y_w|x)}{\pi_{\text{ref}}(y_w|x)} - \beta \log \frac{\pi_\theta(y_l|x)}{\pi_{\text{ref}}(y_l|x)}\right)\right]$}\\[6pt]
    {\scriptsize $y_w$ = preferred response, \quad $y_l$ = dispreferred, \quad $\pi_{\text{ref}}$ = frozen SFT model}
  };

  % Comparison
  \node[draw=warnred, fill=warnred!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.5, -0.5) {
    \textbf{\textcolor{warnred}{RLHF}}\\[3pt]
    3 stages\\4 models in memory\\Unstable RL training\\Complex hyperparameters
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.5, -0.5) {
    \textbf{\textcolor{paramgreen}{DPO}}\\[3pt]
  ...
  % Results
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -2.3) {
    DPO matches or exceeds PPO quality: 61\% win rate vs.\ PPO's 57\% on summarization
  };

  % Models
  \node[font=\small, text=gray] at (0, -3.2) {
    \textbf{Used by:} Zephyr, Tulu, many open-source models \quad |\quad \textbf{Mathematically equivalent to RLHF}
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  % Step 1
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12.5cm, align=left, inner sep=6pt, font=\small] at (0, 3.2) {
  ...
  % Step 2
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=12.5cm, align=left, inner sep=6pt, font=\small] at (0, 2.0) {
    \textbf{Step 2:} Closed-form optimal policy \quad $\displaystyle \pi^*(y|x) = \frac{1}{Z(x)}\, \pi_{\text{ref}}(y|x)\, \exp\!\Big(\frac{R(x,y)}{\beta}\Big)$
  };

  % Step 3
  \node[draw=orange1, fill=orange1!5, rounded corners=6pt, text width=12.5cm, align=left, inner sep=6pt, font=\small] at (0, 0.8) {
    \textbf{Step 3:} Rearrange to express reward via policy \quad $R(x,y) = \beta \log \dfrac{\pi^*(y|x)}{\pi_{\text{ref}}(y|x)} + \beta \log Z(x)$
  };

  % Step 4
  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=12.5cm, align=left, inner sep=6pt, font=\small] at (0, -0.4) {
  ...
  % Final result
  \node[draw=sampred, fill=sampred!8, rounded corners=8pt, text width=12.5cm, align=center, inner sep=8pt, font=\small] at (0, -2) {
  ...
  % Intuition
  \node[draw=popblue, fill=lightbg, rounded corners=6pt, text width=12.5cm, align=center, inner sep=6pt, font=\small] at (0, -3.5) {
...
\begin{tikzpicture}
  % Title
  \node[font=\small, text=gray] at (0, 3.5) {DeepSeek's innovation: eliminate the value network from PPO};
...
\begin{tikzpicture}
  % Full GRPO objective
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, inner sep=8pt, text width=12.5cm, align=center] at (0, 3) {
...
\begin{tikzpicture}
  ...
  % Why 8x
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=4.5cm, align=center, inner sep=6pt, font=\small] at (5, 2) {
  ...
  % Solution
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=4.5cm, align=center, inner sep=6pt, font=\small] at (5, -0.5) {
  ...
  \node[font=\scriptsize, text=gray] at (-2, -2.2) {A100 GPU = 80 GB \quad $\cdot$ \quad Consumer RTX 4090 = 24 GB};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.85]
  ...
  % ChatGPT
  \node[draw, rounded corners=4pt, fill=popblue!8, text width=11cm, align=left, inner sep=5pt, font=\scriptsize] at (0, -1.1) {
    \textbf{ChatGPT:} \quad Pre-train (GPT-3.5/4) $\to$ SFT (demonstrations) $\to$ RLHF (PPO + human preferences)
  };
  % Llama
  \node[draw, rounded corners=4pt, fill=paramgreen!8, text width=11cm, align=left, inner sep=5pt, font=\scriptsize] at (0, -1.9) {
    \textbf{Llama 2 Chat:} \quad Pre-train (2T tokens) $\to$ SFT (27.5k examples) $\to$ RLHF (iterative, 5 rounds)
  };
  % DeepSeek
  \node[draw, rounded corners=4pt, fill=orange1!8, text width=11cm, align=left, inner sep=5pt, font=\scriptsize] at (0, -2.7) {
    \textbf{DeepSeek-R1:} \quad Pre-train $\to$ Cold-start SFT $\to$ GRPO (rule-based rewards) $\to$ SFT on distilled data $\to$ GRPO again
  };
  % Zephyr
  \node[draw, rounded corners=4pt, fill=violet1!8, text width=11cm, align=left, inner sep=5pt, font=\scriptsize] at (0, -3.5) {
...
\begin{center}
\begin{tikzpicture}
  \node[font=\small\bfseries, text=popblue] at (0, 3.5) {Which method should I use?};

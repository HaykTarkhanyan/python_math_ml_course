...
% ============================================================
\section{Ranking \& retrieval metrics}
\begin{frame}
\frametitle{Ranking metrics: is the relevant stuff near the top?}
\vspace{-0.15cm}
{\small In search / retrieval / RAG the system returns a \textbf{ranked list}; we score \emph{where} the relevant items land. Say 3 relevant docs exist and the top 5 returned are:}

\vspace{0.2cm}
\begin{center}
\begin{tikzpicture}[font=\small]
  \foreach \i/\sym/\c in {1/{$\times$}/gray, 2/{\checkmark}/paramgreen, 3/{\checkmark}/paramgreen, 4/{$\times$}/gray, 5/{\checkmark}/paramgreen} {
    \node[draw, minimum size=0.8cm, rounded corners=3pt, fill=\c!15] at ({\i*1.3 - 3.9}, 0) {\textcolor{\c}{\sym}};
    \node[font=\scriptsize, text=gray] at ({\i*1.3 - 3.9}, -0.65) {rank \i};
  }
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[font=\small]
  \node[draw=popblue, fill=popblue!7, rounded corners=6pt, text width=3.9cm, align=left, inner sep=7pt] at (-4.2, 0) {
    \textbf{\textcolor{popblue}{Precision@k}}\\[2pt]
    rel in top-$k$ / $k$\\[3pt]
    P@3 $= 2/3$
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=3.9cm, align=left, inner sep=7pt] at (0, 0) {
    \textbf{\textcolor{paramgreen}{Recall@k}}\\[2pt]
    rel in top-$k$ / total rel\\[3pt]
    R@3 $= 2/3$
  };
  \node[draw=orange1, fill=orange1!9, rounded corners=6pt, text width=3.9cm, align=left, inner sep=7pt] at (4.2, 0) {
    \textbf{\textcolor{orange1}{MRR}}\\[2pt]
    mean of $1/(\text{rank of 1st rel})$\\[3pt]
    RR $= 1/2$
  };
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}
  \node[draw=gray, fill=gray!5, rounded corners=6pt, text width=12.6cm, align=center, inner sep=6pt, font=\small] {
    \textbf{Precision@k} / \textbf{Recall@k} ignore order \emph{within} the top-$k$. \textbf{MRR} rewards getting \emph{one} relevant hit high (great for QA with a single answer). For full-ordering quality, next slide.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Rank-aware: MAP and nDCG}
\vspace{-0.15cm}
{\small Same ranked list (relevant at ranks 2, 3, 5; 3 relevant total). These reward putting relevant items \emph{higher}.}

\vspace{0.25cm}
\begin{center}
\begin{tikzpicture}[font=\small]
  \node[draw=violet1, fill=violet1!7, rounded corners=6pt, text width=6.1cm, align=left, inner sep=8pt] at (-3.4, 0) {
    \textbf{\textcolor{violet1}{Average Precision (AP)}}\\[3pt]
    average of P@$k$ at each relevant rank:\\[3pt]
    $\text{AP} = \tfrac{1}{3}\!\left(\tfrac{1}{2} + \tfrac{2}{3} + \tfrac{3}{5}\right) \approx 0.59$\\[4pt]
    {\scriptsize \textbf{MAP} $=$ mean AP over all queries}
  };
  \node[draw=popblue, fill=popblue!7, rounded corners=6pt, text width=6.1cm, align=left, inner sep=8pt] at (3.4, 0) {
    \textbf{\textcolor{popblue}{nDCG}}\\[3pt]
    $\text{DCG} = \sum_i \dfrac{\text{rel}_i}{\log_2(i+1)}$, normalized by the ideal ranking:\\[3pt]
    $\text{nDCG} = \dfrac{1.52}{2.13} \approx 0.71$\\[3pt]
    {\scriptsize handles \textbf{graded} relevance, not just yes/no}
  };
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=12.6cm, align=center, inner sep=7pt, font=\small] {
    \textbf{nDCG} is the standard for graded relevance --- exactly what the RAG deck reports (\texttt{nDCG@10}). \textbf{MAP} is the standard binary-relevance summary. Both reward a good \emph{ordering}, not just a good set.
  };
\end{tikzpicture}
\end{center}
\end{frame}

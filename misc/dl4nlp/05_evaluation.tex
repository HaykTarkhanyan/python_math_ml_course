...
\begin{tikzpicture}
  ...
  \node[draw=violet1, fill=violet1!8, rounded corners=4pt, inner sep=6pt, font=\small, text width=5cm, align=center] at (0, -3) {
    Equivalently: $\text{PP} = \exp(H)$ where $H = -\frac{1}{N}\sum \log P(w_i \mid w_{<i})$
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{BERTScore --- embedding-level similarity}

\vspace{-0.2cm}
\begin{center}
\begin{tikzpicture}[scale=0.90, transform shape]
  % Citation
  \node[font=\small, text=gray] at (0, 3.5) {Zhang et al., ICLR 2020 --- uses contextual embeddings instead of $n$-gram matching};

  % Core idea
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, 2.4) {
    \textbf{Key idea:} embed each token with a pre-trained model (e.g., RoBERTa), then compute\\
    pairwise cosine similarities between reference and candidate tokens
  };

  % Three scores
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=6pt, font=\small] at (-4.5, 0.5) {
    \textbf{\textcolor{paramgreen}{Precision ($P$)}}\\[4pt]
    For each \emph{candidate} token,\\find max-similarity\\reference token.\\Average over candidate.
  };

  \node[draw=popblue, fill=popblue!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=6pt, font=\small] at (0, 0.5) {
    \textbf{\textcolor{popblue}{Recall ($R$)}}\\[4pt]
    For each \emph{reference} token,\\find max-similarity\\candidate token.\\Average over reference.
  };

  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=3.3cm, align=center, inner sep=6pt, font=\small] at (4.5, 0.5) {
    \textbf{\textcolor{orange1}{F1}}\\[4pt]
    $\displaystyle F_1 = 2\frac{P \cdot R}{P + R}$\\[6pt]
    {\scriptsize Most commonly reported}
  };

  % Advantages
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=5.3cm, align=left, inner sep=6pt, font=\small] at (-3.2, -2) {
    \textbf{\textcolor{paramgreen}{Strengths:}}\\[2pt]
    {\scriptsize \textbullet~Captures synonyms \& paraphrases\\
    \textbullet~Correlates better with human judgment\\
    \textbullet~Works across many languages\\
    \textbullet~No training needed (use off-the-shelf LM)}
  };

  \node[draw=warnred, fill=warnred!5, rounded corners=6pt, text width=5.3cm, align=left, inner sep=6pt, font=\small] at (3.2, -2) {
    \textbf{\textcolor{warnred}{Limitations:}}\\[2pt]
    {\scriptsize \textbullet~Depends on choice of embedding model\\
    \textbullet~Slower than BLEU/ROUGE (GPU needed)\\
...
% ============================================================
\begin{frame}
\frametitle{LLM-as-Judge --- using LLMs to evaluate LLMs}

\vspace{-0.2cm}
\begin{center}
\begin{tikzpicture}[scale=0.90, transform shape]
  % Citation
  \node[font=\small, text=gray] at (0, 3.5) {Zheng et al., 2023 --- now a dominant evaluation paradigm for open-ended generation};

  % Core idea
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, 2.4) {
    \textbf{Core idea:} use a strong LLM (e.g., GPT-4) as an automated judge to score or rank\\
    outputs from other models. \textbf{No reference text needed.}
  };

  % Two approaches
  \node[draw=popblue, fill=popblue!10, rounded corners=8pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (-3.2, 0.5) {
    \textbf{\textcolor{popblue}{MT-Bench}}\\[3pt]
    80 multi-turn questions\\across 8 categories\\[2pt]
    Judge LLM scores 1--10\\on helpfulness, relevance,\\accuracy, creativity\\[2pt]
    {\scriptsize Single model evaluation}
  };

  \node[draw=sampred, fill=sampred!10, rounded corners=8pt, text width=5cm, align=center, inner sep=6pt, font=\small] at (3.2, 0.5) {
    \textbf{\textcolor{sampred}{Chatbot Arena (LMSYS)}}\\[3pt]
    Crowdsourced blind A/B tests:\\humans pick preferred response\\[2pt]
    Elo ratings from 1M+ votes\\(like chess rankings)\\[2pt]
    {\scriptsize Gold standard for chat models}
  };

  % Biases and mitigations
  \node[draw=warnred, fill=warnred!5, rounded corners=6pt, text width=5.3cm, align=left, inner sep=5pt, font=\small] at (-3.2, -2) {
    \textbf{\textcolor{warnred}{Known biases:}}\\[2pt]
    {\scriptsize \textbullet~Position bias (prefers first response)\\
    \textbullet~Verbosity bias (longer = ``better'')\\
    \textbullet~Self-enhancement (favors own outputs)\\
    \textbullet~Style over substance}
  };

  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=5.3cm, align=left, inner sep=5pt, font=\small] at (3.2, -2) {
    \textbf{\textcolor{paramgreen}{Why it dominates:}}\\[2pt]
    {\scriptsize \textbullet~Scales to any open-ended task\\
...
\begin{tabular}{>{\bfseries}l l l l}
  ...
  \textcolor{paramgreen}{Winogrande} & Coreference & Pronoun resolution & Linguistic understanding \\[2pt]
  \textcolor{warnred}{MMLU-Pro} & Knowledge (harder) & Multiple choice (10-way) & Harder MMLU + reasoning \\[2pt]
  \textcolor{popblue}{GPQA} & Graduate-level science & Multiple choice & PhD-level physics/chem/bio \\[2pt]
  \textcolor{orange1}{SWE-Bench} & Coding agents & Real GitHub issues & End-to-end software eng. \\[2pt]

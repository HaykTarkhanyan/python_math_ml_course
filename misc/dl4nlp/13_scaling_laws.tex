...
% ============================================================
\begin{frame}
\frametitle{The other budget: training memory}
\vspace{-0.15cm}
{\small Compute is $C \approx 6ND$; \textbf{memory} is the other wall. Training one parameter with mixed-precision Adam costs $\approx$\,16 bytes, not 2:}

\vspace{0.25cm}
\begin{center}
\begin{tikzpicture}[font=\small, b/.style={draw, rounded corners=5pt, inner sep=5pt, align=center, minimum height=1.15cm}]
  \node[b, fill=popblue!12, text width=1.9cm] (w) at (-5.4,0) {Weights\\fp16\\\textbf{2 B}};
  \node[b, fill=paramgreen!12, text width=1.9cm] (g) at (-3.3,0) {Gradients\\fp16\\\textbf{2 B}};
  \node[b, fill=orange1!14, text width=4.7cm] (o) at (0.1,0) {Adam optimizer states (fp32)\\master 4 B $+$ momentum 4 B $+$ variance 4 B\\\textbf{12 B}};
  \node[b, fill=sampred!14, text width=1.7cm] (t) at (4.9,0) {\textbf{$\approx$ 16 B}\\per param};
  \draw[-Stealth, thick] (o.east) -- (t.west);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[font=\small]
  \node[draw=popblue, fill=lightbg, rounded corners=6pt, text width=6.1cm, align=left, inner sep=7pt] at (-3.4,0) {
    \textbf{\textcolor{popblue}{Example: a 7B model}}\\[2pt]
    $16 \times 7\text{B} \approx \mathbf{112}$ GB just for weights $+$ grads $+$ optimizer --- before \textbf{activations}. One 80\,GB GPU can't hold it.
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=6.1cm, align=left, inner sep=7pt] at (3.4,0) {
    \textbf{\textcolor{paramgreen}{So we spread it out}}\\[2pt]
    activation checkpointing, plus sharding across GPUs (ZeRO / tensor \& pipeline parallelism) split these states.
  };
\end{tikzpicture}
\end{center}

\vspace{0.15cm}
\begin{center}{\footnotesize\textcolor{gray}{\textbf{Mixed precision:} compute in fp16/bf16 for speed, keep an fp32 master copy of the weights for numerical stability.}}\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt, font=\small] at (0, -2.3) {
    \textbf{OpenAI o1 / DeepSeek-R1 (2024--25):} scale inference compute via hidden chain-of-thought.\\
    o1 on AIME: 15.6\% $\to$ \textbf{83.3\%}. See \textbf{Slide 15} for deep dive on test-time compute.
  };

  \node[font=\scriptsize, text=gray] at (0, -3.1) {
    Four axes of scaling: pre-training compute, data quality, inference compute, post-training (RL/RLHF)
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node at (0, 1) {
    ...
    \begin{tabular}{l r r r l}
      ...
      LLaMA-3.1 405B (2024) & 405B & 15.6T & 39 & 2$\times$ \textcolor{popblue}{over-trained} \\
    \end{tabular}
    }
  };
  ...
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -1.3) {
    \textbf{Key insight:} LLaMA-1 7B (1T tokens) matched \textbf{GPT-3 175B} on many benchmarks.\\
    \textbf{LLaMA-3.1 405B} (Meta, 2024): deliberately trained well beyond Chinchilla-optimal for inference efficiency.
  };
...
\begin{tikzpicture}
  ...
  \fill[orange1] (5.5, 0) circle (3pt);
  \node[font=\tiny, text=orange1] at (5.5, 0.3) {LLaMA-3.1 405B};
...
\begin{tikzpicture}
  ...
  \node[draw=sampred, fill=sampred!8, rounded corners=6pt, text width=12cm, align=left, inner sep=6pt, font=\small] at (0, -3.1) {
    \textbf{5.\ Inference-time compute is the new frontier} (see Slide 15).\\
    \hspace{0.5cm}{\scriptsize Smaller model + more thinking at test time can beat 14$\times$ larger model (Snell 2024, o1, R1).}
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.88, transform shape]
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 2.5) {
    ...
    \textbullet~Muennighoff et al.\ (2024), ``Scaling Data-Constrained Language Models''\\[2pt]
    \textbullet~Dubey et al.\ (2024), ``The Llama 3 Herd of Models'' (over-training for inference efficiency)
    }
  };

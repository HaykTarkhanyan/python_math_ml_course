...
% ============================================================
\begin{frame}{Late Interaction: ColBERT}
\begin{center}
\begin{tikzpicture}[scale=0.88, transform shape,
  box/.style={draw, rounded corners=3pt, font=\small, inner sep=4pt, align=center},
]
% Three paradigms
\node[font=\small\bfseries, popblue] at (0, 3.2) {Three retrieval paradigms};

% Bi-encoder
\node[box, fill=popblue!10, text width=3.2cm, minimum height=2.2cm] at (-4.5, 1.5)
{\textbf{\color{popblue}Bi-encoder}\\[3pt]
{\scriptsize Single vector per doc\\$\cos(\mathbf{q}, \mathbf{d})$\\[2pt]
Fast, but loses\\token-level detail}};

% Cross-encoder
\node[box, fill=orange1!10, text width=3.2cm, minimum height=2.2cm] at (0, 1.5)
{\textbf{\color{orange1}Cross-encoder}\\[3pt]
{\scriptsize Joint $(q, d)$ through BERT\\Full interaction\\[2pt]
Accurate, but $O(n)$\\can't pre-compute}};

% ColBERT
\node[box, fill=paramgreen!10, text width=3.2cm, minimum height=2.2cm] at (4.5, 1.5)
{\textbf{\color{paramgreen}Late Interaction}\\(ColBERT)\\[3pt]
{\scriptsize Per-token embeddings\\MaxSim matching\\[2pt]
Best of both worlds}};

% MaxSim formula
\node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=10cm, align=center, inner sep=6pt, font=\small] at (0, -0.6) {
  \textbf{ColBERT scoring (MaxSim):}\\[4pt]
  {\normalsize $S(q, d) = \displaystyle\sum_{i=1}^{|q|} \max_{j=1}^{|d|} \; \mathbf{q}_i^\top \mathbf{d}_j$}\\[4pt]
  {\scriptsize Each query token finds its best-matching document token --- captures fine-grained similarity}
};

% Advantage
\node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=10cm, align=center, inner sep=5pt, font=\small] at (0, -2.2) {
  Document token embeddings can be \textbf{pre-computed and indexed}\\
  $\Rightarrow$ scalable like bi-encoder, expressive like cross-encoder
};
...
% GRAPHRAG
% ============================================================
\begin{frame}{GraphRAG (Edge et al., 2024)}
\begin{center}
\begin{tikzpicture}[scale=0.88, transform shape,
  box/.style={draw, rounded corners=3pt, minimum height=0.7cm, font=\small,
              inner sep=4pt, align=center},
...
% Problem
\node[font=\small, text width=13cm, align=center] at (6, 3)
{\textbf{Problem}: standard RAG fails on \textbf{\color{warnred}global queries} (``What are the main themes across all documents?'')};

% Pipeline
\node[box, fill=popblue!10, text width=2cm] (docs) at (0, 1.3) {Documents};
\node[box, fill=orange1!10, text width=2.2cm] (extract) at (3, 1.3) {LLM extracts\\entities \&\\relations};
\node[box, fill=violet1!10, text width=2.2cm] (kg) at (6, 1.3) {Knowledge\\graph};
\node[box, fill=paramgreen!10, text width=2.2cm] (comm) at (9, 1.3) {Community\\detection\\{\scriptsize (Leiden)}};
\node[box, fill=sampred!10, text width=2.2cm] (summ) at (12, 1.3) {Community\\summaries};

\draw[-{Stealth}, thick, popblue] (docs) -- (extract);
\draw[-{Stealth}, thick, popblue] (extract) -- (kg);
\draw[-{Stealth}, thick, popblue] (kg) -- (comm);
\draw[-{Stealth}, thick, popblue] (comm) -- (summ);

% Two query modes
\node[draw, rounded corners=3pt, fill=popblue!8, inner sep=4pt, text width=4.8cm,
      align=left, font=\scriptsize] at (2.5, -1.2)
{\textbf{Local search:}\\[2pt]
Entity $\to$ neighbors $\to$ community context\\
Good for specific factual questions\\
Similar to standard RAG but graph-guided};

\node[draw, rounded corners=3pt, fill=orange1!8, inner sep=4pt, text width=4.8cm,
      align=left, font=\scriptsize] at (9, -1.2)
{\textbf{Global search:}\\[2pt]
Map community summaries $\to$ partial answers\\
Reduce into final synthesized answer\\
Handles ``what are the themes'' queries};

% Bottom
\node[draw, rounded corners=3pt, fill=paramgreen!10, inner sep=4pt, text width=11cm,
      align=center, font=\small] at (6, -2.8)
{Structures corpus knowledge as a graph $\Rightarrow$ enables \textbf{multi-hop reasoning}\\and \textbf{global summarization} that flat vector search cannot do};
\end{tikzpicture}
\end{center}
\end{frame}
...
% Idea
\node[font=\small, text width=13cm, align=center] at (6, 3)
{\textbf{Key idea}: the LLM acts as an \textbf{agent} that decides \emph{when}, \emph{what}, and \emph{how} to retrieve};

% Agent loop
\node[box, fill=popblue!10, text width=2cm] (q) at (0, 1.2) {User\\query};
\node[box, fill=orange1!10, text width=2.5cm] (plan) at (3.5, 1.2) {LLM plans\\{\scriptsize decompose, route}};
\node[box, fill=violet1!10, text width=2cm] (tool) at (7, 1.2) {Tool call\\{\scriptsize search, SQL,\\API, web}};
\node[box, fill=paramgreen!10, text width=2cm] (eval) at (10, 1.2) {Evaluate\\{\scriptsize sufficient?}};

\draw[-{Stealth}, thick, popblue] (q) -- (plan);
\draw[-{Stealth}, thick, popblue] (plan) -- (tool);
\draw[-{Stealth}, thick, popblue] (tool) -- (eval);

% Loop back
\draw[-{Stealth}, thick, sampred] (eval.south) -- ++(0, -0.8) -| (plan.south)
     node[pos=0.25, below, font=\scriptsize, sampred] {not enough $\to$ re-plan \& retrieve again};

% Final answer
\node[box, fill=sampred!10, text width=2cm] (ans) at (13, 1.2) {Final\\answer};
\draw[-{Stealth}, thick, paramgreen] (eval) -- (ans)
     node[midway, above, font=\scriptsize, paramgreen] {done};

% Capabilities
\node[draw, rounded corners=3pt, fill=lightbg, inner sep=4pt, text width=3.4cm,
      align=center, font=\scriptsize] at (1.5, -1.8)
{\textbf{Query decomposition}\\[2pt]
Break complex questions\\into sub-queries};

\node[draw, rounded corners=3pt, fill=lightbg, inner sep=4pt, text width=3.4cm,
      align=center, font=\scriptsize] at (6, -1.8)
{\textbf{Tool selection}\\[2pt]
Vector DB, web search,\\SQL, calculator, API};

\node[draw, rounded corners=3pt, fill=lightbg, inner sep=4pt, text width=3.4cm,
      align=center, font=\scriptsize] at (10.5, -1.8)
{\textbf{Iterative refinement}\\[2pt]
Retrieve, reflect, re-query\\until answer is satisfactory};

% Bottom
\node[font=\scriptsize, gray, text width=11cm, align=center] at (6, -3.2)
{Frameworks: LangGraph, LlamaIndex Agents, CrewAI $\;\cdot\;$ The frontier of RAG research};
\end{tikzpicture}
\end{center}
\end{frame}
...
\node[evt, fill=sampred!12] at (11, 1.3) {CRAG, GraphRAG\\corrective + graph};
...
\node[evt, fill=paramgreen!12] at (11, -1.3) {ColBERT v2\\ColPali};
...
\begin{tikzpicture}[scale=0.88, transform shape]
  ...
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 0.3) {
    ...
    \textbullet~Edge et al.\ (2024), ``From Local to Global: A Graph RAG Approach to Query-Focused Summarization''\\[2pt]
  ...
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -1.7) {
    \textbf{\textcolor{orange1}{Retrieval \& Evaluation}}\\[4pt]
    {\small
    \textbullet~Khattab \& Zaharia (2020), ``ColBERT: Efficient and Effective Passage Search via Late Interaction''\\[2pt]

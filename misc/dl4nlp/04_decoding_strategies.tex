...
\subtitle{Greedy $\cdot$ Beam Search $\cdot$ Temperature $\cdot$ Top-$k$ $\cdot$ Top-$p$ $\cdot$ Min-$p$ $\cdot$ Constrained}
\date{}
...
\begin{center}
\begin{tikzpicture}[
  cbox/.style={draw, rounded corners=6pt, minimum width=5.5cm, minimum height=3.5cm, text width=5cm, align=left, inner sep=8pt}
]
  % The idea
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, text width=12cm, align=center, inner sep=10pt, font=\normalsize] at (0, 2.8) {
    \textbf{Idea:} At each decoding step, \textbf{mask out} tokens that would violate a grammar or schema.\\[2pt]
    The LLM's output is \textit{guaranteed} to conform to the desired structure.
  };

  % JSON mode
  \node[cbox, fill=paramgreen!8] at (-4, -0.6) {
    \textbf{\large\textcolor{paramgreen}{JSON mode}}\\[6pt]
    \small
    Force output to be valid JSON\\[2pt]
    \textbullet\ At each step, only allow tokens\\
    \quad that keep the JSON well-formed\\[2pt]
    \textbullet\ OpenAI, Anthropic, vLLM all\\
    \quad support this natively\\[2pt]
    \textbullet\ Critical for tool use / function\\
    \quad calling in LLM agents
  };

  % Grammar-guided
  \node[cbox, fill=orange1!8] at (4, -0.6) {
    \textbf{\large\textcolor{orange1}{Grammar-guided}}\\[6pt]
    \small
    Define a CFG or regex; decode\\
    only tokens matching it\\[2pt]
    \textbullet\ \textbf{Outlines} (2024): compile any\\
    \quad regex/JSON schema to a mask\\[2pt]
    \textbullet\ \textbf{Guidance} (Microsoft): interleave\\
    \quad free generation \& constrained\\[2pt]
    \textbullet\ Used for SQL, code, configs
  };

  % Bottom note
  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=12cm, align=center, inner sep=6pt, font=\small] at (0, -3.4) {
    Structured decoding is orthogonal to sampling strategy --- you can combine it with\\
    temperature, top-$p$, min-$p$, etc.\ It only restricts \textit{which} tokens are candidates.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, text width=11cm, align=center, inner sep=10pt, font=\small] {
    \textbf{In practice}, most LLM APIs use \textbf{temperature + top-$p$} (min-$p$ gaining adoption).\\
    Beam search is reserved for structured tasks (translation, summarization).\\
    Greedy ($T{=}0$) for deterministic output; \textbf{constrained decoding} for structured formats.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.88, transform shape]
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 2.5) {
    \textbf{\textcolor{popblue}{Decoding Methods}}\\[4pt]
    {\small
    \textbullet~Holtzman et al.\ (2020), ``The Curious Case of Neural Text Degeneration'' (nucleus/top-$p$ sampling)\\[2pt]
    \textbullet~Fan et al.\ (2018), ``Hierarchical Neural Story Generation'' (top-$k$ sampling)\\[2pt]
    \textbullet~Vijayakumar et al.\ (2018), ``Diverse Beam Search'' (diverse decoding)
    }
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 0.3) {
    \textbf{\textcolor{paramgreen}{Analysis \& Improvements}}\\[4pt]
    {\small
    \textbullet~Meister et al.\ (2020), ``If Beam Search is the Answer, What was the Question?''\\[2pt]
    \textbullet~Su et al.\ (2022), ``A Contrastive Framework for Neural Text Generation'' (contrastive search)\\[2pt]
    \textbullet~Nguyen et al.\ (2024), ``Turning up the Heat: Min-p Sampling'' (min-$p$)
    }
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -1.9) {
    \textbf{\textcolor{orange1}{Practical Guide \& Constrained Decoding}}\\[4pt]
    {\small
    \textbullet~HuggingFace blog, ``How to generate text: using different decoding methods''\\[2pt]
    \textbullet~Willard \& Louf (2023), ``Efficient Guided Generation for Large Language Models'' (Outlines)\\[2pt]
    \textbullet~Jurafsky \& Martin, \emph{Speech and Language Processing}, Ch.\,10 --- Language Model Decoding
    }
  };
\end{tikzpicture}
\end{center}

...
\begin{tikzpicture}
  ...
  \node[draw=popblue, fill=popblue!5, rounded corners=8pt, text width=11cm, align=center, inner sep=6pt, font=\small] at (0, -0.8) {
    \textbf{Expert parallelism (EP):} each GPU holds a subset of experts.\\
    Tokens dispatched via \textbf{all-to-all} collective to the GPU hosting the chosen expert.\\[3pt]
    Combined with \textbf{data parallelism} (replicate non-expert params) $\cdot$
    \textbf{tensor parallelism} (shard within experts) $\cdot$ \textbf{pipeline parallelism} (across layers).
  };

  % Frameworks
  \node[font=\scriptsize, text=gray, text width=10cm, align=center] at (0, -2.5) {
    Frameworks: DeepSpeed-MoE, Tutel, FasterMoE, Megablocks (dropless MoE) $\cdot$ DeepSeek-V3: EP + DualPipe overlap
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=8pt, text width=11cm, align=center, inner sep=6pt, font=\small] at (0, -1.2) {
    \textbf{Training:} 2.788M H800 GPU hours $\approx$ \textbf{\$5.5M} on \textbf{14.8T tokens}\\[3pt]
    {\scriptsize Compare: LLaMA3-405B estimated at \$60--100M. GPT-4 estimated at \$100M+.}\\[2pt]
    {\scriptsize MoE + engineering innovations (FP8 mixed precision, DualPipe) $\Rightarrow$ 10--20x cost reduction.}
  };
...
\begin{tikzpicture}[scale=0.88, transform shape]
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 2.5) {
    ...
    {\small
    \textbullet~Shazeer et al.\ (2017), ``Outrageously Large Neural Networks: The Sparsely-Gated Mixture-of-Experts Layer''\\[2pt]
    \textbullet~Fedus et al.\ (2022), ``Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity''
    }
  };
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, 0.3) {
    \textbf{\textcolor{paramgreen}{Modern MoE Models}}\\[4pt]
    {\small
    \textbullet~Jiang et al.\ (2024), ``Mixtral of Experts'' (Mixtral 8x7B)\\[2pt]
    \textbullet~DeepSeek-AI (2024), ``DeepSeek-V2: A Strong, Economical, and Efficient MoE Language Model''\\[2pt]
    \textbullet~Dai et al.\ (2024), ``DeepSeekMoE: Towards Ultimate Expert Specialization''
    }
  };
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=8pt] at (0, -1.8) {
    ...
    {\small
    \textbullet~Zoph (2022), ``Designing Effective Sparse Expert Models'' (Google, ST-MoE)\\[2pt]
    \textbullet~Cai et al.\ (2024), ``A Survey on Mixture of Experts''
    }
  };
\end{tikzpicture}
\end{center}

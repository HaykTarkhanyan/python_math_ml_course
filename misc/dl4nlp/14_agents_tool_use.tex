...
\begin{tikzpicture}
  ...
  \node[font=\scriptsize, text=gray, text width=10cm, align=center] at (0, -2.6) {
    Also: \textbf{Cursor} (AI-native IDE, inline agent + multi-file edits), \textbf{GitHub Copilot Coding Agent} (May 2025).
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=11cm, align=center, inner sep=5pt, font=\small] at (0, -1.8) {
    ...
    {\scriptsize Agents interact with \textbf{any GUI} --- browsers, desktops, mobile apps --- via screenshots + actions.\\
    Also: open-source \textbf{Browser Use} framework (2024) for web automation with LLM agents.}
  };
\end{tikzpicture}
\end{center}
\end{frame}

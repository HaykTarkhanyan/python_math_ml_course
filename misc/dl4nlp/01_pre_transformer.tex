...
% ============================================================
\section{The NLP task landscape}
\begin{frame}
\frametitle{What NLP tasks look like}
\vspace{-0.2cm}
\begin{center}
\begin{tikzpicture}[scale=0.9, transform shape,
  tbox/.style={draw, rounded corners=6pt, text width=5.5cm, align=left, inner sep=7pt, font=\small}
]
  \node[tbox, fill=popblue!8, draw=popblue] (a) at (-3.4, 1.7) {
    \textbf{\textcolor{popblue}{Token labeling}}\\[3pt]
    one label per word\\
    {\scriptsize POS tagging, NER, chunking}
  };
  \node[tbox, fill=paramgreen!8, draw=paramgreen] (b) at (3.4, 1.7) {
    \textbf{\textcolor{paramgreen}{Sequence classification}}\\[3pt]
    one label per text\\
    {\scriptsize sentiment, topic, NLI, spam}
  };
  \node[tbox, fill=violet1!8, draw=violet1] (c) at (-3.4, -1.2) {
    \textbf{\textcolor{violet1}{Structured prediction}}\\[3pt]
    predict a structure\\
    {\scriptsize dependency/constituency parsing, coreference resolution}
  };
  \node[tbox, fill=orange1!10, draw=orange1] (d) at (3.4, -1.2) {
    \textbf{\textcolor{orange1}{Generation (seq2seq)}}\\[3pt]
    text in $\to$ text out\\
    {\scriptsize translation, summarization, QA, dialogue}
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, text width=12.6cm, align=center, inner sep=7pt, font=\small] {
    Classically each task had its own model and output format. Modern LLMs \textbf{unify} them: cast everything as text-to-text (``translate:\,\ldots'', ``sentiment:\,\ldots'') and one model does all of it.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Sequence labeling: the BIO tagging scheme}
\vspace{-0.15cm}
{\small Token-labeling tasks (NER, chunking) tag \emph{spans}. The \textbf{BIO} scheme marks each token as \textbf{B}egin, \textbf{I}nside, or \textbf{O}utside a span --- turning span-finding into per-token classification.}

\vspace{0.3cm}
\begin{center}
\begin{tikzpicture}[scale=0.95, transform shape, font=\small]
  \node (t1) at (-5.2, 1) {Aram};
  \node (t2) at (-3.4, 1) {visited};
  \node (t3) at (-1.6, 1) {New};
  \node (t4) at (0.2, 1) {York};
  \node (t5) at (2.0, 1) {in};
  \node (t6) at (3.4, 1) {April};
  \node[draw=sampred, fill=sampred!12, rounded corners=3pt, inner sep=3pt] at (-5.2, 0) {B-PER};
  \node[draw=gray, fill=gray!8, rounded corners=3pt, inner sep=3pt] at (-3.4, 0) {O};
  \node[draw=popblue, fill=popblue!12, rounded corners=3pt, inner sep=3pt] at (-1.6, 0) {B-LOC};
  \node[draw=popblue, fill=popblue!12, rounded corners=3pt, inner sep=3pt] at (0.2, 0) {I-LOC};
  \node[draw=gray, fill=gray!8, rounded corners=3pt, inner sep=3pt] at (2.0, 0) {O};
  \node[draw=paramgreen, fill=paramgreen!12, rounded corners=3pt, inner sep=3pt] at (3.4, 0) {B-DATE};
  \foreach \x in {-5.2,-3.4,-1.6,0.2,2.0,3.4} { \draw[-Stealth, gray] (\x, 0.72) -- (\x, 0.32); }
  \draw[decorate, decoration={brace, amplitude=4pt, mirror}, thick, popblue] (-2.15, -0.5) -- (0.75, -0.5);
  \node[font=\scriptsize, text=popblue] at (-0.7, -0.98) {one LOC span: ``New York''};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
  \node[draw=paramgreen, fill=paramgreen!7, rounded corners=6pt, text width=12.6cm, align=center, inner sep=7pt, font=\small] {
    \textbf{B-} starts a new entity, \textbf{I-} continues it, \textbf{O} is outside any entity. The tag set is $2k{+}1$ tags for $k$ entity types. POS tagging is the same idea with one tag per token (no spans).
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  % Bigram window (n=2: conditions on the 1 preceding word "the")
  \draw[thick, sampred, rounded corners=3pt] (0.45, 0.6) rectangle (1.55, 1.4);
  \node[font=\scriptsize, text=sampred] at (1.0, 1.75) {bigram context};

  % Trigram window (n=3: conditions on the 2 preceding words "on the")
  \draw[thick, paramgreen, rounded corners=3pt, dashed] (-1.15, 0.5) rectangle (1.65, 1.5);
  \node[font=\scriptsize, text=paramgreen] at (-0.1, 0.15) {trigram context};
...
% ============================================================
% ============================================================
% NEURAL PROBABILISTIC LM (BENGIO 2003)
% ============================================================
\begin{frame}
\frametitle{The first neural LM (Bengio et al., 2003)}
\vspace{-0.15cm}
{\small One-hot vectors are discrete, so n-grams can't generalize. Bengio's \textbf{Neural Probabilistic Language Model} was the first fix: feed the previous words through \emph{learned} dense embeddings and a neural net.}

\vspace{0.25cm}
\begin{center}
\begin{tikzpicture}[scale=0.9, transform shape, font=\small,
  b/.style={draw, rounded corners=4pt, inner sep=5pt}]
  \node[b, fill=popblue!10] (w1) at (-6, 0.8) {$w_{t-3}$};
  \node[b, fill=popblue!10] (w2) at (-6, 0) {$w_{t-2}$};
  \node[b, fill=popblue!10] (w3) at (-6, -0.8) {$w_{t-1}$};
  \node[b, fill=paramgreen!15, text width=1.6cm, align=center] (emb) at (-3.7, 0) {embed $C$\\{\scriptsize (shared)}};
  \draw[-Stealth] (w1) -- (emb); \draw[-Stealth] (w2) -- (emb); \draw[-Stealth] (w3) -- (emb);
  \node[b, fill=paramgreen!8, text width=1.4cm, align=center] (cat) at (-1.5, 0) {concat};
  \draw[-Stealth] (emb) -- (cat);
  \node[b, fill=orange1!12, text width=1.5cm, align=center] (hid) at (0.6, 0) {tanh\\hidden};
  \draw[-Stealth] (cat) -- (hid);
  \node[b, fill=sampred!12, text width=1.7cm, align=center] (sm) at (2.9, 0) {softmax\\over $V$};
  \draw[-Stealth] (hid) -- (sm);
  \node[b, fill=popblue!12] (out) at (5.3, 0) {$P(w_t \mid \text{ctx})$};
  \draw[-Stealth] (sm) -- (out);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[font=\small]
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=6.1cm, align=left, inner sep=7pt] at (-3.3, 0) {
    \textbf{\textcolor{paramgreen}{Fixes generalization}}\\[2pt]
    ``cat'' and ``dog'' get similar vectors, so seeing one helps predict after the other. The embeddings $C$ are learned by the LM.
  };
  \node[draw=warnred, fill=warnred!8, rounded corners=6pt, text width=6.1cm, align=left, inner sep=7pt] at (3.3, 0) {
    \textbf{\textcolor{warnred}{Still a fixed window}}\\[2pt]
    It only sees $n{-}1$ previous words, like n-grams. Removing that limit is what motivates RNNs.
  };
\end{tikzpicture}
\end{center}

\vspace{0.12cm}
\begin{center}{\footnotesize\textcolor{gray}{Word2Vec (2013) later keeps just the embeddings --- throwing away the LM --- to learn word vectors cheaply.}}\end{center}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, inner sep=8pt, text width=10cm, align=center, font=\small] at (0, -0.8) {
    $\displaystyle \frac{\partial \mathcal{L}}{\partial h_1} = \frac{\partial \mathcal{L}}{\partial h_T} \cdot \prod_{t=2}^{T} \frac{\partial h_t}{\partial h_{t-1}}$ \\[6pt] Product of many values $<1$ $\Rightarrow$ gradient vanishes
  };
...
% ============================================================
\begin{frame}
\frametitle{The other failure mode: exploding gradients}
\vspace{-0.15cm}
{\small The same product of Jacobians that can \emph{vanish} can also \emph{explode}: if the factors are $>1$, the gradient grows exponentially through time --- loss jumps to \texttt{NaN} and training destabilizes.}

\vspace{0.25cm}
\begin{center}
\begin{tikzpicture}[scale=0.95, transform shape, font=\small]
  \draw[thick, -Stealth] (-5.6, -1.2) -- (-5.6, 2.2) node[above, font=\scriptsize] {$\|g\|$};
  \draw[thick, -Stealth] (-5.6, -1.2) -- (-0.6, -1.2) node[right, font=\scriptsize] {training step};
  \draw[very thick, sampred] (-5.4,-0.9) -- (-4.7,-0.6) -- (-4.1,-0.8) -- (-3.5,1.9) -- (-3.45,-0.7) -- (-2.7,-0.5) -- (-2.1,-0.7) -- (-1.5,1.6) -- (-1.45,-0.6) -- (-0.9,-0.5);
  \node[font=\scriptsize, text=sampred] at (-3.5, 2.15) {spike};
  \draw[dashed, thick, popblue] (-5.6, 0.4) -- (-0.8, 0.4);
  \node[font=\scriptsize, text=popblue, anchor=west] at (-5.4, 0.68) {threshold $\tau$};

  \node[draw=popblue, fill=popblue!6, rounded corners=6pt, text width=6.2cm, align=center, inner sep=8pt] at (3.3, 0.5) {
    \textbf{\textcolor{popblue}{Gradient clipping}}\\[5pt]
    if $\|g\| > \tau$:\quad $g \leftarrow \tau \cdot \dfrac{g}{\|g\|}$\\[5pt]
    {\scriptsize rescale the gradient back to norm $\tau$; direction is kept}
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
  \node[draw=paramgreen, fill=paramgreen!7, rounded corners=6pt, text width=12.6cm, align=center, inner sep=7pt, font=\small] {
    \textbf{Vanishing} needs an \emph{architectural} fix (LSTM gates, next). \textbf{Exploding} has a cheap \emph{optimization} fix --- clip the norm. Both come from the same repeated matrix multiplication in backprop-through-time.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.95]
  ...
  % Output gate (gates tanh of the cell state to produce h_t)
  \node[draw, circle, fill=orange1!20, minimum size=0.9cm, font=\small] (og) at (3, 1.5) {$\times$};
  \draw[-Stealth, thick, orange1] (3, 2.7) -- (og.north);
  \node[font=\tiny, text=orange1, anchor=west] at (3.15, 2.3) {$\tanh(c_t)$};
  \node[draw, rounded corners=3pt, fill=orange1!10, font=\scriptsize, text width=1.3cm, align=center] (ogbox) at (5.0, 1.5) {Output gate $o_t$};
  \draw[-Stealth, thick, orange1] (ogbox.west) -- (og.east);

  % Hidden state output
  \draw[-Stealth, very thick, orange1] (og.south) -- (3, -1.4);
  \node[font=\small\bfseries, text=orange1] at (3, -1.75) {$h_t$};
...
% ============================================================
\begin{frame}
\frametitle{LSTM --- gate equations}
\vspace{-0.2cm}
\begin{center}
\begin{tikzpicture}[scale=0.92, transform shape]
  % Gate equations
  \node[draw=sampred, fill=sampred!8, rounded corners=6pt, text width=11cm, align=left, inner sep=10pt, font=\normalsize] at (0, 3) {
    \textbf{\textcolor{sampred}{Forget gate:}} \quad $f_t = \sigma\!\big(W_f \cdot [h_{t-1},\, x_t] + b_f\big)$
  };

  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=6pt, text width=11cm, align=left, inner sep=10pt, font=\normalsize] at (0, 1.5) {
    \textbf{\textcolor{paramgreen}{Input gate:}} \quad $i_t = \sigma\!\big(W_i \cdot [h_{t-1},\, x_t] + b_i\big)$ \qquad
    $\tilde{c}_t = \tanh\!\big(W_c \cdot [h_{t-1},\, x_t] + b_c\big)$
  };

  \node[draw=popblue, fill=popblue!8, rounded corners=6pt, text width=11cm, align=left, inner sep=10pt, font=\normalsize] at (0, 0) {
    \textbf{\textcolor{popblue}{Cell update:}} \quad $c_t = f_t \odot c_{t-1} \;+\; i_t \odot \tilde{c}_t$
  };

  \node[draw=orange1, fill=orange1!8, rounded corners=6pt, text width=11cm, align=left, inner sep=10pt, font=\normalsize] at (0, -1.5) {
    \textbf{\textcolor{orange1}{Output gate:}} \quad $o_t = \sigma\!\big(W_o \cdot [h_{t-1},\, x_t] + b_o\big)$ \qquad
    $h_t = o_t \odot \tanh(c_t)$
  };

  % Key insight
  \node[draw=violet1, fill=violet1!5, rounded corners=6pt, text width=12cm, align=center, inner sep=8pt, font=\small] at (0, -3.2) {
    \textbf{Why it works:} the cell state update is \textbf{additive} ($c_t = f_t \odot c_{t-1} + \ldots$), not multiplicative like vanilla RNNs.\\[3pt]
    Gradients flow through the $c_{t-1} \rightarrow c_t$ path with minimal decay --- solving the vanishing gradient problem.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
% ============================================================
% DEEPER & BIDIRECTIONAL RNNs
% ============================================================
\begin{frame}
\frametitle{Deeper and bidirectional RNNs}
\vspace{-0.15cm}
{\small Two standard extensions --- both used by ELMo, next.}

\vspace{0.3cm}
\begin{center}
\begin{tikzpicture}[scale=0.9, transform shape, font=\small,
  n/.style={draw, circle, minimum size=0.5cm, inner sep=0pt}]
  % ===== Stacked (left) =====
  \node[font=\small\bfseries, text=popblue] at (-3.5, 2.7) {Stacked (multi-layer)};
  \foreach \i in {0,1,2,3} {
    \node[n, fill=paramgreen!20] (l1\i) at (-5.2 + \i*1.1, 0.7) {};
    \node[n, fill=paramgreen!40] (l2\i) at (-5.2 + \i*1.1, 1.7) {};
    \draw[-Stealth, gray] (l1\i) -- (l2\i);
  }
  \foreach \i/\j in {0/1,1/2,2/3} {
    \draw[-Stealth, paramgreen] (l1\i) -- (l1\j);
    \draw[-Stealth, paramgreen!70] (l2\i) -- (l2\j);
  }
  \node[font=\scriptsize, text=gray, anchor=east] at (-5.7, 0.7) {layer 1};
  \node[font=\scriptsize, text=gray, anchor=east] at (-5.7, 1.7) {layer 2};
  \node[font=\scriptsize, text width=3.7cm, align=center] at (-3.5, -0.45) {higher layers learn more abstract features};

  % ===== Bidirectional (right) =====
  \node[font=\small\bfseries, text=orange1] at (3.3, 2.7) {Bidirectional};
  \foreach \i in {0,1,2,3} {
    \node[n, fill=paramgreen!25] (fw\i) at (1.7 + \i*1.1, 1.5) {};
    \node[n, fill=sampred!25] (bw\i) at (1.7 + \i*1.1, 0.6) {};
  }
  \foreach \i/\j in {0/1,1/2,2/3} {
    \draw[-Stealth, paramgreen] (fw\i) -- (fw\j);
    \draw[-Stealth, sampred] (bw\j) -- (bw\i);
  }
  \node[font=\scriptsize, text=paramgreen, anchor=west] at (5.1, 1.5) {$\overrightarrow{h}$};
  \node[font=\scriptsize, text=sampred, anchor=west] at (5.1, 0.6) {$\overleftarrow{h}$};
  \node[font=\scriptsize, text width=4.0cm, align=center] at (3.3, -0.45) {each position sees \textbf{left and right} context: $[\overrightarrow{h}_t;\, \overleftarrow{h}_t]$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
  \node[draw=warnred, fill=warnred!6, rounded corners=6pt, text width=12.6cm, align=center, inner sep=7pt, font=\small] {
    Bidirectional RNNs are great for \textbf{encoding / tagging} (the whole sentence is known) but \textbf{not} for autoregressive generation --- you can't peek at future tokens you haven't produced yet.
  };
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{tikzpicture}[scale=0.82, transform shape]
  ...
  % biLSTM architecture
  \node[font=\small\bfseries, text=popblue] at (-6.7, 1.5) {biLSTM};
  ...
  \node[font=\tiny, text=paramgreen] at (-7.8, 0.9) {$\overrightarrow{h}^2$};
  \foreach \i/\j in {1/2, 2/3, 3/4, 4/5} {
...
\begin{tikzpicture}[scale=0.82, transform shape]
  ...
  \node[font=\scriptsize\bfseries] at (-4.7, -2.4) {BLEU score};
...
\begin{tikzpicture}[scale=0.88, transform shape]
  ...
  % Decoder
  \node[font=\small\bfseries, text=orange1] at (4.6, 2.5) {Decoder step $t$};
  \node[draw, circle, fill=orange1!20, minimum size=0.9cm, font=\scriptsize] (dt) at (4.6, 1.5) {$s_t$};

  % Context vector = weighted sum of the encoder states (sits between encoder and decoder)
  \node[draw=sampred, fill=sampred!15, rounded corners=4pt, minimum size=0.7cm, font=\small] (cvec) at (1.5, 2.3) {$c_t$};
  \node[font=\scriptsize, text=sampred] at (1.5, 2.9) {weighted sum};

  % Attention arrows: each encoder state -> context vector, thickness ~ weight
  \draw[-Stealth, thin, sampred!30] (e1) -- (cvec);
  \draw[-Stealth, sampred!55] (e2) -- (cvec);
  \draw[-Stealth, thick, sampred!80] (e3) -- (cvec);
  \draw[-Stealth, very thick, sampred] (e4) -- (cvec);

  % Attention weights on their arrows
  \node[font=\tiny, text=sampred] at (-4.0, 1.75) {$\alpha_1\!=\!.05$};
  \node[font=\tiny, text=sampred] at (-2.8, 1.9) {$\alpha_2\!=\!.10$};
  \node[font=\tiny, text=sampred] at (-1.5, 2.05) {$\alpha_3\!=\!.40$};
  \node[font=\tiny, text=sampred] at (-0.3, 2.2) {$\alpha_4\!=\!.45$};

  % Context feeds the decoder
  \draw[-Stealth, thick, orange1] (cvec) -- (dt) node[midway, above, font=\scriptsize, text=orange1] {feeds decoder};
  ...
  \node[draw=popblue, fill=popblue!5, rounded corners=6pt, inner sep=6pt, text width=12cm, align=center, font=\small] at (0, -0.5) {
    ...
    $\alpha_{ti} = \mathrm{softmax}(e_{ti})$ \qquad
...
% WORD2VEC TRAINING
% ============================================================
\section{Beyond Word2Vec \& wrap-up}
\begin{frame}
...
\begin{tikzpicture}[scale=0.88, transform shape]
  ...
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=5.3cm, align=left, inner sep=6pt] at (3.2, 2.3) {
    ...
    {\scriptsize \texttt{<where>} $\rightarrow$ \texttt{<wh, whe, her, ere, re>}}\\[3pt]
    {\small Handles OOV words \& morphology}
  };

  % Table
  \node[font=\small, text width=13cm, align=center] at (0, -1.0) {
  ...
  % Bottom note
  \node[draw=paramgreen, fill=paramgreen!5, rounded corners=6pt, text width=12cm, align=center, inner sep=5pt, font=\small] at (0, -2.95) {
...
\begin{tikzpicture}[scale=0.88, transform shape]
  ...
  \node[draw=orange1, fill=orange1!8, rounded corners=8pt, text width=13cm, align=left, inner sep=6pt] at (0, 2.9) {
  ...
  % Word Embeddings
  \node[draw=paramgreen, fill=paramgreen!8, rounded corners=8pt, text width=13cm, align=left, inner sep=6pt] at (0, 1.05) {
  ...
  \node[draw=popblue, fill=popblue!8, rounded corners=8pt, text width=13cm, align=left, inner sep=6pt] at (0, -1.85) {

...
% ──────────────────────────────────────────────
\begin{frame}{Previously, on Agent Skills\ldots}
\begin{columns}[T]
\column{0.5\textwidth}
\begin{block}{What Are Skills?}
Reusable markdown instructions (\texttt{SKILL.md}) that Claude activates automatically via semantic matching.
\end{block}

\begin{block}{Frontmatter}
\texttt{name}, \texttt{description}, \texttt{allowed-tools}, \texttt{model}, \texttt{context}, \texttt{user-invocable}.
\end{block}

\begin{block}{Locations}
Personal (\texttt{\textasciitilde/.claude/skills/}) vs.\ Project (\texttt{.claude/skills/}).
\end{block}

\column{0.5\textwidth}
\begin{block}{Matching}
Only names + descriptions loaded at startup. Full body loaded after confirmation.
\end{block}

\begin{block}{Sharing}
Via git, plugins, or enterprise. Priority: Enterprise $>$ Personal $>$ Project $>$ Plugins.
\end{block}
\end{columns}
\end{frame}
...
\begin{itemize}
  \item Parent sets \texttt{run\_in\_background: true}
...
\column{0.5\textwidth}
\textbf{\color{orange1}Shared Memory}
\begin{itemize}
  \item Agents share the file-based memory system (\texttt{\textasciitilde/.claude/} directory)
...
\begin{itemize}
  ...
  \item \texttt{@"reviewer (agent)"} (explicit mention)
  \item SDK: \texttt{query(\{ agent: "reviewer" \})}
\end{itemize}
\end{columns}
\end{frame}

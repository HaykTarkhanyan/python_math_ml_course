...
\begin{itemize}
  ...
  \item Claude Code vs.\ Copilot
  \item Coding LLM benchmarks
\end{itemize}
...
Same Claude 3.5 Sonnet: \textbf{49\%} basic scaffold $\to$ \textbf{64\%} agentic scaffolding (+30\% relative gain). \\
{\small Orchestration intelligence matters as much as raw model intelligence.}
\end{block}
\end{frame}

% ──────────────────────────────────────────────
\begin{frame}{Top LLMs for Coding (March 2026)}
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{\color{popblue}SWE-bench Verified} {\small(500 real GitHub issues)}
\smallskip

{\small
\begin{tabular}{rlr}
\toprule
\textbf{\#} & \textbf{Model} & \textbf{Score} \\
\midrule
1 & Claude Opus 4.5 \textrm{\tiny(Anthropic)} & 80.9\% \\
2 & Claude Opus 4.6 \textrm{\tiny(Anthropic)} & 80.8\% \\
3 & Gemini 3.1 Pro \textrm{\tiny(Google)} & 80.6\% \\
4 & MiniMax M2.5 \textrm{\tiny(open-weight)} & 80.2\% \\
5 & GPT-5.2 \textrm{\tiny(OpenAI)} & 80.0\% \\
6 & Claude Sonnet 4.6 \textrm{\tiny(Anthropic)} & 79.6\% \\
7 & GLM-5 \textrm{\tiny(Zhipu AI)} & 77.8\% \\
\bottomrule
\end{tabular}}

\column{0.42\textwidth}
\textbf{\color{sampred}SWE-bench Pro} {\small(1,865 multi-lang tasks)}
\smallskip
...
\medskip
\begin{alertblock}{\small Contamination caveat}
\footnotesize OpenAI found training data leakage in Verified. Pro is cleaner but fewer models submit.
\end{alertblock}
\end{columns}

\vspace{-0.5em}
\footnotesize
Source: \href{https://www.swebench.com}{swebench.com}, \href{https://labs.scale.com/leaderboard/swe_bench_pro_public}{Scale Labs} (March 2026). Scores change rapidly --- check leaderboards for latest.
\end{frame}
